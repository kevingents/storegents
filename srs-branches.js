...
export async function createLabelRecord(input) {
  ...
  const record = {
    ...
    senderAddressName: input.senderAddressName || '',
    ...
    shippingCurrency: input.shippingCurrency || 'EUR',
    costBreakdown: input.costBreakdown || [],
    status: input.status || '',
    shipmentState: input.shipmentState || 'open',
...
export async function updateLabelsByParcelId(updatesByParcelId) {
  const labels = await getLabels();
  let changed = false;

  const nextLabels = labels.map((label) => {
    const updates = updatesByParcelId[String(label.parcelId)];
