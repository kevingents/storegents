...
function shouldUseSrs() {
  return String(process.env.SRS_OPEN_WEBORDERS_SOURCE || 'srs').toLowerCase() !== 'local';
}
...
export async function getSrsOpenWeborders({ store, branchId } = {}) {
  ...
  try {
    ...
    for (const id of branchIds) {
      const items = await getOpenFulfillmentsByBranch(id, { includeDetails: String(process.env.SRS_WEBORDERS_INCLUDE_DETAILS || 'true') !== 'false' });
