...
/* Opt-in kandidaten met winkel verzamelen; stopt zodra genoeg NIEUWE.
   extraFilter (bv. { updatedFrom }) → alleen recent gewijzigde klanten (goedkoop). */
async function collectCandidates(synced, cap, { maxPages = 30, pageSize = 500, extraFilter = {} } = {}) {
  const seen = new Set();
  const fresh = [];
  let scannedOptIn = 0;
  for (let page = 1; page <= maxPages && fresh.length < cap; page++) {
    let batch;
    try { batch = await getCustomers({ page, pageSize, ...extraFilter }); } catch (_) { break; }
...
/**
 * @param {object} opts { dryRun=true, limit, incremental=false, sinceHours=6 }
 *   incremental → alleen klanten die sinds `sinceHours` zijn gewijzigd (near-realtime, goedkoop).
 */
export async function runResendAudienceSync({ dryRun = true, limit, incremental = false, sinceHours = 6 } = {}) {
  ...
  const extraFilter = incremental
    ? { updatedFrom: new Date(Date.now() - Math.max(1, Number(sinceHours) || 6) * 3600000).toISOString().slice(0, 19) }
    : {};
  const { candidates, scannedOptIn } = await collectCandidates(cfg.synced || {}, cap, { maxPages: dryRun ? 20 : 30, extraFilter });
  ...
  const result = { connected: true, ranAt: new Date().toISOString(), incremental, scannedOptIn, pushed, errors, batch: candidates.length, perStore };
  await saveResendAudienceConfig({ mainAudienceId: mainId, storeAudiences, synced, lastRun: result.ranAt, lastResult: result });
  return result;
}

/* Volledige vulling: blijft batches draaien tot er niets nieuws meer is (of tot
   tijd-/aantal-plafond). Voor de "Vul Resend nu volledig"-knop. */
export async function runResendAudienceSyncFull({ maxTotal = 8000, deadlineMs = 260000 } = {}) {
  if (!hasResendKey()) return { connected: false, error: 'RESEND_API_KEY ontbreekt in Vercel.' };
  const start = Date.now();
  let pushed = 0, errors = 0, rounds = 0;
  const perStore = {};
  while (pushed < maxTotal && (Date.now() - start) < deadlineMs) {
    const r = await runResendAudienceSync({ dryRun: false });
    rounds++;
    if (r.connected === false) return r;
    pushed += r.pushed || 0;
    errors += r.errors || 0;
    for (const [k, v] of Object.entries(r.perStore || {})) perStore[k] = (perStore[k] || 0) + v;
    if (!r.batch) break; /* geen nieuwe kandidaten meer */
  }
  const cfg = await getResendAudienceConfig();
  return { connected: true, full: true, rounds, pushed, errors, perStore, totalSynced: Object.keys(cfg.synced || {}).length };
}

/* CSV-export van alle opt-in contacten — voor de bulk-import in Resend
   (Audience → Import contacts). Kolommen: email,first_name,last_name,unsubscribed.
   Optioneel filteren op winkel (voor een per-winkel audience). */
const csvCell = (v) => { const s = String(v == null ? '' : v); return /[",\n;]/.test(s) ? '"' + s.replace(/"/g, '""') + '"' : s; };

export async function collectOptInContactsCsv({ storeFilter = '', maxPages = 200, pageSize = 500 } = {}) {
  const seen = new Set();
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    for (const c of custRows) {
      ...
      seen.add(email);
      rows.push([email, csvCell(c.firstName), csvCell(c.lastName), 'false'].join(','));
      count++;
    }
    if (custRows.length < pageSize) break;
  }
