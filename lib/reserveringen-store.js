...
export async function createReservering(input) {
  ...
  const reservering = {
    ...
    srsSyncStatus: 'pending', /* 'pending' → 'bill_created' / 'paid' / 'failed' / 'skipped_no_price' */
    srsTransactionId: '',
    srsBillNr: '',           /* Door SRS Bills uitgereikt nummer */
    srsBillAmount: 0,         /* Bedrag van de openstaande bill */
    srsBillBranchId: '',      /* BranchId waar de bill is geboekt (RES-filiaal) */
    srsError: '',             /* Laatste SRS-foutmelding indien sync faalde */
...
export async function updateReservering(id, patch = {}, actor = '') {
  ...
  if (patch.srsBillNr) next.srsBillNr = String(patch.srsBillNr).trim();
  if (patch.srsBillAmount !== undefined) next.srsBillAmount = Number(patch.srsBillAmount) || 0;
  if (patch.srsBillBranchId) next.srsBillBranchId = String(patch.srsBillBranchId).trim();
  if (patch.srsError !== undefined) next.srsError = String(patch.srsError || '').trim().slice(0, 500);
  if (patch.customer) {
