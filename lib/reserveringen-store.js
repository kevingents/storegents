...
export async function createReservering(input) {
  ...
  const reservering = {
    ...
    srsSyncStatus: 'pending', /* 'pending' → 'weborder_created' / 'failed' */
    srsTransactionId: '',      /* Bij weborder-sync: SRS orderId (bv. R...) */
...
export async function updateReservering(id, patch = {}, actor = '') {
  ...
  if (patch.srsRawSnippet !== undefined) next.srsRawSnippet = String(patch.srsRawSnippet || '').slice(0, 500);
