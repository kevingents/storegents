...
async function postSoap(action, xml) {
  ...
  try {
    const response = await fetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'text/xml; charset=utf-8', SOAPAction: action }, body: xml, signal: controller.signal });
...
function buildIdentifierXml({ fulfillmentId = '', orderLineNr = '' } = {}) {
  ...
  if (cleanFulfillmentId) return `<web:FulfillmentId>${xmlEscape(cleanFulfillmentId)}</web:FulfillmentId>`;
  if (cleanOrderLineNr) return `<web:OrderLineNr>${xmlEscape(cleanOrderLineNr)}</web:OrderLineNr>`;
...
function parseCancelResponse({ raw, action, orderNr, fulfillmentId, orderLineNr, sku, pieces, price, dateTime }) {
  ...
  return { success: responseSuccess, responseSuccess, action, status, statuses, messages, transactionId, orderNr, fulfillmentId: clean(fulfillmentId), orderLineNr: clean(orderLineNr), sku, pieces, price, dateTime, raw };
}
...
function rowSummary(row = {}) {
  return { id: row.id, orderNr: row.orderNr, fulfillmentId: row.fulfillmentId, orderLineNr: row.orderLineNr, sku: row.sku || row.barcode, status: row.status, normalizedStatus: normalizeStatus(row.status), isUnavailable: isSrsUnavailableStatus(row.status), isCancelled: isSrsCancelledStatus(row.status) };
}
...
async function verifyCancelOnce({ orderNr, fulfillmentId, orderLineNr, sku } = {}) {
  ...
  return { success: matchingRows.length > 0 && cancelled && !stillUnavailable, cancelled, stillUnavailable, foundMatchingLine: matchingRows.length > 0, statuses, matchingRows: matchingRows.map(rowSummary), allRows: rows.map(rowSummary), raw: result.raw };
}
...
function cancelVariants(options = {}) {
  const variants = [options];
  if (Number(options.pieces || 0) > 1) {
    variants.push({ ...options, pieces: 1, price: Number(options.price || 0) / Number(options.pieces || 1) });
    variants.push({ ...options, pieces: 1 });
  }
  return variants;
}

export async function cancelFulfillment({ orderNr, fulfillmentId = '', orderLineNr = '', sku = '', barcode = '', pieces = 1, price = 0, dateTime = '' } = {}) {
  ...
  const baseOptions = { orderNr: cleanOrderNr, fulfillmentId, orderLineNr, sku: cleanSku, pieces: cleanPieces, price: cleanPrice, dateTime: cancelDateTime };
  const attempts = [];
  const errors = [];

  for (const options of cancelVariants(baseOptions)) {
    for (const action of CANCEL_ACTIONS) {
      try {
        ...
        if (attempt.success) return { ...attempt, attempts };
      } catch (error) {
        errors.push({ action, pieces: options.pieces, price: options.price, message: error.message || String(error), status: error.status || null, fault: error.fault || null, responseText: error.responseText || '' });
      }
    }
  }
  ...
  return { success: false, responseSuccess: Boolean(best?.responseSuccess), status: best?.status || 'not_confirmed', statuses: best?.statuses || [], messages: [...(best?.messages || []), message], transactionId: best?.transactionId || '', orderNr: cleanOrderNr, fulfillmentId: clean(fulfillmentId), orderLineNr: clean(orderLineNr), sku: cleanSku, pieces: cleanPieces, price: cleanPrice, dateTime: cancelDateTime, verification: lastVerification, attempts, errors, raw: best?.raw || '' };
}
