...
function buildIdentifierXml({ fulfillmentId = '', orderLineNr = '' } = {}) {
  ...
  // Bij niet-leverbare regels is OrderLineNr de meest stabiele SRS sleutel.
  // FulfillmentId blijft fallback voor regels waar OrderLineNr ontbreekt.
  if (cleanOrderLineNr) {
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tran="https://messages.storeinfo.nl/v1/Weborders/Transactions" xmlns:com="https://messages.storeinfo.nl/v1/Common">
      ...
      <tran:Body>
        ...
        <tran:Items>
          <tran:Item>
            ...
            <tran:Pieces>${xmlEscape(cleanPieces)}</tran:Pieces>
            <tran:Price>${xmlEscape(cleanPrice.toFixed(2))}</tran:Price>
          </tran:Item>
        </tran:Items>
      </tran:Body>
    </tran:Cancel>
  ...
  const success = statuses.length ? statuses.some(isCompletedStatus) : isCompletedStatus(status);
