...
/**
 * Zoek bestaande Shopify-bundles en bouw een omgekeerde index
 * (component-productId → bundle(s)), zodat we kunnen zien of een colbert/broek
 * al in een bundle zit (voorkomt dubbele aanmaak). Scant producten op het
 * Product.bundleComponents-veld. Fout-tolerant: bij een schema-/API-probleem
 * geven we checked:false terug i.p.v. de hele pagina te breken.
 */
export async function findExistingBundles({ maxPages = 15 } = {}) {
  const cfg = shopCfg();
  if (!cfg) return { checked: false, configured: false, bundles: [], byComponent: {}, count: 0 };

  const Q = `query($cursor:String){
    products(first:100, after:$cursor){
      pageInfo{ hasNextPage endCursor }
      edges{ node{ id title handle status
        bundleComponents(first:30){ edges{ node{ componentProduct{ id title } } } }
      } }
    }
  }`;
  ...
  try {
    while (pages < maxPages) {
      ...
      if (json?.errors) throw new Error(JSON.stringify(json.errors).slice(0, 180));
      const conn = json?.data?.products;
      if (!conn) break;

      for (const e of (conn.edges || [])) {
        const n = e?.node || {};
        const compEdges = n.bundleComponents?.edges || [];
        if (!compEdges.length) continue;
        const componentIds = compEdges.map((c) => clean(c?.node?.componentProduct?.id)).filter(Boolean);
        const b = { id: clean(n.id), title: clean(n.title), handle: clean(n.handle), status: clean(n.status), componentIds };
        bundles.push(b);
        for (const cid of componentIds) {
          (byComponent[cid] = byComponent[cid] || []).push({ id: b.id, title: b.title, handle: b.handle });
        }
      }
      pages += 1;
    ...
    return { checked: true, configured: true, bundles, byComponent, count: bundles.length, truncated };
  } catch (e) {
    return { checked: false, configured: true, error: e.message || 'bundle-check faalde', bundles: [], byComponent: {}, count: 0 };
  }
}
