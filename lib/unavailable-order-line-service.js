...
/* Verdacht-signaal uit de lost&found-cron (srs-unavailable-lost-found-check): die
   schrijft het signaal weg op het item (line.lostFoundCheck.signal) én top-level op
   de cancellation. We lezen het per item; fallback op cancellation-niveau alléén bij
   een 1-regel cancellation, zodat het signaal van regel A niet aan regel B plakt. */
...
function isUnavailableLineStatus(value) {
  ...
  return status.includes('niet leverbaar') ||
    ...
    /* SRS-cancelled fulfillments tellen ook als niet-leverbaar (schade):
       warehouse kon niet leveren → cancelled in SRS = niet leverbaar voor klant */
    status.includes('cancelled in srs') ||
    status.includes('canceled in srs') ||
    status.includes('geannuleerd in srs') ||
    /* Sources van automatische SRS sync — alle cancelled fulfillments uit SRS */
    status.includes('srs historic nightly backfill') ||
    status.includes('srs get fulfillments order line sync') ||
    status.includes('srs get fulfillments order lines per store');
}
...
function lineRowsForCancellation(cancellation = {}) {
  ...
  return lines.map((line, index) => {
    ...
    return {
      ...
      source,
      // Een al verwerkte regel (auto óf handmatig 'gemeld als verwerkt') mag NIET
      // teruggeforceerd worden naar 'open' op basis van de SRS-bron — anders blijft
      // 'ie eeuwig in de open-lijst staan ondanks dat refund + cancel klaar zijn.
      status:
        cancellation.status === 'processed' || cancellation.processedAt
          ? 'processed'
          : isUnavailableLineStatus(source) || isUnavailableLineStatus(srsLineStatus)
            ? 'open'
      ...
      ...suspicionFields(line, cancellation, lines.length === 1),
...
/** Bol/POS-orders lopen niet via de Shopify-refund-flow (en de SRS-sync hercreëert
    ze telkens → duplicaten). We weren ze uit het niet-leverbaar-overzicht. */
function isBolOrPosOrder(row = {}) {
  return [row.orderNr, row.shopifyOrderNr, row.srsOrderNr, row.customerOrderNr, row.weborderNr]
    .map((v) => clean(v).replace(/^#/, '').toUpperCase())
    .some((v) => /^BOL[-_]/.test(v) || /^POS\d/.test(v));
}
...
export async function listUnavailableOrderLines({ store = '', status = 'open', dateFrom = '', dateTo = '', query = '' } = {}) {
  ...
  // Bol/POS weren: geen Shopify-refund mogelijk en bron van de duplicaten.
  rows = rows.filter((row) => !isBolOrPosOrder(row));
