...
function lineRowsForCancellation(cancellation = {}) {
  ...
  return lines.map((line, index) => {
    ...
    const srsCancelStatus = clean(cancellation.srsCancelStatus || cancellation.srsCancelResult?.status || '') || (
      ...
      normalizeStatus(cancellation.srsStatus).includes('cancelled') ? 'cancelled_in_srs' : 'pending'
    );
...
function isCompleted(row) {
  const refundDone = normalizeStatus(row.refundStatus).includes('refunded') || normalizeStatus(row.refundStatus).includes('already');
  const srsCancelDone = normalizeStatus(row.srsCancelStatus).includes('cancel') || normalizeStatus(row.srsStatus).includes('cancelled in srs') || normalizeStatus(row.srsStatus).includes('cancelled_in_srs');
...
async function cancelInSrs(row, { force = false } = {}) {
  ...
  if (!force && current.includes('cancel')) return row.originalCancellation.srsResult || { success: true, skipped: true, status: 'already_cancelled' };
...
async function refundInShopify(row, { force = false, employeeName = '' } = {}) {
  ...
  if (!force && (current.includes('refund') || current.includes('already'))) {
    return row.originalCancellation.refundResult || { success: true, skipped: true, status: 'already_refunded', matchedAmount: row.amount || 0, refundAmount: 0 };
  }
...
export async function processUnavailableOrderLine({ id, steps = ['refund', 'srs_cancel'], employeeName = 'Administratie', force = false } = {}) {
  ...
  try {
    if (steps.includes('refund')) {
      ...
      const amountPatch = patchLineAmounts(cancellation, row, results.refund?.refundAmount || results.refund?.matchedAmount || row.amount || 0);
