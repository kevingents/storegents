...
function freeShippingDiscountCode() {
  return clean(
    process.env.UNAVAILABLE_FREE_SHIPPING_CODE ||
    process.env.FREE_SHIPPING_DISCOUNT_CODE ||
    process.env.GRATIS_VERZENDING_CODE ||
    'GRATISVERZENDING'
  );
}
...
function isCompleted(row) {
  const mailDone = normalizeStatus(row.mailStatus) === 'sent';
  const refundDone =
    normalizeStatus(row.refundStatus).includes('refunded') ||
  ...
  return mailDone && refundDone && srsCancelDone;
}
...
async function sendUnavailableMail(row, { force = false } = {}) {
  ...
  const code = freeShippingDiscountCode();

  if (!to) throw new Error('Klant e-mail ontbreekt. Excuusmail kan niet worden verzonden.');
  if (!apiKey) throw new Error('RESEND_API_KEY ontbreekt. Excuusmail kan niet worden verzonden.');

  const subject = `Onze excuses: artikel niet leverbaar in order ${row.orderNr}`;
  const html = `
    ...
    <p>Je ontvangt via Shopify een aparte terugbetaalbevestiging voor dit artikel. De overige artikelen in je bestelling blijven gewoon doorlopen.</p>
    <p>Als excuus ontvang je van ons een kortingscode voor gratis verzending bij je volgende bestelling:</p>
    <p><strong style="font-size:18px;">${code}</strong></p>
  ...
  return { success: true, provider: 'resend', id: data.id || '', to, subject, freeShippingCode: code };
}
...
export async function processUnavailableOrderLine({
  ...
  if (!row) throw new Error('Niet-leverbare orderregel niet gevonden. Geannuleerde regels vallen buiten deze workflow.');
  ...
  try {
    ...
    return {
      ...
      results,
      message: 'Niet-leverbare orderregel verwerkt. Alleen deze regel is geannuleerd/terugbetaald; overige orderregels blijven ongemoeid. Voorraad is niet teruggeboekt.'
    };
  } catch (error) {
    patch.status = 'failed';
