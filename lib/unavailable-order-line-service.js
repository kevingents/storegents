...
function isSrsCancelDone(row = {}) {
  return isPositiveSrsCancelStatus([row.srsCancelStatus, row.srsStatus].filter(Boolean).join(' '));
}
...
async function cancelInSrs(row, { force = false } = {}) {
  ...
  if (!force && isPositiveSrsCancelStatus(current)) return row.originalCancellation.srsResult || { success: true, skipped: true, status: 'already_cancelled' };
    ...
    if (requiredSteps.includes('srs_cancel')) {
      try {
        ...
        await writeProcessLog(row, {
          ...
          refundStatus: patch.refundStatus,
          srsCancelStatus: patch.srsCancelStatus,
          message: results.srsCancel?.success ? 'SRS orderregel geannuleerd.' : patch.error,
