...
function isSrsCancelDone(row = {}) {
  const status = normalizeStatus([row.srsCancelStatus, row.srsStatus].filter(Boolean).join(' '));
...
async function cancelInSrs(row, { force = false } = {}) {
  const current = normalizeStatus(row.originalCancellation.srsStatus || row.originalCancellation.srsCancelStatus);
...
function finalStatusFromPatch(patch = {}) {
  ...
  const srsDone = normalizeStatus([patch.srsCancelStatus, patch.srsStatus].filter(Boolean).join(' ')).includes('cancel');
    ...
    if (requiredSteps.includes('srs_cancel')) {
      try {
        ...
        await writeProcessLog(row, {
          ...
          refundStatus: patch.refundStatus,
          srsCancelStatus: patch.srsCancelStatus,
          message: results.srsCancel?.success ? 'SRS orderregel geannuleerd.' : 'SRS cancel gaf geen success terug.',
