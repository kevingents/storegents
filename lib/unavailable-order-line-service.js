...
function isProblemLineStatus(value) {
  ...
  return status.includes('niet leverbaar') ||
    ...
    status.includes('annulled') ||
    status.includes('srs global get fulfillments unavailable') ||
    status.includes('srs global get fulfillments cancelled') ||
    status.includes('srs global fulfillments') ||
    status.includes('order cancellation') ||
...
function isUnavailableLike(row = {}) {
  ...
  return isProblemLineStatus(value);
}
...
function lineRowsForCancellation(cancellation = {}) {
  ...
  return lines.map((line, index) => {
    ...
    return {
      ...
      reason: cancellation.reason || 'Niet leverbaar / geannuleerd',
      ...
      source,
      status: isProblemLineStatus(source) || isProblemLineStatus(srsLineStatus) ? 'open' : (cancellation.status || 'open'),
...
export async function processUnavailableOrderLine({
  ...
  if (!row) throw new Error('Niet-leverbare/geannuleerde orderregel niet gevonden.');
  ...
  try {
    ...
    return {
      ...
      results,
      message: 'Orderregel verwerkt. Voorraad is niet teruggeboekt.'
    };
  } catch (error) {
    patch.status = 'failed';
