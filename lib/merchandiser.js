 ...
 */
export function buildRedistribution(rows, { cacheBySku = {}, costBySku = {}, physical = null, minUnits = 1, maxSuggestions = 300 } = {}) {
  const bySku = new Map();
  for (const r of rows || []) {
  ...
  suggestions.sort((a, b) => (b.value || 0) - (a.value || 0) || b.units - a.units);
...
export async function buildMerchandiser(view = 'overview', { limit = 300 } = {}) {
  ...
  const [rows, summary, advies, cost, cache] = await Promise.all([
  ...
  const meta = { voorraadAt: summary && summary.generatedAt, adviesAt: advies && advies.generatedAt, rijen: rows.length };

  if (view === 'herverdeling') {
    return { view, ...buildRedistribution(rows, { cacheBySku, costBySku, physical, maxSuggestions: limit }), meta };
  }
  if (view === 'misgrijpen') {
  ...
  const herv = buildRedistribution(rows, { cacheBySku, costBySku, physical, maxSuggestions: 8 });
