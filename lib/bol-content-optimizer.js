...
export async function buildBolContentPlan() {
  ...
  const stored = {
    ...
    buckets,
    contentScore,
    itemsTotaal: items.length,
    items: items.slice(0, 2000) /* dashboard toont max 2000; blob klein houden */
  };
  try { await writeJsonBlob(PATH, stored); } catch (_) {}
  /* In-memory geven we de VOLLEDIGE items terug — de autonome push moet de hele
     catalogus kunnen dekken, niet alleen de eerste 2000. */
  return { ...stored, items };
}
