...
function parseCustomer(block) {
  ...
  return {
    ...
    */
    ...
    /* Studentvereniging / klantgroep — kan onder verschillende XML-tags zitten
       afhankelijk van SRS-config. We proberen de meest waarschijnlijke. */
    studentvereniging: textFromFirst(block, [
      'Studentvereniging',
      'Studentenvereniging',
      'StudentAssociation',
      'StudentenVereniging',
...
function bodyFiltersXml(filters = {}) {
  ...
  */
  ...
  if (filters.page || filters.pageSize) {
    parts.push(`
      <data:Pagination>
        ${filters.page ? `<com:Page>${xmlEscape(filters.page)}</com:Page>` : ''}
        ${filters.pageSize ? `<com:PageSize>${xmlEscape(filters.pageSize)}</com:PageSize>` : ''}
      </data:Pagination>
    `);
  }
