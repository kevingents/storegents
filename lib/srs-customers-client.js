...
function parseCustomer(block) {
  ...
  const email = emailBlocks
    .map((emailBlock) => getNodeText(emailBlock, 'EmailAddress'))
    .find(Boolean) || '';
  ...
  return {
    ...
    street: getNodeText(addressBlock, 'Address1'),
    ...
    country: getNodeText(addressBlock, 'Country') || 'NL',
    raw: block
  };
}
