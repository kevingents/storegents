import { googleRefreshToken } from './google-oauth-token.js';

const GOOGLE_TOKEN_URL = 'https://oauth2.googleapis.com/token';
const GOOGLE_REVIEWS_BASE_URL = 'https://mybusiness.googleapis.com/v4';
const GOOGLE_ACCOUNTS_BASE_URL = 'https://' + 'mybusinessaccountmanagement.googleapis.com/v1';
const GOOGLE_LOCATIONS_BASE_URL = 'https://' + 'mybusinessbusinessinformation.googleapis.com/v1';
...
function readConfig() {
  ...
  if (!refreshToken) throw new Error('Geen Google refresh token — zet GOOGLE_REFRESH_TOKEN in Vercel (geautoriseerd met de business.manage-scope).');
...
async function fetchGoogleJson(baseUrl, path, { timeoutMs = DEFAULT_TIMEOUT_MS, query = {} } = {}) {
...
export async function listBusinessAccounts() {
  const data = await fetchGoogleJson(GOOGLE_ACCOUNTS_BASE_URL, '/accounts');
...
export async function listBusinessLocations({ accountId = '', pageSize = 100, pageToken = '' } = {}) {
  ...
  const parent = `accounts/${normalizeAccountName(configuredAccountId)}`;
  const data = await fetchGoogleJson(GOOGLE_LOCATIONS_BASE_URL, `/${parent}/locations`, {
    query: {
...
export async function listBusinessReviews({ accountId = '', locationId = '', store = '', branchId = '', pageSize = 50, pageToken = '', orderBy = 'updateTime desc' } = {}) {
  ...
  const data = await fetchGoogleJson(GOOGLE_REVIEWS_BASE_URL, `/accounts/${encodeURIComponent(effectiveAccountId)}/locations/${encodeURIComponent(effectiveLocationId)}/reviews`, {
