  ...
  return `
    <tr>
      <td style="padding:0 0 14px;">
        <div style="font-size:15px;line-height:1.35;color:${PRIMARY};font-weight:700;">${escapeHtml(question)}</div>
        <div style="margin-top:5px;font-size:14px;line-height:1.6;color:${SECONDARY};">${answer}</div>
      </td>
    </tr>`;
}
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:720px;background:#ffffff;border:1px solid ${BORDER};border-radius:30px;overflow:hidden;box-shadow:0 18px 45px rgba(10,31,51,.08);">
          ...
          <tr>
            <td style="padding:26px 34px 10px;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#ffffff;border:1px solid ${BORDER};border-radius:24px;">
                <tr>
                  <td style="padding:24px;">
                    ...
                    <p style="margin:0;color:${SECONDARY};font-size:15px;line-height:1.65;">Je kunt de barcode in de winkel laten scannen. De voucher moet in één keer worden besteed. ${onlineText}</p>
                    <div style="height:18px;"></div>
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:14px;background:${SOFT};border:1px solid ${BORDER};border-radius:24px;">
                <tr>
                  <td style="padding:24px;">
                    <h2 style="margin:0 0 16px;color:${PRIMARY};font-size:24px;line-height:1.15;font-weight:400;letter-spacing:-.03em;">Veelgestelde vragen</h2>
                    <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                      ${faqItem('Hoe lang is mijn voucher geldig?', `De voucher is 3 maanden geldig vanaf de uitgiftedatum. De exacte einddatum staat hierboven bij <strong style="color:${PRIMARY};">Geldig t/m</strong>.`)}
                      ${faqItem('Kan ik de voucher laten uitbetalen?', 'Nee, de voucher is niet inwisselbaar voor geld en kan niet contant worden uitgekeerd.')}
                      ${faqItem('Kan ik de voucher online gebruiken?', `Ja. Wil je de voucher digitaal gebruiken, zet hem dan eerst om via <a href="${VOUCHER_EXCHANGE_URL}" style="color:${PRIMARY};font-weight:700;text-decoration:underline;">gents.nl/pages/voucher-omwisselen</a>. Daarna is de oorspronkelijke SRS-code niet meer actief.`)}
                      ${faqItem('Moet ik de hele waarde in één keer besteden?', 'Ja. De voucher moet in één keer worden uitgegeven. Er wordt geen restwaarde uitgekeerd of bewaard wanneer je minder besteedt dan de voucherwaarde.')}
                      ${faqItem('Wat als de barcode niet scant?', 'Laat dan de vouchercode handmatig invoeren aan de kassa. Controleer daarbij goed of alle cijfers kloppen.')}
                      ${faqItem('Waar kan ik de voucher gebruiken?', `Je kunt de voucher gebruiken in onze winkels. Bekijk alle locaties via <a href="${STORE_LOCATOR_URL}" style="color:${PRIMARY};font-weight:700;text-decoration:underline;">de winkellocator</a>.`)}
                    </table>
                  </td>
                </tr>
              </table>
...
export async function sendVoucherEmail({
  ...
  return resend.emails.send({
    ...
    tags: [
      { name: 'category', value: 'voucher' },
      { name: 'kind', value: 'new-voucher' }
    ],
...
export async function sendVoucherReminderEmail({
  ...
  return resend.emails.send({
    ...
    subject: isExpiryReminder
      ? `Je GENTS voucher vervalt bijna`
      : `Je GENTS voucher wacht nog op je`,
    html: baseEmailTemplate({
      preheader: isExpiryReminder
        ? `Je GENTS voucher vervalt bijna. Gebruik hem voordat hij verloopt.`
        : `Je GENTS voucher staat nog klaar. Ontdek je nieuwe favorieten bij GENTS.`,
      title: isExpiryReminder
        ? 'Je voucher vervalt bijna'
        : 'Je voucher staat nog klaar',
      intro: isExpiryReminder
        ? 'Je GENTS voucher is nog niet gebruikt en vervalt binnenkort. Dit is een mooi moment om je garderobe aan te vullen met iets nieuws.'
        : 'Je GENTS voucher is nog niet gebruikt. Kom langs in één van onze winkels of zet je voucher om voor online gebruik en ontdek je nieuwe favorieten.',
      customerName,
      voucherCode,
      amount,
      currency,
      ...
      note,
      barcodeDataUrl,
      ctaText: isExpiryReminder ? 'Voucher vervalt bijna' : 'Herinnering',
      reminderText: isExpiryReminder
        ? `Gebruik je voucher uiterlijk op ${validTo}. Daarna vervalt de waarde.`
        : 'Je voucher is nog 3 maanden geldig vanaf uitgifte en moet in één keer worden besteed.'
    })
  });
}
