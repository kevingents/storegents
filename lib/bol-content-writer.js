...
export async function discoverBolCatalog(ean) {
  ...
  try { raw = await bolGet(`${base}/${e}`); }
    ...
    if (cfg.demo) throw new Error(`bol-demo gaf een fout op de voorbeeld-EAN. De demo kent alleen ${DEMO_EAN}; zet BOL_DEMO=0 (live) om je eigen artikelen uit te lezen. (${err.message})`);
    throw err;
  }
