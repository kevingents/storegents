...
const DESC_ATTR = clean(process.env.BOL_ATTR_DESCRIPTION) || 'Productbeschrijving';
...
export function buildBolPayload(item, extra = {}) {
  ...
  if (IMAGES_ON && imgs.length) {
    payload.assets = imgs.map((url, i) => ({ url, labels: [i === 0 ? 'PRODUCT_IMAGE_MAIN' : 'PRODUCT_IMAGE_ADDITIONAL'] }));
  }
  return payload;
}
