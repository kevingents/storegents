 ...
 */
export async function ensureBolFamilies({ maxCheck = 120, dryRun = false } = {}) {
  ...
  for (const it of todo) {
    ...
    if (!live) { if (resultaten.length < 100) resultaten.push({ ean: it.ean, familyName: fam, actie: heeft === false ? 'aanmaken' : 'controleren' }); continue; }
    try {
