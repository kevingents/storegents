...
const ROWS_PATH     = 'srs-voorraad/rows-latest.json';      /* DASHBOARDS/ANALYSE — eigendom van de SRS-import (3x/dag) */
const SRS_ROWS_PATH = 'srs-voorraad/srs-rows-latest.json'; /* rauwe SRS-voorraad (ALLE filialen incl. interne branches zoals 701) — Shopify-snapshot raakt dit NIET aan */
const SHOPIFY_ROWS_PATH = 'srs-voorraad/shopify-rows-latest.json'; /* LIVE Shopify-voorraad — voor operationele live-stock-consumers (bol-sync) */
const LOCATIES_PATH = 'srs-voorraad/locaties-latest.json';
const MAGNIETWINKEL_WM_PATH = 'srs-voorraad/magazijn-niet-winkel-watermark.json';
...
export async function writeVoorraadSnapshot(rows, meta = {}) {
  ...
  /* Werk de "magazijn-niet-winkel" leeftijd-watermark bij (mag nooit de
     import laten falen). */
  try { await updateMagazijnNietWinkelWatermark(rows); }
  catch (e) { console.error('[srs-voorraad-store] watermark-update faalde:', e.message); }
...
/* ──────────────────────────────────────────────────────────────────────
 * Rauwe SRS-voorraad — ALLE filialen incl. interne branches (701 fotoshoot,
 * enz.). De Shopify-snapshot bouwt rows-latest.json alleen voor filialen met
 * een Shopify-locatie en overschrijft de SRS-data; dit aparte blob bewaart de
 * volledige SRS-voorraad zodat tools voor niet-Shopify-filialen blijven werken.
 * Alleen de SRS-import schrijft hier — Shopify raakt het niet aan.
 * ────────────────────────────────────────────────────────────────────── */
export async function writeSrsVoorraadRows(rows, meta = {}) {
  await writeJsonBlob(SRS_ROWS_PATH, { rows: Array.isArray(rows) ? rows : [], generatedAt: new Date().toISOString(), ...meta });
}

export async function readSrsVoorraadRows() {
  const data = await readJsonBlob(SRS_ROWS_PATH, { rows: [] });
  return Array.isArray(data?.rows) ? data.rows : [];
}

/* ──────────────────────────────────────────────────────────────────────
 * Live Shopify-voorraad — geschreven door de shopify-stock-snapshot (elke
 * 20 min). Voor operationele live-stock-consumers die NIET op de 3x/dag
 * SRS-cijfers willen wachten (bv. bol-voorraad-sync = wat online verkoopbaar
 * is). Dashboards/analyse gebruiken juist rows-latest.json (SRS).
 * ────────────────────────────────────────────────────────────────────── */
export async function writeShopifyVoorraadRows(rows, meta = {}) {
  await writeJsonBlob(SHOPIFY_ROWS_PATH, { rows: Array.isArray(rows) ? rows : [], generatedAt: new Date().toISOString(), ...meta });
}

export async function readShopifyVoorraadRows() {
  const data = await readJsonBlob(SHOPIFY_ROWS_PATH, { rows: [] });
  const rows = Array.isArray(data?.rows) ? data.rows : [];
  if (rows.length) return rows;
  /* Fallback tijdens transitie (Shopify-snapshot nog niet gedraaid sinds deploy). */
  return readVoorraadRows();
}
 ...
 *
 * @param {Object} [opts]
 * @param {Object<string,string>} [opts.firstSeen] map sku → 'YYYY-MM-DD' (watermark):
 *        wanneer de SKU voor het eerst in deze staat kwam. Voegt per rij
 *        `sinds` (datum) + `dagen` (aantal dagen onuitgeleverd) toe.
 * @returns {Array<{ sku, magazijn, magazijnVoorraad, winkelsMetTarget, winkelTargetTotaal, sinds?, dagen? }>}
 *          gesorteerd op winkelsMetTarget (actionabel), dan langst-liggend, dan voorraad.
 */
export function computeMagazijnNietWinkel(voorraadRows, opts = {}) {
  ...
  for (const [sku, m] of magBySku.entries()) {
    ...
    if (firstSeen) {
      const d = firstSeen[sku];
      row.sinds = d || null;
      row.dagen = d ? Math.max(0, Math.floor((today - Date.parse(d)) / 86_400_000)) : null;
    }
    rows.push(row);
  }
  rows.sort((a, b) => {
    if (b.winkelsMetTarget !== a.winkelsMetTarget) return b.winkelsMetTarget - a.winkelsMetTarget;
    if (firstSeen && (b.dagen || 0) !== (a.dagen || 0)) return (b.dagen || 0) - (a.dagen || 0);
...
/* ──────────────────────────────────────────────────────────────────────
 * Watermark "magazijn-niet-winkel" — sinds wanneer ligt een SKU al
 * onuitgeleverd? De SRS-export bevat geen mutatiedatum, dus we houden zelf
 * per import bij wanneer een SKU voor het eerst in deze staat kwam.
 * ────────────────────────────────────────────────────────────────────── */

/** Lees de watermark { firstSeen: { sku: 'YYYY-MM-DD' }, updatedAt }. */
export async function readMagazijnNietWinkelWatermark() {
  return readJsonBlob(MAGNIETWINKEL_WM_PATH, { firstSeen: {}, updatedAt: null });
}

/**
 * Actualiseer de watermark op basis van de huidige voorraad-rijen.
 * - SKU nieuw in de set → firstSeen = vandaag.
 * - SKU al bekend       → behoud bestaande firstSeen (teller loopt door).
 * - SKU niet meer in de set (uitgeleverd) → verwijderd.
 * Idempotent binnen één dag (datum-granulariteit).
 */
async function updateMagazijnNietWinkelWatermark(rows) {
  const set = computeMagazijnNietWinkel(rows); /* zonder firstSeen — alleen de skus */
  /* NL-datum gebruiken: import-cron draait avond/nacht, UTC-slice zou
     firstSeen op 'morgen' zetten → aging-tellers 1 dag off + bij maand-edge
     wordt aging in de verkeerde maand geboekt. */
  const today = nlTodayIso();
  const prev = await readJsonBlob(MAGNIETWINKEL_WM_PATH, { firstSeen: {} });
  const prevSeen = (prev && prev.firstSeen) || {};
  const nextSeen = {};
  for (const r of set) nextSeen[r.sku] = prevSeen[r.sku] || today;
  await writeJsonBlob(MAGNIETWINKEL_WM_PATH, {
    firstSeen: nextSeen,
    count: set.length,
