...
/* Derving-bestemming: waar gaat het naartoe (informatief op de correctie).
   afkeur → filiaal 708 (komt niet terug); herstel → 707 (klacht, komt terug). */
export const DERVING_BESTEMMINGEN = [
  { code: 'afkeur', label: 'Afkeur / derving (komt niet terug)', filiaal: '708' },
  { code: 'herstel', label: 'Klacht / herstel (komt terug)', filiaal: '707' }
];
/* Lifecycle van een herstel-melding (alleen bij bestemming = herstel). */
export const HERSTEL_STATUSES = ['gemeld', 'in_herstel', 'hersteld', 'afgekeurd'];

/* Redenen die als derving tellen wanneer er (nog) geen bestemming gekozen is. */
export const DERVING_REASON_CODES = ['damage', 'theft', 'sample', 'gift'];

function clean(v) { return String(v ?? '').trim(); }
function normBestemming(v) { const s = clean(v).toLowerCase(); return (s === 'afkeur' || s === 'herstel') ? s : ''; }
...
function normalizeSize(input) {
  ...
  return {
    ...
    /* Per-maat identifiers — Shopify variant + SRS sizeId/variantId zodat
       admin direct in beide systemen kan terugzoeken. */
    variantId: clean(input?.variantId),
    srsSizeId: clean(input?.srsSizeId),
    countType,
...
function normalizeArticle(input) {
  ...
  return {
    sku: clean(input?.sku),
    /* SRS unieke artikel-id + Shopify product-id voor admin-koppeling */
    articleId: clean(input?.articleId),
    productId: clean(input?.productId || input?.shopifyProductId),
    barcode: clean(input?.barcode),
    shopifyProductId: clean(input?.shopifyProductId || input?.productId),
 ...
 *  - requestedByUserId
 */
/* Telt een aanvraag als derving? (expliciete bestemming, of een derving-reden). */
export function isDervingRequest(r) {
  if (normBestemming(r?.bestemming)) return true;
  return (r?.articles || []).some((a) => DERVING_REASON_CODES.includes(clean(a?.reasonCode).toLowerCase()));
}

export async function listRequests({ store, status, from, to, requestedByUserId, bestemming, derving } = {}) {
  const requests = await readAllRequests();
  ...
  const wantedBestemming = normBestemming(bestemming);
  ...
  return requests
    .filter((r) => {
      ...
      if (wantedBestemming && normBestemming(r.bestemming) !== wantedBestemming) return false;
      if (derving && !isDervingRequest(r)) return false;
...
export async function createRequest(input = {}, actor = null) {
  ...
  const bestemming = normBestemming(input.bestemming);
  ...
  const request = {
    ...
    requestedAt: now,
    status: 'pending',
    bestemming,                                          /* '' | 'afkeur' | 'herstel' */
    herstelStatus: bestemming === 'herstel' ? 'gemeld' : '',
    articles,
    note: clean(input.note),
    decidedBy: null,
    decidedAt: null,
...
/* Zet de herstel-status van een herstel-melding (klacht → 707). */
export async function setHerstelStatus(id, status, actor = null) {
  const s = clean(status).toLowerCase();
  if (!HERSTEL_STATUSES.includes(s)) throw new Error(`Onbekende herstel-status: ${status}`);
  const all = await readStore();
  const idx = all.requests.findIndex((r) => r.id === id);
  if (idx < 0) throw new Error('Aanvraag niet gevonden.');
  const existing = all.requests[idx];
  if (normBestemming(existing.bestemming) !== 'herstel') throw new Error('Herstel-status alleen voor herstel-meldingen.');
  all.requests[idx] = {
    ...existing,
    herstelStatus: s,
    herstelStatusBy: normalizeActor(actor),
    herstelStatusAt: nowIso(),
    updatedAt: nowIso()
  };
  await writeStore(all);
  return all.requests[idx];
}
