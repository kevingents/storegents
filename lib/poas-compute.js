...
export async function computePoasForRange({ from, to }) {
  ...
  /* "Gewoon" model: verzendkost = kost/zending × aantal zendingen, volledig afgetrokken.
     De klant-bijdrage (€ onder de drempel) zit al aan de omzetkant (total_price), dus
     die verrekenen we hier niet nóg een keer. verzendOpbrengstEx blijft puur ter info. */
  const verzendkostNettoEx = verzendkostBrutoEx;
  ...
  const overigeKostenEx = r2(verzendkostBrutoEx + pickingKostEx + transactieKostEx);
