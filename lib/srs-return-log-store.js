...
async function readBlobText(url) {
  /* Cache-buster + no-store: anders serveert de CDN tot cacheControlMaxAge de
     oude versie ná een write → de retouren-pagina toont een net verwerkte
     retour (of een correctie) pas na een refresh. */
...
export async function createSrsReturnLog(input) {
  ...
  const log = {
    ...
    /* Ruil: klant nam een ander artikel mee i.p.v. refund. Voor ruil-vs-retour
       cijfers per winkel (toegevoegd 2026-06). */
    exchange: Boolean(input.exchange),
    exchangeItem: String(input.exchangeItem || '').trim(),
    exchangeAmount: Number(input.exchangeAmount || 0) || 0,
