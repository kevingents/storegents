 ...
 */
export async function getReturnRequests({ createdFrom, createdTo, maxRecords = 2000, pageSize = 100 } = {}) {
 ...
 */
export async function getReturnOrders({ createdFrom, createdTo, maxRecords = 1000, pageSize = 100 } = {}) {
  const fromDate = createdFrom || new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
  const records = [];
  let page = 1;
  let hasMore = true;
