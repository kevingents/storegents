...
export async function getMetaAdsSpend({ from, to } = {}) {
  ...
  try {
    ...
    if (!resp.ok || j.error) {
      /* Detecteer OAuthException (code 190) / expired access-token expliciet zodat
         admin direct ziet dat het token het probleem is, niet de config of API. */
      const err = j.error || {};
      const isOAuth = err.code === 190 || err.type === 'OAuthException' || /access token/i.test(err.message || '');
      const friendlyMsg = isOAuth
        ? 'Meta access-token verlopen of ingetrokken. Vernieuw META_ADS_ACCESS_TOKEN in Vercel (Business Manager → System Users → token regenereren met ads_read).'
        : (err.message || `Meta API fout ${resp.status}`);
      return { ok: false, spend: null, byDay: [], error: friendlyMsg, tokenRevoked: isOAuth };
    }
    let total = 0;
    const byDay = [];
...
export async function getMetaAdsCampaigns({ from, to } = {}) {
  ...
  try {
    /* 1. Campagnes (actief + gepauzeerd) — naam, status, doel, budget. PAUSED
       wordt meegenomen zodat je ze ook vanuit de portal weer kunt activeren;
       hieronder filteren we oude gepauzeerde zonder spend eruit. */
    const cp = auth(new URLSearchParams());
    cp.set('fields', 'name,effective_status,objective,daily_budget,lifetime_budget');
    cp.set('effective_status', JSON.stringify(['ACTIVE', 'PAUSED']));
    cp.set('limit', '200');
    ...
    const campaigns = active.map((c) => {
      ...
      /* Gepauzeerde campagnes alleen tonen als ze in de periode geld uitgaven
         (recent/relevant) — anders vervuilen oude campagnes de lijst. */
      .filter((c) => c.active || c.spend > 0)
      /* Actieve eerst, daarna op spend. */
