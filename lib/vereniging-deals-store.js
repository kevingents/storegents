...
export async function upsertDeal(input = {}, createdBy = 'admin') {
  ...
  const now = nowIso();
  /* Products: vrij-tekst lijst van waar de deal voor geldt. Kan zijn:
     - Categorie: "Alle pakken", "Gilets + dassen"
     - Specifieke SKUs / artikelnummers
     - "Alle artikelen" voor algemene korting
     Wordt als string-array opgeslagen, één regel per item. */
  const products = Array.isArray(input.products)
    ? input.products.map(clean).filter(Boolean)
    : (clean(input.products)
        ? clean(input.products).split('\n').map(clean).filter(Boolean)
        : []);
