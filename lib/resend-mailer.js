...
export async function sendGentsMail({ to, subject, html, text, type = 'algemeen', store = '', meta = {}, from: fromOverride = '', replyTo = '', headers = null, tags = null }) {
  ...
  try {
    ...
    /* Resend SDK v3+ returnt { data, error } in plaats van throwen op 422
       (bv. unsubscribed/suppressed adres). Promote dat naar een echte error
       zodat we niet stilletjes 'sent' loggen voor bounces. */
    if (result?.error) {
      const errMessage = result.error.message || result.error.name || 'Resend mail-error';
