...
const MAX_MAILINGS = 24;          // begrens het aantal stat-calls per run
...
export async function refreshSpotlerMetrics() {
  ...
  /* /mailing vereist een fromDate. Doc zegt ISO 8601; sommige instances willen
     epoch-millis. Probeer ISO (zonder ms), val terug op epoch bij fromDate-fout. */
  const since = Date.now() - LOOKBACK_DAYS * 86400000;
  const fromIso = new Date(since).toISOString().replace(/\.\d{3}Z$/, 'Z');
  let list;
  try {
    list = await spotlerRequest('GET', 'mailing', { query: { fromDate: fromIso, pageSize: '100' } });
  } catch (e) {
    if (/fromdate/i.test(e.message || '')) {
      list = await spotlerRequest('GET', 'mailing', { query: { fromDate: String(since), pageSize: '100' } });
    } else throw e;
  }
