...
export async function createFacilitairOrder(input) {
  ...
  const order = {
    ...
    deliveryEta: '',
    adminNote: '',
    pickedItems: {}, /* { productId: { at, by } } — admin vinkt items af tijdens picken */
...
/**
 * Toggle "gepicked" voor een specifiek product binnen een bestelling.
 * Wanneer alle items gepicked zijn → suggestion in UI om status door te zetten.
 */
export async function togglePickedItem(orderId, productId, picked, actor = 'admin') {
  const target = String(orderId || '').trim();
  if (!target) throw new Error('Geen ordernummer opgegeven.');
  if (!productId) throw new Error('Geen product opgegeven.');
  const all = await loadAll();
  const idx = all.findIndex((row) => row.id === target);
  if (idx === -1) throw new Error('Bestelling niet gevonden.');
  const current = all[idx];
  const next = { ...current, pickedItems: { ...(current.pickedItems || {}) } };
  if (picked) {
    next.pickedItems[productId] = { at: new Date().toISOString(), by: actor };
  } else {
    delete next.pickedItems[productId];
  }
  next.updatedAt = new Date().toISOString();
  all[idx] = next;
  await saveAll(all);
