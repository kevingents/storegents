...
export async function boekUitwisseling({ vanFiliaal, naarFiliaal, referentie, regels }) {
  ...
  for (const cfg of configs) {
    ...
    try {
      ...
      /* SRS gooit SOAP Fault bij Login-fout — pak die op en probeer volgende
         credential-set. Andere fouten (netwerk, timeout, parser) gooien we
         direct door. */
      const faultMsg = err?.fault?.message || err?.message || '';
      const isLoginFault = /login.*failed|authentication|not authorized|invalid.*credentials/i.test(faultMsg);
      if (isLoginFault) {
        lastRet = faultMsg;
        lastSource = source;
        lastResponseText = err?.responseText || '';
        continue; /* probeer volgende set */
      }
      throw err;
    }
    ...
    /* Login-fout in return-tekst? Probeer volgende credential-set.
       Andere SRS-fout (bv. 'artikel niet bekend')? Stop direct. */
