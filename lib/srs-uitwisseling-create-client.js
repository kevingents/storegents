 ...
 *
 * Credentials:
 *   SRS_UITWISSEL_USER + SRS_UITWISSEL_PASSWORD (per-filiaal accounts), of
 *   SRS_MESSAGE_USER + SRS_MESSAGE_PASSWORD (global account met rechten)
 */

const DEFAULT_BASE_URL = 'https://ws.srs.nl';
const PATH = '/webservices/si_uitwisselingen.php';
const SOAP_TIMEOUT_MS = Number(process.env.SRS_UITWISSEL_TIMEOUT_MS || 20000);
...
function getConfig() {
  const username = process.env.SRS_UITWISSEL_USER || process.env.SRS_MESSAGE_USER || process.env.srs_message_user || '';
  const password = process.env.SRS_UITWISSEL_PASSWORD || process.env.SRS_MESSAGE_PASSWORD || process.env.srs_message_password || '';
  const baseUrl = (process.env.SRS_UITWISSEL_BASE_URL || process.env.SRS_BASE_URL || DEFAULT_BASE_URL).replace(/\/$/, '');
  if (!username || !password) {
    throw new Error('SRS_UITWISSEL_USER en/of SRS_UITWISSEL_PASSWORD (of SRS_MESSAGE_USER/PASSWORD) ontbreken.');
  }
