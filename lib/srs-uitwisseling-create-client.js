...
function getConfig(vanFiliaal) {
  ...
  /* 1. Dedicated SRS_API_USER_UITWISSELINGEN / SRS_API_PASS_UITWISSELINGEN
     — door GENTS in Vercel gezet als specifieke uitwisseling-credentials. */
  ...
  if (dedUser && dedPass) {
    return { username: dedUser, password: dedPass, endpoint, source: 'api-uitwisselingen' };
  }

  /* 2. Per-filiaal lookup uit SRS_UITWISSEL_CREDS_JSON */
  ...
  /* 3. Global SRS_UITWISSEL_USER/PASSWORD */
  ...
  /* 4. Fallback op SRS_MESSAGE_USER/PASSWORD */
  ...
  /* 5. Fallback op SRS_API_USER/PASSWORD — zelfde credentials als weborders */
  const apiUser = process.env.SRS_API_USER || process.env.SRS_API_USERNAME || '';
  const apiPass = process.env.SRS_API_PASSWORD || '';
  if (apiUser && apiPass) {
    return { username: apiUser, password: apiPass, endpoint, source: 'api-fallback' };
  }
  ...
  throw new Error(
    ...
    'óf een van de andere fallback opties.'
  );
}
