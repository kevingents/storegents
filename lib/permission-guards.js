...
import { rolePermissions, resolvePermissions, isValidPermission } from './user-roles.js';
import { readRolePermissions, resolveRolePermissions } from './role-permissions-store.js';

/**
 * Effectieve permissie-set voor een rol: hardcoded defaults + de rol-overrides
 * uit de beheer-store (config/role-permissions.json) — die laatste maakt zowel
 * matrix-aanpassingen als CUSTOM rollen daadwerkelijk actief — daarna de
 * per-gebruiker extra/revoked. Valt terug op hardcoded defaults als de store
 * (nog) niet leesbaar is.
 */
async function effectivePermsForRole(role, extra = [], revoked = []) {
  let base;
  try {
    const rpState = await readRolePermissions();
    base = resolveRolePermissions(role, rpState);
  } catch (e) {
    console.warn('[permission-guards] role-overrides niet leesbaar, fallback naar defaults:', e.message);
    base = rolePermissions(role);
  }
  const set = new Set(base);
  (extra || []).forEach((p) => { if (isValidPermission(p)) set.add(p); });
...
export async function getCallerPermissions(req) {
  ...
  const perms = await effectivePermsForRole(role, entry.extraPermissions || [], entry.revokedPermissions || []);
  return {
    isSystemAdmin: false,
