...
export function readAdsConfig() {
  return {
    ...
    apiVersion: clean(process.env.GOOGLE_ADS_API_VERSION) || 'v21'
  };
}
...
export async function probeGoogleAds() {
  ...
  else if (ads && ads.error) diagnosis = /\b404\b/.test(ads.error)
    ? `Ads-API 404 — de API-versie (${cfg.apiVersion}) is waarschijnlijk verlopen/onjuist. Zet GOOGLE_ADS_API_VERSION op de huidige Google Ads API-versie (bv. v21 of nieuwer).`
    : `Ads-API gaf een fout: ${ads.error}.${cfg.customerId ? '' : ' Stel ook GOOGLE_ADS_CUSTOMER_ID in.'}`;
