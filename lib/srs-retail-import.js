...
import { listBranches, getStoreNameByBranchId, isWarehouseStore } from './branch-metrics.js';
...
import { writeVelocity } from './verkoop-velocity-store.js';
...
/* Herkomst-filiaal van een retour uit het retour_bonnummer "V<fil><bon>". */
function originFiliaalOf(rb) { const m = String(rb || '').match(/^V?(\d{3})/); return m ? String(Number(m[1])) : ''; }

/* Web-RETOUR aan de kassa: een negatieve regel op een fysiek filiaal waarvan het
   oorspronkelijke bon uit het magazijn/web (filiaal 99/97 = warehouse) komt. Die
   retour hoort bij de WEBSHOP, niet bij de winkel — anders duwt 'ie de
   winkel-omzet onterecht negatief. */
function isWebReturnRow(r = {}) {
  if (toInt(r.gerealiseerd_bedrag) >= 0) return false;       /* alleen retour-regels */
  const orig = originFiliaalOf(r.retour_bonnummer);
  if (!orig) return false;
  if (orig === '99' || orig === '97') return true;
  const st = getStoreNameByBranchId(orig);
  return st ? isWarehouseStore(st) : false;
}

/* Een regel telt NIET als winkel(-omzet/-retour): weborder-verwerking óf web-retour. */
function isStoreExcludedRow(r = {}) { return isWeborderRow(r) || isWebReturnRow(r); }
...
function buildRetourData(verkopen, physical) {
  ...
  for (const r of verkopen) {
    ...
    if (isStoreExcludedRow(r)) continue;
...
export async function diagnoseVerkopen({ remotePath = '/Dataexport', sampleSize = 5, filiaal = '', datum = '' } = {}) {
  ...
  if (fFil && fDat) {
    ...
    const byExtOrder = new Map(), byBarcode = new Map(), byGescand = new Map(), byBon = new Map();
    ...
    for (const r of verkopen) {
      ...
      const ext = String(r.ext_ordernumber || '').trim();
      bump(byExtOrder, 'ext ' + (ext ? ext.replace(/[0-9]+$/, '#') : '(leeg)'), cents);
      bump(byBarcode, 'barcode ' + (r.bekende_barcode ?? '(leeg)'), cents);
      bump(byGescand, 'gescand ' + (r.verkoopbon_gescand || '(leeg)'), cents);
      bump(byBon, String(r.bon_nummer || '(leeg)'), cents);
      if (regelsCompact.length < 300) regelsCompact.push({
        bon: r.bon_nummer, soort: r.verkoop_soort, ext, bc: r.bekende_barcode,
        scan: r.verkoopbon_gescand, memo: r.memo || '', sku: r.sku_code, aantal: r.aantal,
        eur: round2(cents / 100)
      });
    }
    ...
    focus = {
      ...
      uniekeBonnen: byBon.size,
      perAfdeling: fmt(byAfdeling),
      perVerkoopSoort: fmt(bySoort),
      perBtw: fmt(byBtw),
      perRekening: fmt(byRekening),
      perExtOrder: fmt(byExtOrder),
      perBarcode: fmt(byBarcode),
      perGescand: fmt(byGescand),
      regelsCompact
    };
  }
...
export async function rebuildLedger({ remotePath = '/Dataexport', maxWindows = 120 } = {}) {
  ...
  for (const win of windows) {
    ...
    for (const r of verkopen) {
      ...
      const datum = String(r.datum || '').trim();
      if (!physical.has(fil) || !datum) continue;
      if (isStoreExcludedRow(r)) { excludedWebRows += 1; excludedWebCents += toInt(r.gerealiseerd_bedrag); continue; }
      const cents = toInt(r.gerealiseerd_bedrag);
...
export async function importRetailPerformance({ remotePath = '/Dataexport', windowDays = DEFAULT_WINDOW_DAYS } = {}) {
  ...
  try {
    ...
    for (const r of verkopen) {
      ...
      const datum = String(r.datum || '').trim();
      if (!physical.has(fil) || !datum) continue;
      if (isStoreExcludedRow(r)) continue; /* weborder-verwerking + web-retour → webshop, niet winkel */
      const cents = toInt(r.gerealiseerd_bedrag);
      const qty = Math.abs(toInt(r.aantal));
      const bon = String(r.bon_nummer || '');
  ...
  /* Per-artikel verkoopsnelheid (sku × winkel) over het venster → Merchandiser.
     Bruto verkoop (geen retouren), alleen fysieke winkels. Additief/optioneel. */
  try {
    const bySkuStore = {}, bySku = {};
    for (const r of verkopen) {
      if (String(r.verkoop_soort || '').toLowerCase() !== 'verkoop') continue;
      const sku = String(r.sku_code || '').trim();
      const fil = String(r.filiaal_nummer || '').trim();
      if (!sku || !fil || !physical.has(fil)) continue;
      const q = Math.abs(toInt(r.aantal)) || 0;
      if (!q) continue;
      const k = `${sku}|${fil}`;
      bySkuStore[k] = (bySkuStore[k] || 0) + q;
      bySku[sku] = (bySku[sku] || 0) + q;
    }
    if (Object.keys(bySkuStore).length) {
      await writeVelocity({ bySkuStore, bySku, windowDays, from: fromDate, to: maxDate });
      snapshot.velocitySkus = Object.keys(bySku).length;
    }
  } catch (e) {
    console.error('[srs-retail-import] velocity faalde:', e.message);
  }
