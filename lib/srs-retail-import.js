 ...
 */
export async function diagnoseVerkopen({ remotePath = '/Dataexport', sampleSize = 5 } = {}) {
  ...
  /* Retour-analyse: van de RETOUR-regels (negatief bedrag bij fysiek filiaal),
     hoeveel dragen de weborder-memo? Zo zien we of winkel-verwerkte WEBSHOP-
     retouren herkenbaar zijn (en dus van webshop i.p.v. winkel af moeten). */
  ...
  for (const r of verkopen) {
    if (!physical.has(String(r.filiaal_nummer || '').trim())) continue;
    const cents = toInt(r.gerealiseerd_bedrag);
    if (cents >= 0) continue; /* alleen retouren */
    retour.totalRows += 1; retour.totalEur += cents;
    if (String(r.retour_code || '').trim()) retour.retourCodes.add(String(r.retour_code).trim());
