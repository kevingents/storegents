...
function srsArtikelId(item = {}) {
  return clean(firstFilled(
    nested(item, 'SRSERP.artikel_id'),
    nested(item, 'SRSERP.article_id'),
    nested(item, 'srserp.artikel_id'),
    nested(item, 'srsERP.artikel_id'),
    item.SRSERP_artikel_id,
    item.srserp_artikel_id,
    item.artikel_id,
    item.articleId,
    item.article_id
...
function srsRveArtikelnummer(item = {}) {
  return clean(firstFilled(
    nested(item, 'SRSERP.rve_artikelnummer'),
    nested(item, 'SRSERP.rve_article_number'),
    nested(item, 'srserp.rve_artikelnummer'),
    nested(item, 'srsERP.rve_artikelnummer'),
    item.SRSERP_rve_artikelnummer,
    item.srserp_rve_artikelnummer,
    item.rve_artikelnummer,
    item.rveArticleNumber,
    item.rve_article_number,
    item.articleNumber,
    item.artikelnummer
  ));
}
...
function productImage(item = {}) {
  return clean(firstFilled(
    ...
    item.featuredImage,
    nested(item, 'product.imageUrl'),
    nested(item, 'product.image.url'),
    nested(item, 'product.featuredImage.url'),
    item.imageUrl,
    item.image,
    nested(item, 'image.url'),
    nested(item, 'image.src')
  ));
}
...
export function isOpenWeborderStatus(status) {
  ...
  return value.includes('open') || value.includes('aangemaakt') || value.includes('aangevraagd') || value.includes('in behandeling') || value.includes('te verzenden');
}

export function isClosedWeborderStatus(status) {
  const value = lower(status);
  if (!value) return false;
  return value.includes('geleverd') || value.includes('afgerond') || value.includes('geannuleerd') || value.includes('cancelled') || value.includes('canceled') || value.includes('closed') || value.includes('done');
}

export function isDeliveredLineLocation(value) {
  const location = lower(stripLocationPrefix(value));
  return location === 'klant' || location.includes('klant') || location.includes('uitlevertafel') || location.includes('uitgeleverd') || location.includes('geleverd');
}

export function isWarehouseLineLocation(value) {
  const raw = lower(value);
  const location = lower(stripLocationPrefix(value));
  return raw.startsWith('99 -') || location === 'magazijn' || location.includes('magazijn') || location.includes('warehouse') || location.includes('webshop');
}

export function isOverdueWeborder(item = {}, deadlineHours = 48) {
  return isOpenWeborderStatus(item.status) && !isClosedWeborderStatus(item.status) && getAgeInHours(item.createdAt || item.orderDate || item.created || item.dateTime) >= Number(deadlineHours || 48);
}

export function getOrderKey(item = {}) {
  return clean(firstFilled(item.orderNr, item.orderNumber, item.orderName, item.orderId, item.weborderNumber, item.deliveryOrder, item.leveropdracht, item.fulfillmentId, item.id));
}
...
export function getOrderLineKey(item = {}) {
  return clean(firstFilled(
    ...
    item.fulfillmentId && firstFilled(item.sku, item.barcode, srsRveArtikelnummer(item), srsArtikelId(item)) ? `${item.fulfillmentId}-${firstFilled(item.sku, item.barcode, srsRveArtikelnummer(item), srsArtikelId(item))}` : '',
    `${getOrderKey(item)}-${firstFilled(item.lineNr, item.lineNumber, item.articleId, item.articleNumber, item.artikelnummer, item.sku, item.barcode, srsArtikelId(item), srsRveArtikelnummer(item))}-${firstFilled(item.currentBranchId, item.huidigBranchId, item.fulfilmentBranchId, item.fulfillmentBranchId)}`
  ));
}

export function normalizeWeborder(item = {}) {
  const originLocation = firstFilled(item.originStore, item.herkomstStore, item.herkomstFiliaalNaam, item.herkomstFiliaal, item.sellingStore, item.fromStore, item.sourceStore);
  const currentLocation = firstFilled(item.currentStore, item.huidigFiliaalNaam, item.huidigFiliaal, item.currentBranchName, item.currentBranch, item.fulfilmentStore, item.fulfillmentStore, item.store, item.branchName);
  const originStore = stripLocationPrefix(originLocation);
  const currentStore = stripLocationPrefix(currentLocation);
  const originBranchId = clean(firstFilled(item.originBranchId, item.herkomstBranchId, extractBranchId(originLocation), item.sellingBranchId));
  const currentBranchId = clean(firstFilled(item.currentBranchId, item.huidigBranchId, item.huidigFiliaalId, extractBranchId(currentLocation), item.fulfilmentBranchId, item.fulfillmentBranchId, item.branchId));
  const status = clean(firstFilled(item.status, item.srsStatus, item.lineStatus, item.orderLineStatus, item.fulfillmentStatus, item.statusLabel));
  const createdAt = clean(firstFilled(item.createdAt, item.orderDate, item.created, item.dateTime, item.datum));
  const artikelId = srsArtikelId(item);
  const rveArtikelnummer = srsRveArtikelnummer(item);
  const barcode = clean(firstFilled(item.barcode, item.Barcode, item.barCode, item.ean, item.EAN, item.gtin, item.GTIN, item.productBarcode, item.variantBarcode));
  const sku = clean(firstFilled(item.sku, item.SKU, item.productSku, item.variantSku, rveArtikelnummer, artikelId, item.articleNumber, item.artikelnummer, barcode));
  ...
  const normalized = {
    ...
    articleId: artikelId,
    srsArtikelId: artikelId,
    rve_artikelnummer: rveArtikelnummer,
    rveArtikelnummer,
    productImageUrl: productImage(item),
...
export function groupOpenLinesByOrder(lines = [], storeName = '') {
  const openLines = lines.map(normalizeWeborder).filter((line) => isOrderLineOpenForStore(line, storeName));
  const map = new Map();
...
/* Naam waaronder magazijn-orders als aanspreekbare entiteit worden gerapporteerd. */
export const WAREHOUSE_STORE_LABEL = process.env.WEBORDER_WAREHOUSE_LABEL || 'GENTS Magazijn';

export function summarizeOverdueByStore(requests = []) {
  const normalized = requests.map(normalizeWeborder);
  const map = new Map();
  normalized.forEach((item) => {
    if (item.closed || item.delivered) return;
    if (!isOpenWeborderStatus(item.status)) return;
    /* Magazijn telt mee als eigen, aanspreekbare "winkel"-rij (i.p.v. uitgesloten),
       zodat het magazijn ook op te-late orders aangesproken kan worden. */
    const store = item.warehouse ? WAREHOUSE_STORE_LABEL : item.currentStore;
    if (!store) return;
    if (!map.has(store)) {
      map.set(store, { store, openCount: 0, openLineCount: 0, overdueCount: 0, overdueLineCount: 0, overdueRate: 0, oldestAgeHours: 0, items: [], orderKeys: new Set(), overdueOrderKeys: new Set() });
    }
  ...
  return Array.from(map.values()).map((row) => {
    ...
    return { store: row.store, openCount, openLineCount: row.openLineCount, overdueCount, overdueLineCount: row.overdueLineCount, overdueRate: openCount ? Math.round((overdueCount / openCount) * 100) : 0, oldestAgeHours: row.oldestAgeHours, items: row.items };
