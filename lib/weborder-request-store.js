...
export function isOverdueWeborder(item = {}, deadlineHours = 48) {
  return isOpenWeborderStatus(item.status) &&
    getAgeInHours(item.createdAt) >= Number(deadlineHours || 48);
}

export function normalizeWeborder(item = {}) {
  const fulfilmentStore = item.fulfilmentStore || item.fulfillmentStore || item.store || '';
  const fulfilmentBranchId = item.fulfilmentBranchId || item.fulfillmentBranchId || item.branchId || '';
...
export function summarizeOpenWeborders(requests = [], storeName = '') {
  ...
  const overdue = open.filter((item) =>
    (item.sellingStore === storeName || item.fulfilmentStore === storeName) &&
    item.overdue
  );
...
export function summarizeOverdueByStore(requests = []) {
  const normalized = requests
    ...
    .filter((item) => isOpenWeborderStatus(item.status));

  const map = new Map();

  normalized.forEach((item) => {
    const store = item.fulfilmentStore || item.fulfillmentStore || item.sellingStore || 'Onbekend';
