...
export const EVENT_TYPES = ['online-zonder-foto', 'new-bol-order'];
...
function autoName(trigger) {
  ...
  if (trigger.event === 'new-bol-order') return 'Nieuwe bol-bestelling';
...
/**
 * Verwijder een regel. Returnt het aantal werkelijk verwijderde regels (0 of 1)
 * zodat de aanroeper kan onderscheiden tussen "niet gevonden" en "verwijderd".
 */
export async function deleteRule(id) {
  ...
  await mutateJsonBlob(PATH, (d0) => {
    ...
    d.rules = d.rules.filter((r) => String(r.id) !== String(id));
    removed = before - d.rules.length;
    return d;
  }, { fallback: { rules: [] }, cacheMaxAge: 0 });
  return removed;
}
