...
function buildLineItems(bolItems, cache, priceByEan) {
  ...
  for (const it of (bolItems || [])) {
    ...
    /* Bol-prijs uit detail-call: stuur als string met 2 decimalen zoals Shopify
       verwacht. Als geen prijs bekend → Shopify gebruikt variant-default. */
...
export async function pushBolOrdersToShopify({ dryRun = false, maxPerRun = 50, force = false } = {}) {
  ...
  for (const order of orders) {
    ...
    const { items: lineItems, missing } = buildLineItems(order.items || [], productsCache, priceByEan);
    if (!lineItems.length) {
      skippedNoVariant += 1;
      results.push({
        orderId,
