...
function buildLineItems(bolItems, cache, priceByEan) {
  ...
  for (const it of (bolItems || [])) {
    ...
    const srsSku = clean(it.sku || it.offerReference || it.product?.reference || ''); /* SKU-fallback voor lookup */
...
export async function pushBolOrdersToShopify({ dryRun = false, maxPerRun = 50, force = false } = {}) {
  ...
  for (const order of orders) {
    ...
    const { items: lineItems, missing, lookupHits } = buildLineItems(baseItems, productsCache, priceByEan);
    if (!lineItems.length) {
      skippedNoVariant += 1;
      results.push({
        orderId,
