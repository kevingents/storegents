...
import { bolGet, getBolConfig, bolOrdersVersion } from './bol-client.js';
...
/**
 * Extract pure-numerieke Shopify variant-ID uit een cache-entry of live-search-
 * result. Cache slaat een GraphQL GID op (gid://shopify/ProductVariant/12345);
 * live-search returnt al de pure numerieke string. Beide moeten naar Number.
 *
 * Critical: zonder deze strip wordt Number('gid://shopify/...') = NaN,
 * waardoor de push elke cache-hit ten onrechte skipt met
 * "variant gevonden maar zonder Shopify variant_id".
 */
function extractNumericVariantId(v) {
  if (!v) return 0;
  const raw = String(v.shopifyVariantId || v.variantId || v.id || '').trim();
  if (!raw) return 0;
  const numeric = raw.replace(/^gid:\/\/shopify\/ProductVariant\//, '');
...
async function buildLineItemsWithFallback(cfg, bolItems, cache, priceByEan) {
  ...
  for (const it of (bolItems || [])) {
    ...
    const variantId = extractNumericVariantId(v);
    if (!variantId) {
...
async function fetchBolOrderDetail(orderId) {
  try {
    const data = await bolGet(`/orders/${encodeURIComponent(orderId)}`, { version: bolOrdersVersion() });
    return data || null;
  } catch (e) {
...
export async function pushBolOrdersToShopify({ dryRun = false, maxPerRun = 50, force = false } = {}) {
  ...
  for (const order of orders) {
    ...
    /* Bepaal of magazijn-voorraad ontbreekt op een van de items — voor tag-only,
       NIET om de push te skippen. We zetten ALTIJD door zodat bol de order niet
       annuleert wegens uitblijvende verzendbevestiging. Bij missende voorraad
       voegen we 'backorder' tag toe zodat het in Shopify direct zichtbaar is. */
    const hasNoStock = Array.isArray(baseItems) && baseItems.some((it) => it && (it.magazijnLeverbaar === false || it.magazijnVoorraad === 0));
    const tagList = ['bol-marketplace', 'channable-bypass', `bol-id-${orderId}`];
    if (hasNoStock) tagList.push('backorder', 'niet-op-magazijn');
    const payload = {
      order: {
        ...
        /* decrement_ignoring_policy: trek voorraad af MAAR negeer inventory_policy=deny.
           Anders kan Shopify de order weigeren bij variants zonder voorraad. Voor
           bol-orders willen we NOOIT geweigerd worden — bol annuleert anders. */
