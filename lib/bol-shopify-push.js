...
function buildLineItems(bolItems, cache, priceByEan) {
  const items = [];
  const missing = [];
  const lookupHits = { barcode: 0, 'sku-as-ean': 0, 'srs-sku': 0, 'barcode-as-sku': 0 };
  for (const it of (bolItems || [])) {
    const ean = clean(it.ean);
    const srsSku = clean(it.sku || ''); /* meestal niet aanwezig in bol-cache, maar veilig fallback */
    const qty = Math.max(1, Number(it.qty || it.quantity || 1));
    const { variant: v, hit } = lookupVariantByEan(cache, ean, srsSku);
    if (!v) {
...
export async function pushBolOrdersToShopify({ dryRun = false, maxPerRun = 50, force = false } = {}) {
  ...
  for (const order of orders) {
    ...
    /* EAN → prijs map uit detail (unitPrice per orderItem). */
    const priceByEan = new Map();
    if (detailOk) {
      for (const di of (detail.orderItems || [])) {
        const eanKey = String(di.ean || di.product?.ean || '').toLowerCase();
        const up = Number(di.unitPrice);
        if (eanKey && Number.isFinite(up) && up > 0) priceByEan.set(eanKey, up);
      }
    }

    /* Map line items via EAN → Shopify variant, met prijs uit detail. */
    const { items: lineItems, missing, lookupHits } = buildLineItems(order.items || [], productsCache, priceByEan);
    if (!lineItems.length) {
      skippedNoVariant += 1;
      results.push({
        orderId,
