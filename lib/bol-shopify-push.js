 ...
 * de barcode niet kent (oude producten waar cache-build iets miste, of net
 * ingevulde barcodes vóór de 03:00 UTC refresh-cron).
 *
 * Gebruikt dezelfde productVariants(query: "barcode:X") syntax die Shopify
 * search-UI ook gebruikt — wat search vindt, vinden wij ook.
 */
async function searchShopifyVariantByBarcode(cfg, barcode) {
  const q = `barcode:${String(barcode).trim()}`;
  const query = `query VariantByBarcode($q: String!) {
  ...
  try {
    ...
    /* Eerste actieve product wint; anders eerste hit. */
    const active = nodes.find((n) => clean(n.product?.status).toLowerCase() === 'active') || nodes[0];
    const variantId = clean(active.id).replace(/^gid:\/\/shopify\/ProductVariant\//, '');
    return {
      shopifyVariantId: variantId,
      sku: clean(active.sku),
      barcode: clean(active.barcode),
      productTitle: clean(active.product?.title),
      status: clean(active.product?.status)
    };
  } catch (_) {
    return null;
  } finally {
