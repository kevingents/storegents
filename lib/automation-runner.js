...
export async function getAutomationConfig(id) {
  ...
  /* Géén .catch(() => null): bij read-fout MOET de aanroeper een echte error
     krijgen. Anders zou een transient blob-fout de defaults teruggeven (zonder
     'sent'-ledger) → saveAutomationConfig schrijft die overheen → de dedupe-set
     van al gemailde klanten is weg → bij volgende run krijgen duizenden klanten
     opnieuw dezelfde marketing-mail (verjaardag/winback). */
  const stored = await readJsonBlob(blobPath(id), null);
...
export async function saveAutomationConfig(id, patch = {}) {
  const def = AUTOMATIONS[id];
  if (!def) throw new Error(`Onbekende automation: ${id}`);
  /* mutateJsonBlob doet read-modify-write atomic (met retry op write-conflict):
     beschermt tegen lost-update als cron + admin tegelijk schrijven, én tegen
     het overschrijven van 'sent'/'processed' bij een transient leesfout
     (mutator krijgt fallback alleen als blob écht leeg/nieuw is). */
  const fallback = { enabled: false, ...def.defaults, sent: {}, processed: {}, lastRun: null, lastResult: null };
  return mutateJsonBlob(blobPath(id), (cur) => ({
    ...(cur || fallback),
    ...patch,
    updatedAt: new Date().toISOString()
  }), { fallback, cacheMaxAge: 0 });
}
