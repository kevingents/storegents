...
/**
 * Set van álle sku's én barcodes die ergens (welke winkel/magazijn dan ook) op
 * voorraad liggen (pieces > 0), lowercased. Voor "alleen op voorraad"-filters
 * over een grote set artikelen (beeldbank/catalogus) — één snapshot-pass, daarna
 * O(1) lookup per artikel.
 *
 * @returns {Promise<Set<string>>}
 */
export async function inStockKeySet() {
  const branches = listAllBranches();
  const snaps = await Promise.all(branches.map(async (b) => {
  ...
  for (const snap of snaps) {
    ...
    for (const r of snap.rows) {
      if (Number(r.pieces || 0) <= 0) continue;
      const s = lower(r.sku);
      const b = lower(r.barcode);
      if (s) set.add(s);
      if (b) set.add(b);
    }
  }
