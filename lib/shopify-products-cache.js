 ...
 *     image, images: [url], collections: [title], videos: [{url,preview}],
...
async function fetchAllProductsViaGraphQL(cfg) {
      ...
      products(first: 100, after: $cursor) {
        ...
        edges {
          node {
            ...
            media(first: 10) { edges { node { mediaContentType ... on Video { sources { url } preview { image { url } } } } } }
            options { name values }
            metafields(first: 50) {
              edges { node { namespace key value } }
            }
  ...
  const PAGE_TIMEOUT_MS = Number(process.env.SHOPIFY_GRAPHQL_PAGE_TIMEOUT_MS || 30000);
  let truncated = false;

  while (pages < MAX_PAGES) {
    /* Per-page timeout via AbortController: Shopify-hiccup op pagina 200 zou
       de hele cron tot Vercel maxDuration (5-10 min) laten hangen zonder dit. */
    const ctrl = new AbortController();
    const timer = setTimeout(() => ctrl.abort(), PAGE_TIMEOUT_MS);
    let resp;
    try {
      resp = await fetch(`https://${cfg.shop}/admin/api/${cfg.version}/graphql.json`, {
        method: 'POST',
        headers: {
      ...
      if (err?.name === 'AbortError') {
        throw new Error(`Shopify GraphQL pagina ${pages + 1} timeout na ${PAGE_TIMEOUT_MS}ms.`);
      }
      throw err;
    }
    clearTimeout(timer);
  ...
  if (pages >= MAX_PAGES) {
    truncated = true;
    console.warn(`[shopify-products-cache] MAX_PAGES (${MAX_PAGES}) bereikt — tail mogelijk gemist (${products.length} producten ingelezen).`);
  }
...
function buildVariantsFromProduct(product, shop) {
  ...
  /* Collecties waar dit product in zit (titel) — voor de Marketing-beeldbank. */
  const collections = (product?.collections?.edges || [])
    .map((e) => clean(e?.node?.title))
    .filter(Boolean);
  /* Geüploade video's (Shopify-media) — voor de beeldbank. */
  ...
  /* Stof-/detailvelden — o.a. voor Mix & Match (pakken combineren). */
  const materiaal           = clean(productMetafields['materiaal']);
  const samenstelling       = clean(productMetafields['samenstelling_materiaal'] || productMetafields['samenstelling']);
  const pasvorm             = clean(productMetafields['pasvorm']);
  const sluiting            = clean(productMetafields['sluiting']);
  const mixAndMatch         = clean(productMetafields['mix_and_match']);
