...
function renderHeader() {
  return section(`
    ...
    <tr><td align="center" bgcolor="${BRAND.bgHeader}" style="background-color:${BRAND.bgHeader};padding:15px">
      <a href="https://gents.nl"><img src="${ASSETS.logoWit}" alt="GENTS" width="228" style="display:inline-block;max-width:228px;height:auto;border:0"></a>
    </td></tr>
  `);
}
...
/* — WELKOM-CONTENT: "Hey {voornaam}," + intro + info-blokken (openingstijden,
   vermaak, loyalty, voucher) + dark-navy CTA-knop. */
...
/* — REVIEWS: 5-sterren banner + "uit 1081 beoordelingen" + 3 review-cards.
   Statisch (zelfde voor alle winkels), 1:1 conform Spotler-template. */
function renderReviews() {
  const reviewCol = (r) => `<td width="33%" valign="top" bgcolor="${BRAND.bgSection}" style="background-color:${BRAND.bgSection};padding:10px 12px 40px;vertical-align:top">
      <a href="${GENTS_CONTACT.reviewsUrl}" style="text-decoration:none"><img src="${ASSETS.starBanner}" alt="" width="136" style="display:block;max-width:100%;height:auto;border:0;margin-bottom:14px"></a>
  ...
  return section(`
    ...
    <tr><td align="center" bgcolor="${BRAND.bgSection}" style="background-color:${BRAND.bgSection};padding:10px 20px">
      <img src="${ASSETS.starBanner}" alt="5 sterren" width="300" style="display:inline-block;max-width:300px;height:auto;border:0">
      <p style="color:${BRAND.text};font:400 13px/1.5 ${BRAND.font};margin:8px 0 0">uit 1081 beoordelingen</p>
    </td></tr>
    <tr><td bgcolor="${BRAND.bgSection}" style="background-color:${BRAND.bgSection};padding:15px 8px 20px">
      <table cellpadding="0" cellspacing="0" border="0" role="presentation" width="100%" style="border-collapse:collapse"><tbody><tr>
        ${STATIC_REVIEWS.map(reviewCol).join('')}
      </tr></tbody></table>
    </td></tr>
  `, BRAND.bgSection);
}
