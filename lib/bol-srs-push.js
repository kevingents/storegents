...
function buildBolWeborderXml({ orderId, bolOrderId, detail, productsXmlStr, total, placedAt, routing = {} }) {
  ...
  const sellingBranchId = clean(routing.sellingBranchId || process.env.SRS_BOL_SELLING_BRANCH_ID || '700'); /* Showroom default */
...
export async function pushBolOrdersToSrs({ dryRun = false, maxPerRun = 50, force = false, onlyBolOrderId = '' } = {}) {
  ...
  return {
    ...
    summary: {
      ...
      srsSellingBranchId: clean(routing.sellingBranchId || process.env.SRS_BOL_SELLING_BRANCH_ID || '700'),
