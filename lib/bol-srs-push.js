...
function buildBolWeborderXml({ orderId, bolOrderId, detail, productsXmlStr, total, placedAt, routing = {} }) {
  ...
  /* Routing uit de bol-instellingen (tool) met env/ingebouwde fallback.
     shopId = "GEPLAATST IN" in SRS. 91 = "GENTS Webshop Bol" (apart van 90 =
     gewone webshop). LET OP: bewust GEEN fallback op SRS_WEBORDER_SHOP_ID, want
     dat is 90 — dan zouden bol-orders weer in de gewone webshop belanden. */
  const sellingBranchId = clean(routing.sellingBranchId || process.env.SRS_BOL_SELLING_BRANCH_ID || '91'); /* 91 = omzet boekt op GENTS Webshop Bol */
  const fulfilmentBranchId = clean(routing.fulfilmentBranchId || process.env.SRS_BOL_FULFILMENT_BRANCH_ID || '99'); /* Magazijn default */
  const shopId = clean(routing.shopId || process.env.SRS_BOL_SHOP_ID || '91');
...
export async function pushBolOrdersToSrs({ dryRun = false, maxPerRun = 50, force = false, onlyBolOrderId = '', weborderUser = '' } = {}) {
  ...
  return {
    ...
    summary: {
      ...
      /* Welke SRS-shop/filialen zijn gebruikt — zo zie je direct of de routing klopt. */
      srsShopId: clean(routing.shopId || process.env.SRS_BOL_SHOP_ID || '91'),
      ...
      /* Met welke SRS-login is geplaatst — die bepaalt de webshop ("Geplaatst in"). */
