...
import { bolGet, bolOrdersVersion } from './bol-client.js';
import {
  getCachedWeborderSessionForUser,
  invalidateWeborderSessionForUser
...
function buildBolWeborderXml({ orderId, bolOrderId, detail, productsXmlStr, total, placedAt, routing = {} }) {
  ...
  /* Routing uit de bol-instellingen (tool) met env/ingebouwde fallback.
     LET OP (bevestigd in SRS si_weborder-doc): <shopid> is een LEGACY-veld —
     SRS negeert de waarde ("kan altijd 10 worden gevuld"). De échte webshop
     ("GEPLAATST IN", 90 vs 91 = GENTS Webshop Bol) wordt bepaald door de
     API-USER waarmee we inloggen (programma "Webshops" in SRS), NIET door dit
     XML-veld. Webshop-routing gaat dus via weborderUser/srsWeborderUser, niet
     via shopId. Het fulfilment-filiaal gaat via extended_attribute afhaal_filiaal
     (moet vooraf met SRS geregistreerd zijn, anders error 140). */
  const sellingBranchId = clean(routing.sellingBranchId || process.env.SRS_BOL_SELLING_BRANCH_ID || '91'); /* via extended_attribute, indien geregistreerd */
  const fulfilmentBranchId = clean(routing.fulfilmentBranchId || process.env.SRS_BOL_FULFILMENT_BRANCH_ID || '99'); /* afhaal_filiaal (magazijn) */
  const shopId = clean(process.env.SRS_BOL_SHOP_ID || '10'); /* legacy — genegeerd door SRS */
 ...
 */
export async function pushBolOrdersToSrs({ dryRun = false, maxPerRun = 50, force = false, onlyBolOrderId = '', weborderUser = '' } = {}) {
  const [bolData, pushedState, productsCache, cancelState, settings] = await Promise.all([
    readBolOrders().catch(() => null),
    readPushedState(),
    readProductsCache().catch(() => null),
    readBolCancellationsState().catch(() => ({ cancelled: {} })),
    getBolSettings().catch(() => ({}))
  ]);
  ...
  /* SRS-routing (welke shop + filialen) uit de bol-instellingen. */
  const routing = {
    shopId: settings?.srsShopId,
    sellingBranchId: settings?.srsSellingBranchId,
    fulfilmentBranchId: settings?.srsFulfilmentBranchId
  };

  /* SRS-weborder-login die bepaalt in welke WEBSHOP de order landt (de login is
     aan een webshop gekoppeld: default-user → 90, Bol_1088 → 91). Leeg = default.
     Volgorde: expliciete opt (test/override) > instelling > env. */
  const bolWeborderUser = clean(weborderUser || settings?.srsWeborderUser || process.env.SRS_BOL_API_USER || '');
  ...
  if (!dryRun) {
    try {
      sessionId = await getCachedWeborderSessionForUser(bolWeborderUser);
    } catch (e) {
      return { success: false, message: `SRS-login mislukt${bolWeborderUser ? ` (user ${bolWeborderUser})` : ''}: ${e.message || e}` };
    }
  }
  ...
  for (const order of orders) {
    ...
    try {
      ...
      if (/session/i.test(e.message || '')) {
        invalidateWeborderSessionForUser(bolWeborderUser);
        try {
          const fresh = await getCachedWeborderSessionForUser(bolWeborderUser);
  ...
  return {
    ...
    summary: {
      ...
      /* shopId is legacy (genegeerd door SRS); filialen gaan via extended_attributes. */
      srsShopIdLegacy: clean(process.env.SRS_BOL_SHOP_ID || '10'),
      srsSellingBranchId: clean(routing.sellingBranchId || process.env.SRS_BOL_SELLING_BRANCH_ID || '91'),
      srsFulfilmentBranchId: clean(routing.fulfilmentBranchId || process.env.SRS_BOL_FULFILMENT_BRANCH_ID || '99'),
      /* DE ECHTE webshop-routing ("Geplaatst in", 90 vs 91): met welke API-user geplaatst is. */
      srsWeborderUser: bolWeborderUser || '(default-user)'
    },
    results
  };
}
