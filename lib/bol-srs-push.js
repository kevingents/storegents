...
function buildBolWeborderXml({ orderId, bolOrderId, detail, productsXmlStr, total, placedAt }) {
  ...
  /* SRS accepteert maar een STRIKTE set extended_attribute-namen. Onbekende
     namen of waarden geven fout 140 'Problem adding order extended attribute'.
     Bewezen veilig (uit srs-weborder-client.js interstore-pad):
       verkoop_filiaal, afhaal_filiaal, verzend_filiaal, aangemaakt_in_filiaal
     Bol-marketplace info gaat in <opmerking> (vrije tekst), niet in attrs. */
  const extendedAttrs = [
    ...
    extendedAttribute('opmerking', `BOL marketplace · bol_orderId: ${bolOrderId} · marketplace_orderid: ${orderId}`)
  ].join('');
