...
export async function upsertUserPermissions(personnelId, patch = {}, updatedBy = 'admin') {
  ...
  const updated = {
    ...
    /* afdeling: expliciete koppeling aan virtuele winkel (Supplychain, Finance,
       Marketing, IT, HR, etc.). Default-store bij login wordt hier op gebaseerd.
       Onafhankelijk van 'department' (= display-label) zodat admin precies kan
       sturen waar iemand op landt. Lege string = geen afdeling. */
    afdeling: patch.afdeling ?? existing.afdeling ?? '',
...
export async function bulkUpsert(items = [], updatedBy = 'admin') {
  ...
  for (const item of items) {
    ...
    all[id] = {
      ...
      afdeling: item.afdeling ?? existing.afdeling ?? '',
