...
async function readBlobText(url) {
  /* Cache-buster + no-store: de blob-URL is CDN-cachebaar, dus zonder dit zou
     een lees vlak na een schrijf (mark-read) de oude versie teruggeven en de
     melding "terugkomen". */
...
async function writeAll(items) {
  await put(STORE_PATH, JSON.stringify(items, null, 2), {
    ...
    cacheControlMaxAge: 0 /* veranderlijke data — geen CDN-cache, anders "gelezen" niet meteen zichtbaar */
  });
...
export async function listForStore(store, { limit = 50, includeRead = false, includeArchived = false, refresh = false } = {}) {
  const all = await getAllNotifications({ refresh });
...
/**
 * Archiveer notificaties voor een specifieke store.
 * Verbergt voor die store maar laat de notificatie zichtbaar voor andere stores.
 */
export async function archiveForStore(store, ids = []) {
  ...
  const next = all.map((n) => {
    ...
    if (!n.archivedBy[store]) { n.archivedBy[store] = now; count++; }
    /* Mark read implicit bij archive */
    if (!n.readBy) n.readBy = {};
    if (!n.readBy[store]) n.readBy[store] = now;
    return n;
  });
  if (count > 0) await writeAll(next);
  return count;
...
/**
 * Archiveer alle notificaties die voor de store relevant zijn.
 */
export async function archiveAllForStore(store) {
  if (!store) return 0;
  const all = await getAllNotifications({ refresh: true });
  const now = new Date().toISOString();
  let count = 0;
