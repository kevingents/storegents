...
const DEFAULT_SRS_MESSAGE_BASE_URL = 'https://production.storeinfo.nl';
const VOUCHERS_MESSAGE_PATH = '/messages/v1/soap/Vouchers.php';
const VOUCHERS_SOAP_ACTION_BASE = 'https://messages.storeinfo.nl/v1/soap/Vouchers';
const GENTS_LOYALTY_VOUCHER_AMOUNT = '25.00';
...
function soapActionFor(action) {
  return String(action || '').startsWith('http')
    ? String(action)
    : `${VOUCHERS_SOAP_ACTION_BASE}/${action}`;
}

async function postSoap(action, xml) {
  const { endpoint } = getConfig();
  const soapAction = soapActionFor(action);
...
function commonGetStatusXml(transactionId) {
  const { id, password } = getConfig();
...
export function getLoyaltyVoucherRules() {
  ...
  // GENTS loyalty rule: 500 SRS punten = EUR 25 voucher.
  // Do not use env/body overrides here: a wrong decimal override already caused
  // EUR 0.25 / EUR 2.50 test vouchers. SRS CreateFromLoyaltyPoints must always
  // receive the euro voucher value fields below for the automatic loyalty run.
  return {
    stepsOf: GENTS_LOYALTY_VOUCHER_AMOUNT,
    minimum: GENTS_LOYALTY_VOUCHER_AMOUNT,
    maximum: GENTS_LOYALTY_VOUCHER_AMOUNT,
    validityMonths
  };
}
  ...
  <soapenv:Body>
    <tran:CreateFromLoyaltyPoints>
      ...
      <tran:Body>
        ...
        <tran:Value>
          <tran:StepsOf>${xmlEscape(rules.stepsOf)}</tran:StepsOf>
          <tran:Minimum>${xmlEscape(rules.minimum)}</tran:Minimum>
          <tran:Maximum>${xmlEscape(rules.maximum)}</tran:Maximum>
        </tran:Value>
...
export async function getVouchersTransactionStatus(transactionId) {
  ...
  const xml = `<?xml version="1.0" encoding="UTF-8"?>
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:com="https://messages.storeinfo.nl/v1/Common">
  <soapenv:Header/>
  <soapenv:Body>
    <com:GetStatus>
      ${commonGetStatusXml(transactionId)}
    </com:GetStatus>
  </soapenv:Body>
</soapenv:Envelope>`;
