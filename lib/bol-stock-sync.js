...
import { bolPost, bolGetCsv, bolWaitForProcess, isBolConfigured, bolOffersVersion } from './bol-client.js';
...
import { getStockMapForLocations, resolveLocationIdForBranch } from './shopify-stock-source.js';
...
/* ── Magazijnvoorraad per SKU — LEGACY pad (SRS snapshot). ───────────────
   Alleen actief wanneer BOL_STOCK_SOURCE=srs expliciet gezet wordt. Default
   is nu Shopify (zie warehouseStockFromShopify). */
...
/* ── Magazijnvoorraad per EAN — NIEUW pad (Shopify als single source). ─── */
async function warehouseStockFromShopify(allBarcodes) {
  const branches = listBranchesFromConfig({ includeInternal: true });
  const warehouseBranchIds = branches.filter((b) => b.kind === 'warehouse').map((b) => clean(b.branchId)).filter(Boolean);
  /* Vertaal warehouse-branches naar Shopify locationIds (parallel). */
  const locIds = (await Promise.all(warehouseBranchIds.map((bid) => resolveLocationIdForBranch(bid)))).filter(Boolean);
  const warehouseConfigured = locIds.length > 0;
  if (!warehouseConfigured) {
    return { byBarcode: new Map(), warehouseConfigured: false, reason: 'Geen Shopify-locations gevonden voor warehouse-branches' };
  }
  const map = await getStockMapForLocations(allBarcodes, locIds);
  return { byBarcode: map, warehouseConfigured: true, locIds, warehouseBranchIds };
}

/** Bouw per EAN de gewenste bol-voorraad (= magazijnvoorraad).
 *  Voorraad-bron: Shopify (default) of SRS (BOL_STOCK_SOURCE=srs). */
export async function buildBolStockPlan() {
  const useShopify = clean(process.env.BOL_STOCK_SOURCE).toLowerCase() !== 'srs';
  const buffer = Math.max(0, Math.round((await getBolSettings()).stockBuffer));
  const cache = await readProductsCache();
  const byBarcode = (cache && cache.byBarcode) || {};

  /* Verzamel alle unieke EANs uit de cache. */
  const allBarcodes = [];
  const seen = new Set();
  ...
  for (const v of Object.values(byBarcode)) {
    ...
    seen.add(ean);
    allBarcodes.push(ean);
    variantByEan.set(ean, v);
  }

  /* Voorraad-lookup per gekozen bron. */
  let warehouseConfigured = false;
  let stockByEan = new Map();
  let bron = 'shopify';
  if (useShopify) {
    const wh = await warehouseStockFromShopify(allBarcodes);
    warehouseConfigured = wh.warehouseConfigured;
    stockByEan = wh.byBarcode;
  } else {
    bron = 'srs';
    const rows = await readVoorraadRows();
    const { bySku, warehouseConfigured: srsWh } = warehouseStockBySku(rows);
    warehouseConfigured = srsWh;
    /* SRS heeft SKU-keys — vertaal naar EAN-keys via cache. */
    for (const ean of allBarcodes) {
      const v = variantByEan.get(ean);
      stockByEan.set(ean, toStock(bySku.get(skuKey(v?.sku))));
    }
  }
  ...
  for (const ean of allBarcodes) {
    const v = variantByEan.get(ean);
    const magazijn = toStock(stockByEan.get(ean));
  ...
  const result = {
    ...
    bron,
    principe: `bol-voorraad = magazijn (${bron}) − ${buffer} veiligheidsmarge (min. 0)`,
...
async function attemptOfferExport(maxWaitMs) {
  const start = await bolPost('/offers/export', { format: 'CSV' }, { version: bolOffersVersion() });
...
export async function refreshBolOfferMap({ maxWaitMs = 50000 } = {}) {
  ...
  const csv = await bolGetCsv(`/offers/export/${reportId}`, { version: bolOffersVersion() });
...
export async function runBolStockSync({ dryRun = true, onlyChanged = true, refreshMap = false, force = false } = {}) {
  ...
  /* Offer-map up-to-date houden. Bij refresh-fail:
       - geen cached map → afbreken met diagnose
       - cached map < 7 dagen oud → ga door met cache + waarschuwing
       - cached map > 7 dagen → afbreken (risico op schrijven naar verwijderde offers) */
  ...
  if (refreshMap || !mapBlob || !isOfferMapFresh(mapBlob)) {
    ...
    if (rm.ok) {
      mapBlob = await readJsonBlob(OFFER_MAP_PATH, null);
    } else if (!mapBlob) {
      return {
        dryRun: false, ok: false,
        message: rm.message || `offer-map niet beschikbaar (${rm.reason || 'onbekend'})`,
        diagnose: rm.diagnose || null,
        stale: !!rm.stale
      };
    } else {
      /* Fallback: cached map gebruiken als < 7 dagen oud. Voorkomt dat één
         scope-issue bij Bol de hele voorraad-sync stilzet. */
      const mapAgeMs = Date.now() - new Date(mapBlob.updatedAt || mapBlob.refreshedAt || 0).getTime();
      const sevenDaysMs = 7 * 24 * 3600 * 1000;
      if (Number.isFinite(mapAgeMs) && mapAgeMs > 0 && mapAgeMs < sevenDaysMs) {
        const ageDays = Math.round(mapAgeMs / (24 * 3600 * 1000) * 10) / 10;
        staleMapWarning = `offer-map ${ageDays} dagen oud (refresh faalde: ${rm.reason || 'onbekend'}) — sync loopt verder met cache, nieuwe offers worden gemist`;
        console.warn(`[bol-stock] ${staleMapWarning}`);
      } else {
        return {
          dryRun: false, ok: false,
          message: `Offer-map cache te oud (${rm.reason || 'refresh faalde'}) — sync afgebroken om niet naar verwijderde offers te schrijven.`,
          diagnose: rm.diagnose || null,
          stale: !!rm.stale
        };
      }
  ...
  for (const it of plan.items) {
    ...
    try {
      await bolPost(`/offers/${o.offerId}/stock`, { amount: it.bolDoel, managedByRetailer: true }, { method: 'PUT', version: bolOffersVersion() });
      gepusht += 1;
  ...
  return {
    ...
    principe: plan.principe, totaal: plan.totaal,
    gepusht, overgeslagen, fouten, resultaten,
    ...(staleMapWarning ? { mapWarning: staleMapWarning } : {})
  };
}
