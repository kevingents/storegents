...
export async function runBolStockSync({ dryRun = true, onlyChanged = true, refreshMap = false, force = false } = {}) {
  ...
  if (!force) {
    if (!plan.warehouseConfigured) {
      return { dryRun: false, ok: false, aborted: true, reason: 'Geen magazijn-filiaal geconfigureerd — sync afgebroken (force om te forceren).' };
    }
    if (plan.totaal > 0 && plan.metVoorraad === 0) {
      return { dryRun: false, ok: false, aborted: true, reason: 'Magazijnvoorraad is overal 0 — waarschijnlijk een mislukte/lege voorraadimport. Sync afgebroken om bol niet leeg te zetten (force om te forceren).' };
    }
  }
  ...
  if (!force) {
    ...
    if (nuVoorraad >= 20 && naarNul / nuVoorraad > 0.5) {
      return { dryRun: false, ok: false, aborted: true, reason: `Sanity-check: ${naarNul} van ${nuVoorraad} bol-offers mét voorraad zouden naar 0 gaan (>50%). Sync afgebroken (force om te forceren).` };
    }
  }
