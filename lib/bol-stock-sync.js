...
/* Veiligheidsmarge: houd dit aantal stuks achter, zodat een winkelverkoop niet
   tot een bol-annulering leidt. bol-voorraad = max(0, magazijn − marge). */
const SAFETY_BUFFER = Math.max(0, Math.round(Number(process.env.BOL_STOCK_BUFFER ?? 3)));

const clean = (v) => String(v == null ? '' : v).trim();
const skuKey = (v) => clean(v).toLowerCase();
const toStock = (n) => Math.max(0, Math.round(Number(n) || 0));
const bolTarget = (magazijn) => Math.max(0, magazijn - SAFETY_BUFFER);
...
export async function buildBolStockPlan() {
  ...
  for (const v of Object.values(byBarcode)) {
    ...
    const bolDoel = bolTarget(magazijn);
  ...
  const result = {
    ...
    principe: `bol-voorraad = magazijn − ${SAFETY_BUFFER} veiligheidsmarge (min. 0)`,
    veiligheidsmarge: SAFETY_BUFFER,
    warehouseConfigured,
    totaal: items.length,
    metVoorraad,
