...
export async function refreshBolOfferMap({ maxWaitMs = 50000 } = {}) {
  ...
  const status = clean(done?.status).toUpperCase();
  if (status === 'STALE') {
    /* processStatusId hoort bij een andere/oude account. Geen retry — caller
       moet een verse export starten in de volgende run. */
    return { ok: false, reason: 'stale processStatusId — token-rotation of demo↔prod switch?', stale: true };
  }
  if (status !== 'SUCCESS') {
