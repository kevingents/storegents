...
/**
 * Leaf-variant: pak de TEKST-waarde van een node zónder kind-tags ([^<>]).
 * Nodig omdat de SRS Return-response een BUITENSTE <…:Status> om de Header heeft
 * met dáárin de echte <com:Status>completed</com:Status>. De gewone getNodeText
 * (non-greedy) pakte dan vanaf de buitenste tag tot de binnenste sluit-tag → een
 * hele XML-blob i.p.v. "completed", waardoor élke retour onterecht als mislukt
 * werd gelogd. Deze variant matcht alleen een echte leaf en geeft "completed".
 */
...
export async function createSrsReturn({
  ...
  if (unresolvedItems.length) {
    const missingSkus = unresolvedItems
      .map((item) => item.fulfillmentId || item.orderLineNr || item.sku || `item#${item.index + 1}`)
      .join(', ');
    throw new Error(`Retourregels konden niet aan fulfillment worden gekoppeld (${missingSkus || 'onbekend item'}).`);
  }
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:tran="https://messages.storeinfo.nl/v1/Weborders/Transactions" xmlns:com="https://messages.storeinfo.nl/v1/Common" xmlns:web="https://messages.storeinfo.nl/v1/Weborders">
  ...
  /* Leaf-variant eerst (de echte <com:Status>completed</com:Status>); val terug
     op de gewone extractie als de structuur ooit anders is. */
  const status = getLeafNodeText(responseText, 'Status') || getNodeText(responseText, 'Status') || 'unknown';
  const responseTransactionId = getLeafNodeText(responseText, 'TransactionId') || getNodeText(responseText, 'TransactionId') || transactionId;
