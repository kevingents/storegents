/**
 * DHL hub mapping per GENTS winkel — DEFAULTS.
 *
 * Bron: Excel "Gents B.V. - Contactgegevens per winkel" (DHL pickups).
 *
 * Admin kan per winkel afwijkende waarden instellen in de admin-UI
 * → die overrides staan in de Blob `config/dhl-hubs.json` en winnen van
 * deze defaults. Zie `lib/dhl-hubs-store.js` voor de overrides.
 *
 * Gebruik:
 ...
 *
 * LET OP: voor nieuwe winkels of gewijzigde hubs gebruik je de admin-UI
 *   (Instellingen → DHL hubs) ipv deze file direct te bewerken.
 */
...
/**
 * Geeft DHL hub-info terug voor een specifieke winkel (ALLEEN defaults).
 * Voor de echte data inclusief admin-overrides: gebruik getDhlHubForStoreAsync.
 */
export function getDhlHubForStore(store) {
  if (!store) return null;
  const key = String(store).trim();
  if (DHL_HUBS[key]) return { store: key, ...DHL_HUBS[key], _source: 'default' };
  /* Case-insensitive fallback */
  const lc = key.toLowerCase();
  const matchKey = Object.keys(DHL_HUBS).find((k) => k.toLowerCase() === lc);
  if (matchKey) return { store: matchKey, ...DHL_HUBS[matchKey], _source: 'default' };
  return null;
}

/**
 * Async variant: leest Blob-overrides + merget met defaults.
 * Voor admin-UI + no-show endpoint zodat handmatig ingestelde hubs winnen.
 */
export async function getDhlHubForStoreAsync(store) {
  const fallback = getDhlHubForStore(store);
  try {
    const mod = await import('./dhl-hubs-store.js');
    if (typeof mod.getDhlHubOverride === 'function') {
      const override = await mod.getDhlHubOverride(store);
      if (override && Object.keys(override).length) {
        const base = fallback || { store: String(store).trim() };
        return { ...base, ...override, store: base.store, _source: 'override' };
      }
    }
  } catch (error) {
    console.warn('[getDhlHubForStoreAsync] Blob-override faalde, gebruik default:', error.message);
  }
  return fallback;
}
 ...
 * Sync versie — gebruikt alleen defaults.
 */
...
/**
 * Async variant: bouwt grouping op basis van defaults + admin-overrides.
 */
export async function getDhlHubsGroupedAsync() {
  /* Eerst defaults laden, dan overrides erover */
  const merged = {};
  for (const [store, info] of Object.entries(DHL_HUBS)) {
    merged[store] = { ...info };
  }
  try {
    const mod = await import('./dhl-hubs-store.js');
    if (typeof mod.getAllDhlHubOverrides === 'function') {
      const overrides = await mod.getAllDhlHubOverrides();
      for (const [store, override] of Object.entries(overrides || {})) {
        merged[store] = { ...(merged[store] || {}), ...override };
      }
    }
  } catch (error) {
    console.warn('[getDhlHubsGroupedAsync] Blob faalde, gebruik defaults:', error.message);
  }

  const groups = new Map();
  for (const [store, info] of Object.entries(merged)) {
    const hubName = info.hub || '(onbekend)';
    if (!groups.has(hubName)) {
      groups.set(hubName, {
        hub: hubName,
        email: info.email || '',
        phone: info.phone || '',
    ...
    groups.get(hubName).stores.push({
      store,
      pickupWindow: info.pickupWindow || '',
      pickupAddress: info.pickupAddress || ''
    });
...
/**
 * Volledige merged map van álle winkels (defaults + overrides).
 * Voor admin-UI die editable lijst nodig heeft.
 */
export async function getAllDhlHubsMergedAsync() {
  const merged = {};
  for (const [store, info] of Object.entries(DHL_HUBS)) {
    merged[store] = { ...info, _source: 'default' };
  }
  try {
    const mod = await import('./dhl-hubs-store.js');
    if (typeof mod.getAllDhlHubOverrides === 'function') {
    ...
    console.warn('[getAllDhlHubsMergedAsync] Blob faalde:', error.message);
  }
  return merged;
}

/**
 * Bepaalt het juiste depot-emailadres voor een no-show melding (sync, defaults only).
 */
...
/**
 * Async variant — checkt eerst de admin-overrides, dan defaults, dan env-vars.
 */
export async function getDepotEmailForStoreAsync(store) {
