...
function makeRecord({ fulfillment, detail, line, status, orderFulfillments, shopifyContext }) {
  const orderNr = cleanOrderNr(fulfillment.orderNr);
...
export async function syncGlobalUnavailableOrderLines({ orderNr = '', statuses = '', dateFrom = '', dateTo = '', month = '', dryRun = false, maxRuntimeMs = DEFAULT_MAX_RUNTIME_MS, maxRecords = DEFAULT_MAX_RECORDS, includeResolved = false } = {}) {
  ...
  for (const fulfillment of result.fulfillments || []) {
    ...
    const record = makeRecord({ fulfillment, detail, line, status: fulfillment.status || fulfillment.requestedStatus || 'unavailable', orderFulfillments, shopifyContext });
    records.push(record);
