...
function makeRecord({ fulfillment, detail, line, status, orderFulfillments }) {
  ...
  return {
    ...
    store,
    employeeName: 'SRS niet-leverbaar synchronisatie',
    orderNr,
    type: 'partial',
    reason: 'Niet leverbaar volgens SRS',
    ...
    srsStatus: 'unavailable_in_srs',
    srsCancelStatus: 'pending',
    refundStatus: 'pending',
    mailStatus: 'shopify_refund_mail',
    stockReturnStatus: 'skipped_no_stock_return',
    source: 'srs_global_fulfillments_unavailable_lines',
...
async function getUnavailableFulfillments({ orderNr = '', statuses, startedAt, maxRuntimeMs }) {
  if (orderNr) {
    const rows = await getFulfillmentsForOrder(orderNr);
    return {
      fulfillments: rows.filter((row) => isUnavailableStatus(row.status)),
