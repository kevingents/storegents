...
function makeRecord({ fulfillment, detail, line, status, orderFulfillments, shopifyContext }) {
  ...
  const source = resolvedInSrs
    ? 'srs_global_fulfillments_resolved_unavailable_lines'
    : 'srs_global_fulfillments_unavailable_lines';
  ...
  return {
    idempotencyKey: [
      'srs-global-unavailable-order-line',
      orderNr,
      item.fulfillmentId,
      item.orderLineNr,
      item.articleNumber,
...
async function getUnavailableFulfillments({ orderNr = '', statuses, startedAt, maxRuntimeMs, includeResolved = false }) {
  if (orderNr) {
    ...
    return {
      fulfillments: rows.filter((row) => isUnavailableStatus(row.status) || (includeResolved && isResolvedUnavailableStatus(row.status))),
...
export async function syncGlobalUnavailableOrderLines({
  ...
  const result = await getUnavailableFulfillments({
    orderNr,
  ...
  for (const fulfillment of result.fulfillments || []) {
    ...
    const storedRecord = addResult.cancellation || record;
    records.push(storedRecord);

    if (addResult.duplicate) {
      duplicates += 1;
      duplicateRecords.push(storedRecord);
    } else {
      created += 1;
  ...
  return {
    ...
    message: dryRun
      ...
      : `SRS niet-leverbaar sync klaar. ${created} nieuw, ${duplicates} al bekend, ${skippedByDate} buiten datumfilter overgeslagen.`
  };
}
