...
const DEFAULT_GLOBAL_STATUSES = 'unavailable';
const DEFAULT_MAX_RUNTIME_MS = 55000;
...
function makeRecord({ fulfillment, detail, line, status, orderFulfillments }) {
  ...
  return {
    ...
    store,
    employeeName: 'SRS globale niet-leverbaar synchronisatie',
    orderNr,
    type: 'partial',
    ...
    mailStatus: 'pending',
    source: 'srs_global_get_fulfillments_unavailable',
...
export async function syncGlobalUnavailableOrderLines({
  ...
  return {
    ...
    source: orderNr ? 'srs_order_fulfillments_unavailable' : 'srs_global_fulfillments_unavailable',
    ...
    message: dryRun
      ...
      : `Niet-leverbaar sync klaar. ${created} nieuw, ${duplicates} al bekend, ${skippedByDate} buiten datumfilter overgeslagen.`
  };
}
