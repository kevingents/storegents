...
import { writeVoorraadSnapshot, readVoorraadRows, readSrsVoorraadRows } from './srs-voorraad-store.js';
...
export async function buildShopifyStockSnapshots() {
  ...
  /* SRS-voorraad als bron voor (a) de ideaal-merge en (b) het BEHOUD van interne
     branches zonder Shopify-locatie. Lees uit het rauwe SRS-blob (dat deze
     snapshot NIET overschrijft) zodat de SRS-data ook na de 1e Shopify-run
     beschikbaar blijft; val terug op rows-latest.json. */
  let srsRows = [];
  try {
    srsRows = await readSrsVoorraadRows();
    if (!srsRows.length) srsRows = await readVoorraadRows();
  } catch (e) {
    console.warn('[shopify-stock-snapshot] SRS-rows lezen faalde:', e.message);
  }
  ...
  const matchedBranchIds = new Set(matched.map((m) => String(m.branchId)));
  const voorraadRows = [];
  ...
  /* Behoud interne branches ZONDER Shopify-locatie (bv. 701 fotoshoot, 702
     suitconcern, 97 uitlevertafel): neem hun SRS-voorraad mee, zodat de
     voorraad-tools voor die filialen blijven werken na de Shopify-overstap. */
  let srsCarried = 0;
  for (const r of (srsRows || [])) {
    if (matchedBranchIds.has(String(r.filiaalNummer))) continue;
    const voorraad = Number(r.voorraad) || 0;
    const ideaal = Number(r.ideaal) || 0;
    voorraadRows.push({
      filiaalNummer: String(r.filiaalNummer),
  ...
  await writeVoorraadSnapshot(voorraadRows, { sourceFile: 'shopify-live+srs-intern', bron: 'shopify+srs' });
