...
import { readVoorraadRows, readSrsVoorraadRows, writeShopifyVoorraadRows } from './srs-voorraad-store.js';
...
export async function buildShopifyStockSnapshots() {
  ...
  /* Schrijf de live Shopify-voorraad naar een EIGEN blob — NIET rows-latest.json.
     rows-latest.json is weer eigendom van de SRS-import (3x/dag): alle
     dashboards/analyse draaien daarmee op SRS (ERP-truth + ideaal-targets).
     Deze Shopify-blob voedt alleen de operationele live-stock-consumers
     (bol-voorraad-sync). De winkel-lookup leest de branch-snapshots hierboven. */
  await writeShopifyVoorraadRows(voorraadRows, { sourceFile: 'shopify-live', bron: 'shopify' });
