...
/**
 * Zet een bestaande Meta-campagne op ACTIVE of PAUSED. Pauzeren stopt de
 * uitlevering direct; activeren hervat. Reversibel en veilig (geen geld zonder
 * actief budget). Vereist op het token de scope `ads_management`.
 *
 * @param {string} campaignId  campagne-id (zonder act_-prefix).
 * @param {'ACTIVE'|'PAUSED'} status
 * @returns {Promise<{ok:boolean, id?:string, status?:string, error?:string, hint?:string}>}
 */
export async function setMetaCampaignStatus(campaignId, status) {
  const c = cfg();
  if (!c.token) return { ok: false, error: 'Geen Meta-token (META_ADS_ACCESS_TOKEN).' };
  const id = clean(campaignId);
  if (!id) return { ok: false, error: 'Geen campagne-id.' };
  const st = clean(status).toUpperCase();
  if (st !== 'ACTIVE' && st !== 'PAUSED') return { ok: false, error: 'Status moet ACTIVE of PAUSED zijn.' };
  try {
    /* Marketing API: POST /{campaign_id} met status=ACTIVE|PAUSED. */
    await metaPost(c, id, { status: st });
    return { ok: true, id, status: st };
  } catch (e) {
    const msg = e.message || 'Status wijzigen mislukte.';
    let hint = null;
    if (/permission|ads_management|(#200)|(#10)\b/i.test(msg)) {
      hint = 'Het token mist waarschijnlijk de scope ads_management — voeg die toe aan de System User en genereer het token opnieuw.';
    }
    return { ok: false, error: msg, hint, metaError: e.meta || null, subcode: e.subcode || null, fbtrace: e.fbtrace || null };
  }
}
