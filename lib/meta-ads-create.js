...
export async function createPausedBoost(o = {}) {
  ...
  try {
    ...
    /* 2. Campagne (PAUSED). */
    const camp = await metaPost(c, `${c.account}/campaigns`, {
    ...
    const adset = await metaPost(c, `${c.account}/adsets`, adsetBody);
    created.adsetId = adset.id;

    /* 4. Advertentiemateriaal van de bestaande IG-post. */
    const creativeBody = {
      name: `Boost-creative · ${tag}`,
      object_story_spec: { page_id: pageId, instagram_user_id: c.igId },
      source_instagram_media_id: mediaId
    };
    if (goalKey === 'verkeer' && clean(o.linkUrl)) {
      creativeBody.object_story_spec.link_data = { link: clean(o.linkUrl) };
    }
    const creative = await metaPost(c, `${c.account}/adcreatives`, creativeBody);
    created.creativeId = creative.id;

    /* 5. Advertentie (PAUSED). */
    const ad = await metaPost(c, `${c.account}/ads`, {
      name: `Boost-ad · ${tag}`,
      adset_id: adset.id,
    ...
    return {
      ok: false,
      error: e.message || 'Boost aanmaken mislukte.',
      metaError: e.meta || null,
      created,
      adsManagerUrl: created.campaignId ? adsManagerUrl(c, created.campaignId) : null,
      hint: /permission|ads_management|(#200)|(#10)/i.test(e.message || '')
        ? 'Het token mist waarschijnlijk de scope ads_management — voeg die toe aan de System User en genereer het token opnieuw.'
        : null
    };
  }
}
