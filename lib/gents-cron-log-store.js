...
import { mutateJsonBlob } from './json-blob-store.js';
...
export async function appendCronLog(entry = {}) {
  ...
  /* Veel crons schrijven tegelijk om (bv. 05:00). Optimistische RMW met
     conflict-detectie + retry voorkomt dat gelijktijdige appends elkaars
     log-regels overschrijven. */
  await mutateJsonBlob(
    LOG_PATH,
    (rows0) => {
      const rows = Array.isArray(rows0) ? rows0 : [];
      rows.unshift(row);
      return rows.slice(0, MAX_LOG_ROWS);
    },
    { fallback: [], cacheMaxAge: 60 }
  );
  return row;
}
