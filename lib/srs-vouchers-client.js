...
const SRS_SESSION_KEY = 'si_voucherservice';
...
async function postSoap(action, xml) {
  ...
  try {
    ...
    if (error?.name === 'AbortError') {
      const timeoutError = new Error(`SRS voucher timeout na ${timeout}ms (${action}).`);
...
async function getSession(sessionId) {
  ...
  return getCachedSession(SRS_SESSION_KEY, loginSrsVoucherService);
}

export function invalidateSrsVoucherSession() {
  invalidateSession(SRS_SESSION_KEY);
}

/**
 * Lichte session-fout helper. Roep in catch-block aan om de gecachede sessie
 * leeg te gooien wanneer SRS aangeeft dat de sessie verlopen is.
 */
export function isSrsVoucherSessionError(error) {
