...
import { readVoorraadRows, readLocatiesRows, computeMagazijnNietWinkel, readMagazijnNietWinkelWatermark } from './srs-voorraad-store.js';
...
export const REPORT_BUILDER_SOURCES = {
  ...
  /* ═════════════════════ MAGAZIJN NIET IN WINKEL ═════════════════════ */
  'magazijn-niet-winkel': {
    key: 'magazijn-niet-winkel',
    label: 'Magazijn ≠ winkel',
    description: 'Artikelen wél op voorraad in het magazijn maar bij geen enkele winkel. Toont of uitlevering/herbevoorrading hapert.',
    icon: 'package',
    availableColumns: [
      { key: 'sku',                label: 'SKU',                  type: 'string' },
      { key: 'magazijn',           label: 'Magazijn',             type: 'string' },
      { key: 'magazijnVoorraad',   label: 'Magazijn-voorraad',    type: 'number' },
      { key: 'winkelsMetTarget',   label: 'Winkels willen het',   type: 'number' },
      { key: 'winkelTargetTotaal', label: 'Winkel-target totaal', type: 'number' },
      { key: 'dagen',              label: 'Dagen onuitgeleverd',  type: 'number' },
      { key: 'sinds',              label: 'Onuitgeleverd sinds',  type: 'date' }
    ],
    defaultColumns: ['sku', 'magazijn', 'magazijnVoorraad', 'winkelsMetTarget', 'dagen'],
    availableFilters: [
      { key: 'sku',           label: 'SKU bevat',        type: 'text',         field: 'sku' },
      { key: 'magazijn',      label: 'Magazijn bevat',   type: 'text',         field: 'magazijn' },
      { key: 'voorraadMin',   label: 'Magazijn-voorraad', type: 'number-range', field: 'magazijnVoorraad' },
      { key: 'winkelsMin',    label: 'Min. winkels-met-target', type: 'number-range', field: 'winkelsMetTarget' },
      { key: 'dagenMin',      label: 'Min. dagen onuitgeleverd', type: 'number-range', field: 'dagen' }
    ],
    fetcher: async () => {
      const [rows, wm] = await Promise.all([readVoorraadRows(), readMagazijnNietWinkelWatermark()]);
      return computeMagazijnNietWinkel(rows, { firstSeen: wm.firstSeen || {} });
    }
  },
