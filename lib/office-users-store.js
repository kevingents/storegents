...
let __CACHE__ = null;
let __CACHE_AT__ = 0;
/* Kort (was 30s): deze blob is login-kritisch. Een net-aangemaakte/uitgenodigde
   kantoor-gebruiker moet vrijwel direct zichtbaar + inlogbaar zijn, ook op een
   andere serverless-instance (de in-memory cache leeft per instance). */
const CACHE_TTL_MS = 8 * 1000;

const scryptAsync = promisify(scrypt);
/* Invite-token leeftijd: 2 dagen. Korter = veiliger. Admin kan altijd
   handmatig opnieuw versturen via Gebruikers → resend-invite knop. */
const INVITE_TTL_MS = Number(process.env.INVITE_TTL_MS || 2 * 24 * 60 * 60 * 1000);

async function readBlobText(url) {
  /* Cache-buster + no-store: anders serveert de CDN tot cacheControlMaxAge de
     oude versie ná een write → een net aangemaakte gebruiker zou niet gevonden
     worden bij login. */
...
export async function getAllOfficeUsers({ refresh = false } = {}) {
  ...
  try {
    ...
    /* KRITIEK: bij refresh:true mag de aanroeper GEEN lege fallback krijgen.
       Alle write-functies (upsert, delete, setInvite, setPassword, 2FA,
       bounce) draaien op refresh:true en bouwen daarop een writeAll(all)
       — als we hier een lege {} terugzouden geven zou één transient
       blob-leesfout + 1 gelijktijdige write de COMPLETE office-users blob
       wipen (passwords, 2FA-state, bounces). Dus throw i.p.v. silent
       data-loss. Read-paden (login, lookup) gebruiken refresh:false en
       blijven fail-soft via de cache. */
...
async function writeAll(data) {
  await put(STORE_PATH, JSON.stringify(data, null, 2), {
    ...
    /* cacheControlMaxAge 0 (was 60): deze blob is WRITE-HEAVY (create/delete/
       password/2FA/role). Met 60s CDN-cache kan een delete/edit vlak na een
       andere write een STALE snapshot lezen → 'removed:false' of erger:
       een stale snapshot terugschrijven en recente wijzigingen wissen.
       0 = lees-na-schrijf-consistent (reads raken altijd de origin).
       De in-memory __CACHE__ (30s) vangt read-heavy login/lookup nog op. */
...
export async function deleteOfficeUser(userId) {
  ...
  let all = await getAllOfficeUsers({ refresh: true });
  if (!(userId in all)) {
    /* Net aangemaakt + nog niet zichtbaar (Blob-consistency)? Eén korte retry
       met een verse lees voordat we 'niet gevonden' concluderen. */
    await new Promise((resolve) => setTimeout(resolve, 400));
    all = await getAllOfficeUsers({ refresh: true });
    if (!(userId in all)) return false;
  }
  delete all[userId];
  await writeAll(all);
  /* writeAll slaagde → de verwijdering is persistent geschreven. We doen GEEN
     verse verify-lees: Vercel Blob is eventual consistent, dus een lees direct
     na de write kan nog de oude snapshot teruggeven (vals-negatief). De write
     is leidend. */
  return true;
}
 ...
 *
 *  Probeert eerst de in-memory cache (die direct na upsertOfficeUser ge-update is)
 *  voordat we naar Blob teruggrijpen. Vercel Blob heeft eventual consistency —
 *  direct na een put() kan list() soms nog de oude versie geven, waardoor een
 *  net-aangemaakte user "niet gevonden" lijkt. De cache-first lookup voorkomt
 *  deze race-condition tussen create-flow stap 1 (upsert) en stap 2 (token).
 *
 *  Het toegevoegde 2e refresh-retry geldt als veiligheidsnet voor legitieme
 *  lookup van een oudere user die nog niet in cache zit.
 */
export async function setInviteTokenForUser(userIdOrEmail) {
  const lookupKey = String(userIdOrEmail || '').trim();
  const findUser = (all) => {
    return all[lookupKey]
      || Object.values(all).find((u) => String(u.email || '').toLowerCase() === lookupKey.toLowerCase())
      || Object.values(all).find((u) => String(u.userId || '') === lookupKey);
  };

  /* 1e poging: cache (direct na upsert net ge-update) */
  let all = await getAllOfficeUsers();
  let user = findUser(all);

  /* 2e poging: refresh van Blob — als cache leeg/stale is */
  if (!user) {
    all = await getAllOfficeUsers({ refresh: true });
    user = findUser(all);
  }
...
/**
 * Markeer een e-mailadres als bounced. Wordt aangeroepen door de Resend webhook
 * bij `email.bounced` (type=hard). Mail-flows kunnen `user.bouncedAt` checken om
 * verdere mails te onderdrukken. De bounce wordt NIET automatisch verwijderd —
 * admin moet handmatig clearen via UI als gebruiker nieuw mailadres heeft.
 *
 * @param {string} email     ontvanger-adres uit bounce-event
 * @param {Object} meta      { reason, messageId, at }
 * @returns {Object|null}    de geüpdatete user, of null als geen office-user matched
 */
export async function markEmailBounced(email, meta = {}) {
  const cleanEmail = String(email || '').toLowerCase().trim();
  if (!cleanEmail) return null;
  const all = await getAllOfficeUsers({ refresh: true });
  const user = Object.values(all).find((u) => String(u.email || '').toLowerCase() === cleanEmail);
  if (!user) return null; /* niet een office-user (kan een klant zijn) */

  user.bouncedAt = meta.at || new Date().toISOString();
  user.bounceReason = String(meta.reason || '').slice(0, 500);
  user.bounceMessageId = String(meta.messageId || '');
  user.updatedAt = new Date().toISOString();
  all[user.userId] = user;
  await writeAll(all);
  return user;
...
/**
 * Verwijder de bounce-markering (admin handmatig "OK, ik heb gefixed").
 */
export async function clearEmailBounce(userIdOrEmail) {
  const key = String(userIdOrEmail || '').trim();
  if (!key) return false;
  const all = await getAllOfficeUsers({ refresh: true });
  const user = all[key] || Object.values(all).find((u) => String(u.email || '').toLowerCase() === key.toLowerCase());
  if (!user) return false;
  if (!user.bouncedAt) return true; /* niets te clearen */
  delete user.bouncedAt;
  delete user.bounceReason;
  delete user.bounceMessageId;
  user.updatedAt = new Date().toISOString();
  all[user.userId] = user;
  await writeAll(all);
  return true;
