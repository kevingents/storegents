...
/* ALLE opties (id + naam + waarden) van een product opzoeken. Shopify vereist
   bij productBundleCreate dat élke optie van een component gemapt wordt
   (Maat én Kleur etc.), anders: "Missing or invalid options found". */
async function getProductOptions(productId) {
  const d = await shopifyGraphql(
    `query($id: ID!){ product(id:$id){ id title options { id name optionValues { name } } } }`,
    { id: productId }
  );
  return (d?.product?.options || [])
    .map((o) => ({
      id: clean(o.id),
      name: clean(o.name),
      values: (o.optionValues || []).map((v) => clean(v.name)).filter(Boolean),
      isMaat: MAAT_RE.test(o.name)
    }))
    .filter((o) => o.id);
}
...
/* Maak Shopify-userErrors leesbaar. De ProductBundleOperation geeft soms een
   opaak `#<ActiveModel::Errors:...>` terug; vertaal dat naar een bruikbare hint. */
function fmtBundleErrors(errs) {
  return (errs || []).map((e) => {
    const msg = clean(e && e.message);
    const field = Array.isArray(e && e.field) ? e.field.join('.') : clean(e && e.field);
    if (/ActiveModel::Errors/i.test(msg)) {
      return `${field ? field + ': ' : ''}Shopify gaf een onduidelijke fout. Meestal: een onderdeel is niet gepubliceerd/actief, zit al in een andere bundle, of de onderdelen hebben afwijkende opties (bv. verschillende kleur).`;
    }
    return field ? `${field}: ${msg}` : (msg || 'Onbekende fout');
  }).join(', ');
}
...
async function pollOperation(opId, maxTries = 20) {
  for (let i = 0; i < maxTries; i++) {
    ...
    if (errs.length) throw new Error(fmtBundleErrors(errs));
...
async function createBundle({ title, comps }) {
  /* Per onderdeel ALLE opties ophalen + gemeenschappelijke maten bepalen. */
  const withOpts = [];
  for (const c of comps) {
    const opts = await getProductOptions(c.productId);
    if (!opts.length) throw new Error(`Geen opties gevonden voor ${c.role} (${c.productId}).`);
    if (!opts.some((o) => o.isMaat)) throw new Error(`Geen maat-optie gevonden voor ${c.role} (${c.productId}).`);
    withOpts.push({ ...c, options: opts });
  }
  const common = intersect(withOpts.map((c) => c.options.find((o) => o.isMaat).values)).filter(Boolean);
  if (!common.length) throw new Error('Geen gemeenschappelijke maten tussen de onderdelen.');

  /* Pre-check niet-maat-opties (bv. Kleur): Shopify maakt daar een bundel-optie
     van. Hebben de onderdelen verschillende waarden, dan kan het pak niet
     samengesteld worden (→ onduidelijke ActiveModel::Errors). Vang dat hier af. */
  const nonMaatNames = new Set();
  withOpts.forEach((c) => c.options.filter((o) => !o.isMaat).forEach((o) => nonMaatNames.add(o.name.toLowerCase())));
  for (const name of nonMaatNames) {
    const perComp = withOpts.map((c) => {
      const o = c.options.find((x) => x.name.toLowerCase() === name);
      return { role: c.role, values: o ? o.values.slice().sort() : null };
    });
    if (perComp.some((p) => p.values === null)) {
      throw new Error(`Niet alle onderdelen hebben de optie "${name}" — de onderdelen zijn niet gelijk, dus geen bundle mogelijk.`);
    }
    const ref = perComp[0].values.join('|').toLowerCase();
    if (perComp.some((p) => p.values.join('|').toLowerCase() !== ref)) {
      const detail = perComp.map((p) => `${p.role}: ${p.values.join('/') || '—'}`).join(' · ');
      throw new Error(`Onderdelen hebben verschillende "${name}"-waarden (${detail}). Maak ze gelijk in Shopify of haal de optie weg bij een onderdeel.`);
    }
  }

  /* Map ELKE optie van elk component: Maat → gedeelde bundel-optie (gemeenschap-
     pelijke maten), overige opties (bv. Kleur) → de eigen waarden van dat
     product. Zo voldoen we aan "map all of the options of the product". */
  const components = withOpts.map((c) => ({
    quantity: 1,
    productId: c.productId,
    optionSelections: c.options.map((o) => ({
      componentOptionId: o.id,
      name: o.isMaat ? SHARED_OPTION : o.name,
      values: o.isMaat ? common : o.values
    }))
  }));
