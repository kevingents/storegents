...
export function unavailableLineKey(input = {}) {
  return [
    clean(input.orderNr || input.orderName).replace(/^#/, ''),
    clean(input.fulfillmentId),
    clean(input.orderLineNr),
    clean(input.sku || input.barcode || input.articleNumber || input.articleId)
  ].join('::').toLowerCase();
}
