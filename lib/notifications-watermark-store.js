...
export async function getWatermarks({ refresh = false } = {}) {
  ...
  try {
    ...
    /* In refresh-pad (door markSeen) NIET de lege fallback teruggeven — caller
       zou anders een writeAll({}) doen → watermarks van andere keys (pickup,
       weborder, returns) zijn weg → push-storm bij volgende cron-run. */
    if (refresh) throw e;
    return __CACHE__ || {};
  }
}
