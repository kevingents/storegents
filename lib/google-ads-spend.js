...
export async function getGoogleAdsCampaigns({ from, to } = {}) {
  ...
  try {
    ...
    const campaigns = [...map.values()]
      .map((c) => ({ ...c, spend: r2(c.spend), conversions: r2(c.conversions), conversionValue: r2(c.conversionValue) }))
      .sort((a, b) => b.spend - a.spend);
    return { ok: true, platform: 'google', campaigns, spend: r2(campaigns.reduce((s, c) => s + c.spend, 0)), customerId: cfg.customerId };
  } catch (e) {
