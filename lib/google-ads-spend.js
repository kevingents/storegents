...
/* Winkel-campagnes zijn herkenbaar aan winkelnaam/stad — die halen we uit de
   centrale GENTS-config (één bron van waarheid), zodat je niks hoeft in te tikken. */
let _winkelStoreKw = null;
function winkelStoreKeywords() {
  if (_winkelStoreKw) return _winkelStoreKw;
  const set = new Set();
  try {
    for (const b of (listBranchesFromConfig({ includeInternal: false }) || [])) {
      const store = String(b.store || '').toLowerCase().trim();
      if (!store) continue;
      set.add(store);                                   /* bv. "gents almere" */
      const city = store.replace(/^gents\s+/, '').trim();
      if (city.length >= 4) set.add(city);              /* bv. "almere" */
    }
  } catch (_) { /* config niet beschikbaar → val terug op env + kanaal */ }
  _winkelStoreKw = [...set];
  return _winkelStoreKw;
}
...
export function classifyAdBucket(campaign) {
  ...
  /* 2. Winkelnaam/stad uit de GENTS-config → winkel-advertentie. */
  if (winkelStoreKeywords().some((k) => name.includes(k))) return 'winkel';
