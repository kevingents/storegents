...
import { readJsonBlob, mutateJsonBlob, writeJsonBlob } from './json-blob-store.js';

const PATH = 'marketing/gala-events.json';
const CRAWL_LOG_PATH = 'marketing/gala-crawl-log.json';

export async function readCrawlLog() {
  return readJsonBlob(CRAWL_LOG_PATH, { at: null, gevonden: 0, toegevoegd: 0, checked: [], error: null });
}
export async function writeCrawlLog(data) {
  return writeJsonBlob(CRAWL_LOG_PATH, { ...data });
}
