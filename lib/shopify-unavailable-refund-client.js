import { addTagsToShopifyOrder } from './shopify-order-tag-client.js';
...
function lineUnitPrice(line = {}) {
  return moneyNumber(line.discounted_price || line.price || line.original_price || line.pre_tax_price || 0);
}

function lineAmount(line = {}, quantity = 1) {
  const requestedQuantity = Math.max(1, Number(quantity || 1));
  const price = lineUnitPrice(line);
  if (price > 0) return moneyNumber(price * requestedQuantity);
  return moneyNumber(line.price_set?.shop_money?.amount || line.total_discount_set?.shop_money?.amount || 0);
}
...
function refundableQuantityForLine(order = {}, line = {}, extraRefundQuantity = 0) {
  const quantity = Number(line.quantity || 0);
  const refunded = refundedQuantityForLine(order, line.id);
  return Math.max(0, quantity - refunded - Number(extraRefundQuantity || 0));
}

function refundCompletesOrder(order = {}, matchedLine = {}, refundQuantity = 0) {
  const lines = Array.isArray(order.line_items) ? order.line_items : [];
  if (!lines.length) return false;

  return lines.every((line) => {
    const extra = String(line.id) === String(matchedLine.id) ? Number(refundQuantity || 0) : 0;
    return refundableQuantityForLine(order, line, extra) <= 0;
  });
}

async function tagOrderForFlow(order) {
  return addTagsToShopifyOrder({ order, tags: ['niet-leverbaar-refund'], shopifyRequest });
}
...
export async function refundUnavailableOrderLine({ orderNr, item = {}, quantity = 1, employeeName = 'Administratie', note = '', refundShippingIfOrderComplete = true } = {}) {
  ...
  if (!matchedLine) {
    ...
    const error = new Error(`Geen Shopify orderregel gevonden voor ${item.sku || item.barcode || item.title || 'artikel'}. Beschikbare regels: ${available.map((line) => `${line.title || line.name || '-'} / SKU ${line.sku || '-'} / bedrag ${line.price || '-'}`).join(' | ')}`);
    error.data = { requestedItem: item, availableLineItems: available };
    throw error;
  }

  const tagResult = await tagOrderForFlow(order);
  ...
  const matchedAmount = lineAmount(matchedLine, refundQuantity || quantity || 1);
  const shippingFullRefund = Boolean(refundShippingIfOrderComplete && refundCompletesOrder(order, matchedLine, refundQuantity));
  ...
  if (refundQuantity <= 0) {
    return {
      ...
      matchedAmount,
      refundAmount: 0,
      shippingFullRefund: false,
      tagResult,
      message: 'Deze Shopify orderregel lijkt al volledig terugbetaald.'
    };
  }

  const refundLineItems = [{ line_item_id: matchedLine.id, quantity: refundQuantity, restock_type: 'no_restock' }];
  const shipping = { full_refund: shippingFullRefund };

  const calculated = await shopifyRequest(`/orders/${order.id}/refunds/calculate.json`, {
    method: 'POST',
    body: JSON.stringify({ refund: { shipping, refund_line_items: refundLineItems } })
  });
  ...
  const refundAmount = moneyNumber(transactions.reduce((sum, transaction) => sum + Number(transaction.amount || 0), 0) || matchedAmount);
  ...
  const createRefundPayload = {
    refund: {
      ...
      note: [
        ...
        'Shopify tag: niet-leverbaar-refund.',
        `Medewerker: ${employeeName}.`,
        `Order: ${order.name}.`,
        `Artikel: ${matchedLine.title}.`,
        `SKU: ${matchedLine.sku || '-'}.`,
        `Refundbedrag: ${refundAmount}.`,
        shippingFullRefund ? 'Volledige order terugbetaald: verzendkosten zijn ook terugbetaald.' : '',
