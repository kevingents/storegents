...
export async function refundUnavailableOrderLine({ orderNr, item = {}, quantity = 1, employeeName = 'Administratie', note = '' } = {}) {
  ...
  const calculated = await shopifyRequest(`/orders/${order.id}/refunds/calculate.json`, {
    method: 'POST',
    body: JSON.stringify({ refund: { shipping: { full_refund: false }, refund_line_items: refundLineItems } })
  });
