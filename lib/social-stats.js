...
/* Per-post insights: bereik + totale interacties + saves. Metric-namen wisselen
   per mediatype, dus fallback naar een minimale set bij een fout. */
async function mediaInsights(mediaId, token) {
  ...
  try { return pick(await graph(`${mediaId}/insights`, { metric: 'reach,total_interactions,saved', access_token: token }, 12000)); }
  catch {
    try { return pick(await graph(`${mediaId}/insights`, { metric: 'reach,total_interactions', access_token: token }, 12000)); }
    catch { return null; }
  }
}
...
export async function getSocialStats({ days = 30 } = {}) {
  ...
  /* Per-post insights (bereik + interacties → engagement%) — alleen als de
     inzichten-scope werkt, parallel met max 5 tegelijk. */
  if (out.insightsOk && Array.isArray(out.posts) && out.posts.length) {
    await mapPool(out.posts, 5, async (po) => {
      ...
      if (ins) {
        ...
        po.engagementPct = (ins.reach > 0 && ins.total_interactions != null) ? Math.round((ins.total_interactions / ins.reach) * 1000) / 10 : null;
      }
    });
    const eng = out.posts.map((p) => p.engagementPct).filter((v) => v != null);
    out.avgEngagementPct = eng.length ? Math.round((eng.reduce((s, v) => s + v, 0) / eng.length) * 10) / 10 : null;
