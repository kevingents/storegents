...
function updatedFromValue(updatedFrom = '') {
  if (clean(updatedFrom)) return clean(updatedFrom);
...
export async function getDragerInfo({ store = '', dragerId = '', branchId = '', updatedFrom = '' } = {}) {
  const body = `<drd:GetDragerInfo>${loginBlock('drd')}<drd:Body>${tag('drd:UpdatedFrom', updatedFromValue(updatedFrom))}</drd:Body></drd:GetDragerInfo>`;
