...
const DEFAULT_SITE = 'sc-domain:gents.nl';

const clean = (v) => String(v == null ? '' : v).trim();

export function getGscConfig() {
  /* Het gedeelde Google-OAuth-token GOOGLE_REFRESH_TOKEN (zelfde als Ads/
     Analytics/Business) is de bron van waarheid zodra de scope webmasters.readonly
     eraan hangt — daarom LEIDEND. De aparte GSC-env-vars werken alleen nog als het
     gedeelde token leeg is, zodat een oude/scope-loze GSC-token niets meer breekt. */
  const oauth = googleOAuthClient();
  const clientId = clean(process.env.GOOGLE_CLIENT_ID || process.env.GOOGLE_BUSINESS_CLIENT_ID) || oauth.clientId;
  const clientSecret = clean(process.env.GOOGLE_CLIENT_SECRET || process.env.GOOGLE_BUSINESS_CLIENT_SECRET) || oauth.clientSecret;
  const refreshToken = googleRefreshToken() || clean(process.env.GOOGLE_SEARCH_CONSOLE_REFRESH_TOKEN || process.env.GSC_REFRESH_TOKEN);
  const siteUrl = clean(process.env.GSC_SITE_URL || process.env.GOOGLE_SEARCH_CONSOLE_SITE_URL) || DEFAULT_SITE;
  const missing = [];
  if (!clientId) missing.push('GOOGLE_CLIENT_ID');
  if (!clientSecret) missing.push('GOOGLE_CLIENT_SECRET');
  if (!refreshToken) missing.push('GOOGLE_REFRESH_TOKEN (scope webmasters.readonly)');
