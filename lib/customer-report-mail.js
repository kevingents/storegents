 ...
 *  - laatste dag van de lopende maand (isLastDay): ≥100 groen, daaronder rood
 */
export function pctBand(pct, refDay, isFinal, isLastDay = false) {
  if (pct == null) return 'muted';
  if (isFinal) return pct >= 100 ? 'green' : pct >= 80 ? 'orange' : 'red';
  /* Laatste dag van de lopende maand: de volle 100% moet staan, anders rood. */
  if (isLastDay) return pct >= 100 ? 'green' : 'red';
...
export function rowMetrics(r) {
  ...
  /* E-mail-dekking: met-e-mail ÷ totaal nieuw. E-mail is verplicht bij een
     inschrijving, dus dit hoort 100% te zijn. <100% = inschrijving(en) zonder
     e-mail geregistreerd (tellen niet mee voor target). */
  const capture = nieuw > 0 ? round((withEmail / nieuw) * 100) : null;
  return { store: r.store, nieuw, withEmail, withoutEmail, target, bonnen, pctEmail, conv, capture };
}
...
export function buildOverviewEmailHtml({ mode, label, range, rows, asOfDay, isFinal, nextTargets, includePodium = true }) {
  ...
  /* Laatste dag van de lopende maand? Dan moet de % t.o.v. target op 100% staan.
     range.to = einddatum van de periode (vandaag bij het lopende-maand-rapport). */
  let isLastDay = false;
  if (!isFinal && range && range.to) {
    const end = new Date(`${range.to}T12:00:00Z`);
    if (!Number.isNaN(end.getTime())) {
      const lastDom = new Date(Date.UTC(end.getUTCFullYear(), end.getUTCMonth() + 1, 0)).getUTCDate();
      isLastDay = (Number(asOfDay) || end.getUTCDate()) >= lastDom;
    }
  }

  /* Conversie-kleur: <15% rood · 15–20% oranje · ≥20% groen. */
  const convColor = (p) => p == null ? '#94a3b8' : p < 15 ? '#dc2626' : p < 20 ? '#d97706' : '#16a34a';
  const convHtml = (p, bold = false) => p == null
    ? `<span style="color:#94a3b8">${bold ? '<strong>—</strong>' : '—'}</span>`
    : `<strong style="color:${convColor(p)}">${p}%</strong>`;
  ...
  /* Totale e-mail-dekking (met-e-mail ÷ totaal nieuw). Hoort 100% te zijn. */
  const totCapture = T.nieuw > 0 ? round((T.withEmail / T.nieuw) * 100) : null;
  ...
  const kpis = `
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-bottom:6px"><tr>
      ...
      ${kpiCard('Met e-mail', String(T.withEmail), totCapture == null ? 'telt mee voor target' : `${totCapture}% dekking${T.withoutEmail > 0 ? ` · ${T.withoutEmail} zonder` : ''}`, '#16a34a')}
      ${kpiCard('% met e-mail vs target', totPctEmail == null ? '—' : totPctEmail + '%', `${T.withEmail} / ${T.target || '—'}`, bandColor(pctBand(totPctEmail, asOfDay, isFinal, isLastDay)))}
    </tr><tr>
  ...
  if (includePodium && mode === 'weekly') {
    /* Podium op RELATIEVE prestatie: % inschrijvingen met e-mail t.o.v. target,
       niet de absolute aantallen. Zo wint een kleine winkel die zijn target haalt
       van een grote winkel die er ver onder zit. Winkels zonder target kunnen niet
       relatief gerankt worden → val terug op absolute aantallen als geen enkele
       winkel een target heeft. */
    const ranked = [...metrics].filter((r) => r.pctEmail != null);
    const useRelative = ranked.length > 0;
    const top = (useRelative
      ? ranked.sort((a, b) => (b.pctEmail - a.pctEmail) || (b.withEmail - a.withEmail))
      : [...metrics].filter((r) => r.withEmail > 0).sort((a, b) => b.withEmail - a.withEmail)
    ).slice(0, 3);
    if (top.length) {
      const medals = ['#d4af37', '#9ca3af', '#b87333']; /* goud / zilver / brons */
      podium = `<h3 style="margin:0 0 8px;font-size:15px;color:#0a1f33">Podium — hoogste % inschrijvingen met e-mail</h3>
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-bottom:20px"><tr>${top.map((r, i) => `
        <td width="33%" style="padding:6px" valign="top"><div style="border:1px solid #e1e6eb;border-left:4px solid ${medals[i]};border-radius:12px;padding:12px 14px;background:#fff">
          ...
          <div style="font-size:13px;color:${useRelative ? bandColor(pctBand(r.pctEmail, asOfDay, isFinal, isLastDay)) : '#16a34a'};font-weight:600;margin-top:2px">${useRelative ? `${r.pctEmail}% van target` : `${r.withEmail} met e-mail`}</div>
  ...
  /* E-mail-dekking: groen bij 100%, rood eronder (er glipte een inschrijving
     zonder e-mail door). E-mail is verplicht. */
  const captureHtml = (p) => p == null ? `<span style="color:#94a3b8">—</span>` : `<strong style="color:${p >= 100 ? '#16a34a' : '#dc2626'}">${p}%</strong>`;
  /* Winkelnaam uniform: "GENTS" (klein/grijs) boven de plaatsnaam (vet) — voor
     ELKE winkel, niet alleen de namen die niet naast elkaar passen. */
  const storeCell = (name) => {
    const n = String(name == null ? '' : name).trim();
    const m = n.match(/^GENTS\s+(.+)$/i);
    return m
      ? `<span style="font-size:10.5px;color:#94a3b8;letter-spacing:.04em">GENTS</span><br><strong>${esc(m[1])}</strong>`
      : `<strong>${esc(n || '—')}</strong>`;
  };
  const bodyRows = sorted.map((r) => {
    const band = pctBand(r.pctEmail, asOfDay, isFinal, isLastDay);
    return `<tr>
      ${td(storeCell(r.store))}
      ${td(String(r.nieuw), 'right')}
      ${td(`<span style="color:#16a34a;font-weight:600">${r.withEmail}</span>`, 'right')}
      ${td(captureHtml(r.capture), 'right')}
      ${td(r.target ? String(r.target) : '—', 'right')}
      ${td(pctHtml(r.pctEmail, band), 'right')}
      ${td(convHtml(r.conv), 'right')}
    </tr>`;
  }).join('');
  const totalRow = `<tr style="background:#f8fafc">
    ...
    ${td(`<strong style="color:${totCapture == null ? '#94a3b8' : totCapture >= 100 ? '#16a34a' : '#dc2626'}">${totCapture == null ? '—' : totCapture + '%'}</strong>`, 'right')}
    ${td(`<strong>${T.target || '—'}</strong>`, 'right')}
    ${td(`<strong style="color:${bandColor(pctBand(totPctEmail, asOfDay, isFinal, isLastDay))}">${totPctEmail == null ? '—' : totPctEmail + '%'}</strong>`, 'right')}
    ${td(convHtml(totConv, true), 'right')}
  </tr>`;
  const table = `<h3 style="margin:20px 0 8px;font-size:15px;color:#0a1f33">Per winkel — gesorteerd op % inschrijvingen met e-mail</h3>
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;border:1px solid #e1e6eb;border-radius:14px;overflow:hidden">
      <thead><tr style="background:#f1f5f9">${th('Winkel')}${th('Nieuw', 'right')}${th('Met e-mail', 'right')}${th('% e-mail', 'right')}${th('Target', 'right')}${th('% target', 'right')}${th('Conversie', 'right')}</tr></thead>
      <tbody>${bodyRows || '<tr><td colspan="7" style="padding:10px;color:#94a3b8">Geen data voor deze periode.</td></tr>'}${bodyRows ? totalRow : ''}</tbody>
    </table>`;
  ...
  const targetBandText = isFinal
    ? `<span style="color:#16a34a">groen ≥100%</span> · <span style="color:#d97706">oranje 80–100%</span> · <span style="color:#dc2626">rood &lt;80%</span> (maandafsluiting)`
    : `na 7 dagen ≥25%, na 14 dagen ≥50%, na 21 dagen ≥75%, laatste dag van de maand ≥100% = groen, daaronder rood`;
  const legend = `<p style="font-size:12px;color:#64748b;margin-top:16px"><strong>% e-mail</strong> = inschrijvingen mét e-mail ÷ totaal nieuw — hoort <span style="color:#16a34a">100%</span> te zijn, want we accepteren geen inschrijving zonder e-mail. <span style="color:#dc2626">Rood</span> = er is een inschrijving zonder e-mail geregistreerd (telt niet mee voor de target).<br><strong>% target</strong> = voortgang t.o.v. doel (alleen inschrijvingen mét e-mail tellen mee): ${targetBandText}.</p>`;
  ...
  return baseMailHtml({
    ...
    intro: mode === 'monthly'
      ...
      : `Volledig overzicht van het totaal aantal ingeschreven klanten van deze maand t/m vandaag`,
    bodyHtml: `${podium}${periodLine}${kpis}${table}${nextSection}${legend}`
  });
}
...
/* Preview voor de instellingen-kaart: hoeveel ontvangers krijgt de mail nu? */
export async function previewRecipients(config = DEFAULT_MAIL_CONFIG) {
  let storeEmails = [];
  if (config.includeStoreEmails !== false) {
    try {
      const map = await getAllStoreEmails();
      storeEmails = Object.values(map || {}).map((v) => String(v || '').trim().toLowerCase()).filter((e) => e && EMAIL_RE.test(e));
    } catch { /* geen winkel-emails */ }
  }
  const extra = (config.extraRecipients || []).map((r) => String(r?.email || '').trim().toLowerCase()).filter((e) => e && EMAIL_RE.test(e));
  const total = [...new Set([...storeEmails, ...extra])];
  return {
    storeEmailCount: new Set(storeEmails).size,
    extraCount: new Set(extra).size,
    total: total.length,
    sample: total.slice(0, 8)
  };
}
