...
export function buildOverviewEmailHtml({ mode, label, range, rows, asOfDay, isFinal, nextTargets, includePodium = true }) {
  ...
  const bodyRows = sorted.map((r) => {
    ...
    return `<tr>
      ...
      ${td(`<span style="color:${r.withoutEmail > 0 ? '#dc2626' : '#94a3b8'};font-weight:${r.withoutEmail > 0 ? '700' : '400'}">${r.withoutEmail}</span>`, 'right')}
  ...
  const totalRow = `<tr style="background:#f8fafc">
    ...
    ${td(`<strong style="color:${T.withoutEmail > 0 ? '#dc2626' : '#94a3b8'}">${T.withoutEmail}</strong>`, 'right')}
  ...
  const table = `<h3 style="margin:20px 0 8px;font-size:15px;color:#0a1f33">Per winkel — gesorteerd op % inschrijvingen met e-mail</h3>
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;border:1px solid #e1e6eb;border-radius:14px;overflow:hidden">
      <thead><tr style="background:#f1f5f9">${th('Winkel')}${th('Nieuw', 'right')}${th('Met e-mail', 'right')}${th('Zonder', 'right')}${th('% e-mail', 'right')}${th('Target', 'right')}${th('% target', 'right')}${th('Bonnen', 'right')}${th('Conversie', 'right')}</tr></thead>
      <tbody>${bodyRows || '<tr><td colspan="9" style="padding:10px;color:#94a3b8">Geen data voor deze periode.</td></tr>'}${bodyRows ? totalRow : ''}</tbody>
    </table>`;
