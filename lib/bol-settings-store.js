...
export async function getBolSettings() {
  ...
  return {
    ...
    /* SRS-weborder routing voor bol-orders.
       srsShopId = "GEPLAATST IN" (welke webshop) in SRS. 91 = GENTS Webshop Bol,
       apart van 90 (gewone webshop), zodat bol-orders herkenbaar gescheiden zijn.
       Let op: val NIET terug op SRS_WEBORDER_SHOP_ID (dat is 90) — anders belanden
       bol-orders weer in de gewone webshop. */
    srsShopId: Number.isFinite(s.srsShopId) ? s.srsShopId : envNum('SRS_BOL_SHOP_ID', 91),
    /* Verkoop-/omzetfiliaal = 91 (GENTS Webshop Bol): de bol-omzet wordt op 91
       geboekt, niet op de Showroom. Verzend-/afhaalfiliaal blijft 99 (Magazijn). */
    srsSellingBranchId: Number.isFinite(s.srsSellingBranchId) ? s.srsSellingBranchId : envNum('SRS_BOL_SELLING_BRANCH_ID', 91),
    srsFulfilmentBranchId: Number.isFinite(s.srsFulfilmentBranchId) ? s.srsFulfilmentBranchId : envNum('SRS_BOL_FULFILMENT_BRANCH_ID', 99),
...
export async function saveBolSettings(patch = {}) {
  ...
  setNum('stockBuffer', 0, 25); /* begrensd: een te hoge marge zou offers onterecht op 0 zetten */
  setNum('shippingSurcharge', 0, 100);
  setNum('parityTolerance', 0, 10);
  /* SRS-routing: shop + filialen (ruime bovengrens; SRS-id's zijn klein). */
  setNum('srsShopId', 1, 99999);
  setNum('srsSellingBranchId', 1, 99999);
  setNum('srsFulfilmentBranchId', 1, 99999);
