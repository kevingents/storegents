function getShopifyConfig() {
  const shopDomain = process.env.SHOPIFY_SHOP_DOMAIN || process.env.SHOPIFY_STORE_DOMAIN || process.env.SHOPIFY_STORE_URL || '';
  ...
  return {
    endpoint: `https://${shopDomain.replace(/^https?:\/\//, '').replace(/\/$/, '')}/admin/api/${apiVersion}/graphql.json`,
    token
  };
}
...
function normalizeAmount(amount) {
  ...
  if (!Number.isFinite(value) || value <= 0) {
    throw new Error('Ongeldige giftcard waarde.');
  }
...
function customerMetafieldQueries(namespace, key, id) {
  const escapedId = String(id || '').replace(/"/g, '\\"');
  return [
    `metafields.${namespace}.${key}:"${escapedId}"`,
    `metafields.${namespace}.${key}:${escapedId}`
  ];
}

export async function findShopifyCustomerBySrsCustomerId(customerId, namespace = 'SRSERP', key = 'customer_id') {
  const id = String(customerId || '').trim();
  if (!id) return null;

  for (const query of customerMetafieldQueries(namespace, key, id)) {
    const data = await shopifyGraphql(
      `query FindCustomerByMetafield($query: String!, $namespace: String!, $key: String!) {
        customers(first: 1, query: $query) {
          nodes {
            id
            email
...
export async function updateShopifyCustomerMetafields(customerId, metafields) {
  if (!customerId) {
    throw new Error('Shopify customer ID ontbreekt.');
  }
...
export async function createShopifyGiftCard({ code, amount, currencyCode = 'EUR', expiresOn, note, customerEmail }) {
  ...
  const input = {
    initialValue: normalizeAmount(amount),
    code,
