...
function readGoogleApiKey() {
  return clean(
    process.env.GOOGLE_API_VERCEL_KEY ||
    process.env.GOOGLE_REVIEWS_API_KEY_BACKEND ||
    process.env.GOOGLE_REVIEWS_API_KEY ||
    process.env.GOOGLE_API_KEY
  );
}

function getGooglePlaceIdMap() {
...
function resolveMappedPlaceId(location = {}) {
  ...
  const keys = [
    clean(location.branchId),
    clean(location.storeId),
    clean(location.store_id),
    clean(location.googleLocationCode),
    clean(location.store),
    normalizeKey(location.store),
    clean(location.slug)
  ].filter(Boolean);
...
async function fetchGoogleJson(url, { timeoutMs = 15000, fieldMask } = {}) {
  ...
  try {
    ...
    if (!response.ok) {
      const message = data?.error?.message || data?.error?.status || `Google HTTP ${response.status}`;
...
async function shopifyGraphql(query, variables = {}, { timeoutMs = 20000 } = {}) {
  ...
  try {
    const response = await fetch(`https://${resolveShopifyDomain()}/admin/api/${SHOPIFY_API_VERSION}/graphql.json`, {
      ...
      headers: {
        'Content-Type': 'application/json',
        'X-Shopify-Access-Token': resolveShopifyToken()
      },
...
function pointDate(point = {}) {
  if (point.date?.year && point.date?.month && point.date?.day) {
    return `${point.date.year}-${padTime(point.date.month)}-${padTime(point.date.day)}`;
  }
...
function periodsToDayMap(periods = []) {
  ...
  for (const [dayName, list] of ranges.entries()) {
    map[dayName] = list.join(', ');
  }
...
function locationCandidates(location = {}) {
  return [
    clean(location.shopifyMetaobjectId),
    clean(location.metaobjectId),
    clean(location.shopifyHandle),
    clean(location.handle),
    clean(location.slug),
    clean(location.store_id),
    clean(location.storeId),
    clean(location.branchId),
...
async function findShopifyStoreLocationMetaobject(location = {}) {
  ...
  const found = nodes.find((node) => {
    const values = [
      node.id,
      node.handle,
      fieldValue(node.fields, 'slug'),
      fieldValue(node.fields, 'store_id'),
      fieldValue(node.fields, 'title'),
      fieldValue(node.fields, 'city'),
  ...
  if (!found) {
    throw new Error(`Geen Shopify metaobject gevonden voor ${location.store || location.slug || location.branchId || 'winkel'}. Voeg shopifyMetaobjectId/handle/slug/store_id toe aan GOOGLE_STORE_LOCATIONS_JSON.`);
  }
...
export async function getGoogleOpeningHoursForLocation(location = {}, { language = 'nl', timeoutMs = 15000 } = {}) {
  ...
  const data = await fetchGoogleJson(`${GOOGLE_PLACES_NEW_BASE_URL}/places/${encodeURIComponent(placeId)}?languageCode=${encodeURIComponent(language)}`, {
    timeoutMs,
    fieldMask: GOOGLE_OPENING_HOURS_FIELD_MASK
  });
  ...
  return {
    placeId,
    placeResourceName: data.name || '',
    name: data.displayName?.text || location.store || '',
    address: data.formattedAddress || location.address || '',
    googleMapsUrl: data.googleMapsUri || '',
    hoursJson: regularHoursJson,
    specialHoursJson,
    todayText,
    raw: {
...
export async function syncGoogleOpeningHoursToShopify(location = {}, options = {}) {
  ...
  return {
    ...
    store: location.store || hours.name,
    branchId: clean(location.branchId || location.storeId || location.store_id),
    slug: clean(location.slug),
    metaobjectId: target.id,
    metaobjectHandle: target.handle || updated?.handle || '',
    match: target.match,
    fields,
    hoursJson: hours.hoursJson,
    specialHoursJson: hours.specialHoursJson,
    todayText: hours.todayText,
    google: {
      placeId: hours.placeId,
      name: hours.name,
      address: hours.address,
      googleMapsUrl: hours.googleMapsUrl
    }
  };
}
