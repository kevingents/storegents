...
function specialHoursFromCurrentPeriods(currentOpeningHours = {}, regularHoursJson = {}) {
  ...
  const special = {};
  const datesToCheck = new Set([...byDate.keys()]);

  if (currentPeriods.length) {
    for (let offset = 0; offset < 7; offset += 1) {
      datesToCheck.add(dateIsoOffset(offset));
    }
  }

  [...datesToCheck].sort().forEach((date) => {
    const jsDay = new Date(`${date}T12:00:00Z`).getUTCDay();
    const regular = regularHoursJson[DAY_NAMES[jsDay]] || 'gesloten';
    const current = byDate.has(date) ? byDate.get(date).join(', ') : 'gesloten';
...
function specialHoursEntries(specialHoursJson = {}) {
  return Object.entries(specialHoursJson)
    .filter(([date]) => date >= todayIso())
    .sort(([a], [b]) => a.localeCompare(b))
    .map(([date, value]) => {
      if (value === 'gesloten') return { date, closed: true, label: 'Aangepaste openingstijd' };
      const [open, close] = String(value).split('-');
      return { date, open: clean(open), close: clean(close), label: 'Aangepaste openingstijd' };
    });
}
function todayTextFromHours(currentOpeningHours = {}, regularHoursJson = {}) {
  ...
  return value === 'gesloten' ? 'Vandaag gesloten' : `${dayName}: ${value}`;
}
...
async function findShopifyStoreLocationMetaobject(location = {}) {
  ...
  if (explicitId.startsWith('gid://shopify/Metaobject/')) return { id: explicitId, match: 'explicit-id', fields: [] };
  const query = `#graphql
  ...
  return { id: found.id, handle: found.handle, match: 'search', fields: found.fields || [] };
}
...
/**
 * Haalt Google-reviews op voor één winkel-locatie + bouwt de "Schrijf review"
 * deeplink. Bedoeld voor de welkom-mail (en latere reviews-widgets).
 *
 * Input: location { placeId? | branchId | store | slug } — zelfde resolve-pad
 * als opening-hours.
 *
 * Output:
 *   {
 *     placeId, name, googleMapsUrl,
 *     rating: 4.8, userRatingCount: 1081,
 *     writeReviewUrl: 'https://search.google.com/local/writereview?placeid=…',
 *     reviews: [{ author, photo, rating, text, relativeTime, publishTime, language }]
 *   }
 *
 * Google's `reviews` veld retourneert max ~5 reviews (door Google gefilterd op
 * "most helpful"). We sorteren op rating DESC dan publishTime DESC zodat de
 * mail de beste recente reviews toont.
 */
export async function getGoogleReviewsForLocation(location = {}, { language = 'nl', timeoutMs = 15000, minRating = 4, max = 3 } = {}) {
  ...
  const data = await fetchGoogleJson(
    `${GOOGLE_PLACES_NEW_BASE_URL}/places/${encodeURIComponent(placeId)}?languageCode=${encodeURIComponent(language)}`,
    { timeoutMs, fieldMask: GOOGLE_REVIEWS_FIELD_MASK }
  );
  const rawReviews = Array.isArray(data.reviews) ? data.reviews : [];
  const reviews = rawReviews
    .map((r) => ({
      author: clean(r?.authorAttribution?.displayName),
      photo: clean(r?.authorAttribution?.photoUri),
      rating: Number(r?.rating || 0),
      text: clean(r?.text?.text || r?.originalText?.text || ''),
      language: clean(r?.text?.languageCode || r?.originalText?.languageCode || ''),
      relativeTime: clean(r?.relativePublishTimeDescription),
      publishTime: clean(r?.publishTime)
    }))
    .filter((r) => r.text && r.rating >= minRating)
    .sort((a, b) => (b.rating - a.rating) || String(b.publishTime).localeCompare(String(a.publishTime)))
    .slice(0, Math.max(1, max));
  return {
    placeId,
    name: data.displayName?.text || location.store || '',
...
export async function syncGoogleOpeningHoursToShopify(location = {}, options = {}) {
  ...
  const specialJsonValue = JSON.stringify(hours.specialHoursJson, null, 2);
  const specialListValue = JSON.stringify(hours.specialHoursList, null, 2);
  const fields = [
    ...
    { key: process.env.SHOPIFY_SPECIAL_HOURS_FIELD_KEY || 'special_hours_json', value: specialJsonValue }
  ];
  if (process.env.SHOPIFY_WRITE_SPECIAL_HOURS_TO_SERVICES !== 'false') fields.push({ key: process.env.SHOPIFY_SERVICES_FIELD_KEY || 'services', value: specialListValue });
  const updated = options.dryRun ? null : await updateShopifyStoreLocationFields(target.id, fields);
  return { success: true, dryRun: Boolean(options.dryRun), store: location.store || hours.name, branchId: clean(location.branchId || location.storeId || location.store_id), slug: clean(location.slug), metaobjectId: target.id, metaobjectHandle: target.handle || updated?.handle || '', match: target.match, writtenFieldKeys: fields.map((field) => field.key), fields, updatedFields: updated?.fields || [], existingFieldKeys: target.fields.map((field) => field.key), hoursJson: hours.hoursJson, specialHoursJson: hours.specialHoursJson, specialHoursList: hours.specialHoursList, todayText: hours.todayText, google: { placeId: hours.placeId, name: hours.name, address: hours.address, googleMapsUrl: hours.googleMapsUrl } };
}
