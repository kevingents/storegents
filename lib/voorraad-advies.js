...
/* Resolver die een SRS sku_code probeert te matchen op de productcache via
   meerdere indexen (barcode → sku → artikelnummer → artikel_id), zodat zo veel
   mogelijk regels een titel/maat krijgen i.p.v. een kale barcode. */
function makeLookup({ byBarcode = {}, bySku = {}, bySrsArticleNumber = {}, bySrsArtikelId = {} } = {}) {
  return (sku) => {
    const k = lc(sku);
    if (!k) return {};
    return byBarcode[k] || bySku[k] || bySrsArticleNumber[k] || bySrsArtikelId[k] || {};
  };
}

function labelFor(sku, lookup) {
  const info = lookup(sku) || {};
  const title = clean(info.title);
  /* Mens-leesbaar eerst: titel > artikelnummer > kale sku/barcode. */
  const head = title || clean(info.articleNumber) || sku;
  return {
    sku,
    label: head,
    title,
    ...
    image: clean(info.image),
    artikelId: clean(info.srsArtikelId),
    found: Boolean(title || info.articleNumber)
  };
}
 ...
 * @param {Object} [p.cache]       volledige productcache (byBarcode/bySku/bySrs*) voor bredere matching
 ...
 */
export function computeVoorraadAdvies({ verkopen = [], voorraadRows = [], byBarcode = {}, cache = null, physical, window = null, windowDays = 14 } = {}) {
  const isPhysical = (fil) => (physical ? physical.has(String(fil)) : true);
  /* Productcache-resolver: gebruik de volledige cache indien meegegeven, anders
     alleen de byBarcode-index (backward-compatible). */
  const lookup = makeLookup({
    byBarcode: cache?.byBarcode || byBarcode || {},
    bySku: cache?.bySku || {},
    bySrsArticleNumber: cache?.bySrsArticleNumber || {},
    bySrsArtikelId: cache?.bySrsArtikelId || {}
  });
  const sizeOf = (sku) => clean(lookup(sku).size);
  ...
  for (const [fil, rows] of stockFil.entries()) {
    ...
    for (const r of inStock) {
      ...
      if (sold > 0 && c.dos >= 30) kansen.push({ ...labelFor(r.sku, lookup), voorraad: r.voorraad, sold });
      /* Overvoorraad = boven target, of een dode stapel. */
      const over = r.ideaal > 0 ? (r.voorraad - r.ideaal) : 0;
      if ((over >= 2) || (sold === 0 && r.voorraad >= 5)) {
        overvoorraad.push({ ...labelFor(r.sku, lookup), voorraad: r.voorraad, ideaal: r.ideaal, over: Math.max(over, 0), sold });
      }
    }
    for (const [sku, units] of sales.entries()) {
      const size = sizeOf(sku);
  ...
  for (const [sku, c] of stockSkuChain.entries()) {
    ...
    if (cl.hard && cl.dos >= HARD_DAYS) kansenG.push({ ...labelFor(sku, lookup), voorraad: c.voorraad, sold });
    else if (sold > 0 && cl.dos >= 30) kansenG.push({ ...labelFor(sku, lookup), voorraad: c.voorraad, sold });
    if (cl.hard && cl.dos <= 14) nietAdverteren.push({ ...labelFor(sku, lookup), voorraad: c.voorraad, sold, dagen: round1(cl.dos) });
    const over = c.ideaal > 0 ? (c.voorraad - c.ideaal) : 0;
    if ((over >= 3) || (sold === 0 && c.voorraad >= 8)) overvoorraadG.push({ ...labelFor(sku, lookup), voorraad: c.voorraad, ideaal: c.ideaal, over: Math.max(over, 0), sold });
  }
