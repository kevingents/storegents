...
import { suggestTitle } from './mixmatch-publish.js';
...
export async function savePakket(input = {}, updatedBy = 'admin') {
  ...
  if (Array.isArray(input.components) && input.components.length) {
    const enriched = await enrichComponents(input.components);
    /* Geen naam ingevuld? Stel er automatisch een voor op basis van de colbert. */
    const naam = clean(input.naam) || suggestTitle(enriched) || 'Mix & Match pak';
    input = { ...input, components: enriched, naam };
  }
...
/** Bewaar de aangemaakte Shopify bundle-producten bij een pakket. */
export async function setBundleProducts(id, bundleProducts) {
  const target = clean(id);
  if (!target) return null;
  const { pakketten } = await readPakketten();
  const idx = pakketten.findIndex((p) => p.id === target);
  if (idx === -1) return null;
  pakketten[idx] = { ...pakketten[idx], bundleProducts: Array.isArray(bundleProducts) ? bundleProducts : [], bundlePublishedAt: new Date().toISOString() };
  await writeJsonBlob(PATH, { pakketten, updatedAt: new Date().toISOString() });
  return pakketten[idx];
}
