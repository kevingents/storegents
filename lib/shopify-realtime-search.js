    ...
    products(first: $first, query: $q) {
      edges {
        node {
          ...
          descriptionHtml
          metafields(first: 20, namespace: "${SRS_METAFIELD_NAMESPACE}") {
...
export async function realtimeSearch({ q, ownStore = '', limit = 30 } = {}) {
  ...
  const data = await callShopifyGraphQL(SEARCH_QUERY, { q: shopifyQuery, first: limit });
  const products = (data?.products?.edges || []).map((e) => e.node);

  /* Bouw flat list van variant-rows (zelfde shape als oude article-search) */
  const results = [];
  for (const product of products) {
    ...
    const images = (product?.images?.edges || []).map((e) => clean(e?.node?.url)).filter(Boolean);
    const featuredImage = clean(product?.featuredImage?.url) || images[0] || '';
    const productHandle = clean(product?.handle);

    /* SRSERP metafields per product — kan onder verschillende key-namen
       opgeslagen zijn. Probeer meerdere variants. */
    ...
    for (const vEdge of (product?.variants?.edges || [])) {
      ...
      let totalPieces = 0;
      let branchCount = 0;
      for (const lvlEdge of (variant?.inventoryItem?.inventoryLevels?.edges || [])) {
        const lvl = lvlEdge?.node;
        if (!lvl) continue;
        const storeName = locationToStoreName(lvl.location);
        if (!storeName) continue;
        const available = Number((lvl.quantities || []).find((qx) => qx.name === 'available')?.quantity ?? lvl.available ?? 0);
      ...
      /* Sorteer branches: eigen eerst, dan op pieces aflopend */
      branches.sort((a, b) => {
      ...
      /* articleNumber: SRSERP-rve_artikelnummer > artikel_id > sku */
