 ...
 *
 * BELANGRIJK: voor naam-zoek MOET een wildcard (*) gebruikt worden, anders
 * matched "rokjas" niet "Rokjas polywol" of "Rokjas PV" (exact-title match).
 * Wildcards moeten aan het EIND van het woord staan (prefix-match).
 */
function buildShopifyQuery(q) {
  ...
  /* Multi-word: elk woord als prefix-match in title, AND-combineerd */
  if (/\s/.test(v)) {
  ...
  /* Pure letters → prefix-wildcard match in title + vendor.
     "rokjas" → matches "Rokjas polywol", "Rokjas PV", etc. */
...
/* Variant-search voor BARCODE queries. Shopify's products(query:) ondersteunt
   géén barcode:-filter, maar productVariants(query:) wél. We gebruiken deze
   query als barcode-specifieke fallback en mappen variants terug naar hun
   product. */
const VARIANT_BARCODE_SEARCH_QUERY = `
  query VariantBarcodeSearch($q: String!, $first: Int!) {
...
export async function realtimeSearch({ q, ownStore = '', limit = 30 } = {}) {
  ...
  /* Step 1: search products + variants metadata (NO inventory yet)
     ...
     BARCODE-PATH: Shopify products(query:) ondersteunt géén barcode:-filter,
     dus voor 8+ cijfer numerieke queries doen we eerst een productVariants-
     search. Bij hits mappen we elke variant terug naar zijn product (1
     product per variant in dit pad). Bij 0 hits valt het terug op de
     normale products-zoek. */
  const isBarcodeQuery = shopifyQuery.startsWith('barcode:');
  let products = [];

  if (isBarcodeQuery) {
    try {
      const vData = await callShopifyGraphQL(VARIANT_BARCODE_SEARCH_QUERY, { q: shopifyQuery, first: limit });
      const variantEdges = vData?.productVariants?.edges || [];
      /* Groepeer variants per product → produceer "products"-achtige nodes
         met enkel de gevonden variants erbij. */
      ...
      for (const ve of variantEdges) {
        ...
        productMap.get(pid).variants.edges.push({ node: variant });
      }
      products = Array.from(productMap.values());
    } catch (err) {
      console.warn('[shopify-realtime-search] variant barcode search failed:', err.message);
    }

    /* Fallback: probeer ook products(query: "sku:...") want soms staat de
       barcode-waarde óók in het SKU-veld */
    if (!products.length) {
      const skuQuery = shopifyQuery.replace(/^barcode:/, 'sku:');
      try {
        const sData = await callShopifyGraphQL(PRODUCT_SEARCH_QUERY, { q: skuQuery, first: limit });
        products = (sData?.products?.edges || []).map((e) => e.node);
      } catch (err) {
        console.warn('[shopify-realtime-search] sku fallback search failed:', err.message);
      }
    }
  } else {
  ...
  /* Step 2: batch-fetch inventory voor alle variant-IDs.
     Chunks van 30 om onder de 1000-cost te blijven (30 + 30×20 = 630).
     Track success per variant zodat we weten of inventory data betrouwbaar is. */
    ...
    try {
      ...
      for (const node of nodes) {
        ...
        for (const lvlEdge of (node?.inventoryItem?.inventoryLevels?.edges || [])) {
          ...
          if (!lvl) continue;
          /* GEEN store-mapping vereisten meer: gebruik location.name als-is.
             Vorig: skip als locationToStoreName='' → 0 pieces overal. */
          const storeName = locationToStoreName(lvl.location) || `Location ${clean(lvl.location?.id || '?').slice(-6)}`;
        ...
        variantsWithInventory.add(node.id);
        inventoryStatus.succeeded += 1;
      }
    } catch (err) {
      inventoryStatus.chunksFail += 1;
      inventoryStatus.errors.push(err.message);
  ...
  /* Markeer variants zonder inventory-data zodat frontend dit kan tonen */
  for (const [id, r] of variantIdToResult) {
    r.inventoryFetched = variantsWithInventory.has(id);
  }
...
/**
 * Realtime stock-fetch voor een lijst Shopify variant-IDs.
 *
 * Gebruikt door /api/store/article-stock voor lazy-loading vanuit de UI:
 * frontend belt eerst /api/store/article-search (instant, cache) zonder
 * stock, krijgt variant-IDs terug, en haalt vervolgens per kaartje of
 * batch de realtime stock op via dit endpoint.
 *
 * @param {object} input
 * @param {string[]} input.variantIds — Shopify GraphQL variant IDs
 *                                      (bv. 'gid://shopify/ProductVariant/123')
 * @param {string} [input.ownStore] — winkelnaam voor isOwn-flag
 * @returns {Promise<{
 *   stocks: Record<string, { branches: Array, totalPieces: number, branchCount: number, inventoryFetched: boolean }>,
 *   inventoryStatus: object
 * }>}
 */
export async function realtimeStockForVariants({ variantIds = [], ownStore = '' } = {}) {
  const ids = (Array.isArray(variantIds) ? variantIds : [])
    .map((v) => clean(v))
    .filter((v) => v.startsWith('gid://shopify/ProductVariant/'));
  if (!ids.length) return { stocks: {}, inventoryStatus: { attempted: 0, succeeded: 0, chunksOk: 0, chunksFail: 0 } };

  const stocks = {};
  const inventoryStatus = { attempted: ids.length, succeeded: 0, chunksOk: 0, chunksFail: 0, errors: [] };
  const CHUNK_SIZE = 30;

  /* Parallel chunks ipv sequentieel — versnelt ~3x voor grotere result-sets.
     Promise.allSettled zodat 1 faalende chunk niet de rest blokkeert. */
  const chunks = [];
  ...
  const results = await Promise.allSettled(
    chunks.map((chunk) => callShopifyGraphQL(INVENTORY_BATCH_QUERY, { ids: chunk }))
  );

  for (const r of results) {
    if (r.status !== 'fulfilled') {
      inventoryStatus.chunksFail += 1;
      inventoryStatus.errors.push(r.reason?.message || String(r.reason));
      continue;
    }
    inventoryStatus.chunksOk += 1;
    ...
    for (const node of nodes) {
      ...
      stocks[node.id] = { branches, totalPieces, branchCount, inventoryFetched: true };
      inventoryStatus.succeeded += 1;
    }
  }

  return { stocks, inventoryStatus };
}
