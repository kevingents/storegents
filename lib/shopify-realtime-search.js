 ...
 *
 * Strategie: voor naam-zoek gebruiken we Shopify's default multi-field
 * matching (title + vendor + product_type + tags + sku) door het bare term
 * te sturen. Voor codes filteren we expliciet op sku/barcode.
 */
function buildShopifyQuery(q) {
  ...
  if (/^\d+$/.test(v)) {
    ...
    /* Artikelcode kan in SKU staan met of zonder padding */
    return `sku:${v} OR sku:${stripped}`;
  }
  /* Heeft spaties → laat Shopify het zelf doen (default behavior is AND
     over multiple fields). Geen wildcard prefix want die voorkomt matches. */
  if (/\s/.test(v)) {
    return v;
  }
  /* SKU-stijl identifier */
  if (/[._\\/-]/.test(v)) {
    return `sku:${v} OR title:${v}`;
  }
  /* Pure letters of cijfers-met-letters → bare term, Shopify zoekt in alle
     velden (title, vendor, product_type, tags, sku, barcode). Geen wildcards. */
  return v;
}
