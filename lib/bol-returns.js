...
import { readShopifyVoorraadRows } from './srs-voorraad-store.js';
...
async function fetchAllReturns() {
  ...
  for (const handled of [false, true]) {
    for (let page = 1; page <= MAX_PAGES; page++) {
      let data;
      try { data = await bolGet('/returns', { query: { handled: String(handled) }, page, version: bolOrdersVersion() }); }
      catch (e) {
        if (page === 1 && !handled) throw e;
        partieleFout = e.message || `Returns-API faalde (handled=${handled}, pagina ${page})`; break;
      }
      const rows = data?.returns || [];
      if (!rows.length) break;
      for (const r of rows) out.push(r);
      if (page === MAX_PAGES && rows.length) afgekapt = true; /* pagina-limiet geraakt */
    }
  }
  return { rows: out, afgekapt, partieleFout };
}
...
export async function runBolReturns() {
  ...
  const [retData, cache, voorraadRows] = await Promise.all([
    fetchAllReturns(),
    readProductsCache().catch(() => null),
    readShopifyVoorraadRows().catch(() => [])
  ]);
  const returns = retData.rows;
  ...
  const result = {
    ...
    afgekapt: retData.afgekapt, /* true = pagina-limiet geraakt, analyse mogelijk incompleet */
    partieleFout: retData.partieleFout, /* niet-null = een vervolgpagina faalde */
