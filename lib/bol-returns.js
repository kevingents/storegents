...
export async function runBolReturns() {
  ...
  /* Content-gerelateerde retour-redenen → wijzen op een content-/maat-gat. */
  ...
  const producten = [...byEan.values()].map((e) => {
    ...
    /* Advies: heel veel retouren → stoppen; veel content-redenen → content
       verbeteren; onduidelijke reden → handmatig reviewen; anders stoppen. */
    ...
    return {
      ...
      topReden: nl(topCode), contentAantal, advies, prioriteit,
  ...
  const contentKandidaten = producten.filter((p) => p.advies === 'content')
    .map((p) => ({ ean: p.ean, titel: p.titel, contentAantal: p.contentAantal, aantalRetour: p.aantalRetour, topReden: p.topReden }))
