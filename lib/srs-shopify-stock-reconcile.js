  ...
  while (pages < MAX_PAGES) {
    ...
    for (const p of (conn.nodes || [])) {
      ...
      for (const v of (p.variants?.nodes || [])) {
        ...
        const cur = bySku.get(k) || { sku: clean(v.sku), inventory: 0, productTitle: clean(p.title), productId: idNum, status: clean(p.status), handle: clean(p.handle) };
        cur.inventory += inv;
        bySku.set(k, cur);
      }
    }
...
export async function runStockReconcile() {
  ...
  const [{ bySku: shop, pages }, voorraadRows, locatieRows] = await Promise.all([
  ...
  for (const [k, row] of srs.entries()) {
    const basis = srsBasis(row);
    const sh = shop.get(k);
    if (basis > 0) srsSkusMetVoorraad += 1;
    if (!sh) {
      /* In SRS (magazijn-voorraad) maar geen Shopify-variant met die SKU. */
      if (basis > 0) add('nietInShopify', { sku: row.sku, srsMagazijn: row.magazijn, srsTotaal: row.totaal });
      continue;
    }
    if (sh.inventory !== basis) {
      add('voorraadVerschil', {
        sku: row.sku, srsMagazijn: row.magazijn, srsTotaal: row.totaal,
        shopify: sh.inventory, verschil: sh.inventory - basis,
        productTitle: sh.productTitle, status: sh.status,
  ...
  const result = {
    ...
    pages,
    basis: warehouseConfigured ? 'magazijn' : 'totaal',
    warehouseConfigured,
    counts: {
