...
function classify(node) {
  ...
  return { status, inv, hasImage, onOnlineStore, categoryName, productType, inCollection, visible, hasCategory };
}
...
export async function runProductAudit() {
  ...
  const BUCKET_KEYS = ['verborgenMetVoorraad', 'draft', 'archived', 'geenSalesChannel', 'nietInCollectie', 'onlineZonderFoto'];
  ...
  for (const node of products) {
    ...
    if (!c.onOnlineStore) add('geenSalesChannel', node, c);
    /* 3. Niet aan een collectie toegewezen → verschijnt nergens in de navigatie. */
    if (!c.inCollection) add('nietInCollectie', node, c);
    /* 4. Online zichtbaar maar zonder foto — drijft de 'online zonder foto'-alert. */
    if (c.visible && !c.hasImage) add('onlineZonderFoto', node, c);
  }
