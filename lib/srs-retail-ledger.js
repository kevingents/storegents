...
const MAX_DAYS = 825; /* bewaar ~27 maanden per winkel — genoeg voor jaar-op-jaar
                         (volledig vorig jaar + huidig jaar, ook eind december). */
...
export function periodToRange(period, { from = '', to = '' } = {}) {
  ...
  switch (String(period || '').toLowerCase()) {
    ...
    case 'vorige-week': case 'vorige_week': {
      const mon = startOfWeek(today);
      return { from: addDays(mon, -7), to: addDays(mon, -1) };
    }
    case 'maand': return { from: startOfMonth(today), to: today };
    case 'vorige-maand': case 'vorige_maand': {
      const lastPrev = addDays(startOfMonth(today), -1);
