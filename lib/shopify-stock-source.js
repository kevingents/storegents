...
/* Shopify API 2024-04+ verving InventoryLevel.available door
   ...
   available-getal uit het nieuwe formaat (fallback op oude .available). */
function lvlAvailable(lvl) {
          ...
          nodes {
            ...
            inventoryItem {
              inventoryLevels(first: 30) {
                nodes { quantities(names: ["available"]) { name quantity } location { id name } }
              }
            }
          }
        }
    ...
    try {
      ...
      for (const v of variants) {
        for (const lvl of (v.inventoryItem?.inventoryLevels?.nodes || [])) {
          ...
          result[name] = (result[name] || 0) + lvlAvailable(lvl);
        }
      }
