...
/* Effectieve inkoop-config: portal-instelling (Instellingen-menu) wint, env is
   fallback. Zo kan de ConfigurationId in de tool worden ingevuld i.p.v. Vercel. */
async function resolvePoConfig() {
  let inkoop = {};
  try { inkoop = (await readPortalConfig()).inkoop || {}; } catch (_) { /* val terug op env */ }
  const configId = String(inkoop.srsConfigurationId || process.env.SRS_PO_CONFIGURATION_ID || '').trim();
  const orderType = String(inkoop.srsOrderType || process.env.SRS_PO_ORDER_TYPE || '').trim();
  const envFlag = String(process.env.SRS_PO_CREATE_ENABLED || '').trim().toLowerCase();
  const envDisabled = ['0', 'false', 'nee', 'no', 'off'].includes(envFlag);
  const cfgDisabled = inkoop.srsPushEnabled === false; /* expliciete kill-switch in de tool */
  const enabled = Boolean(configId) && !envDisabled && !cfgDisabled;
  return { configId, orderType, enabled };
}

const PO_DISABLED_MSG = 'Doorzetten naar SRS kan nog niet: er is geen ConfigurationId ingesteld. '
  + 'Vul die in via Instellingen → SRS-koppeling (of vraag SRS om de PurchaseOrders-ConfigurationId). '
  + 'De order is wél opgeslagen en kan gemaild worden.';

/** Async: is doorzetten naar SRS aan (ConfigurationId aanwezig + niet uitgezet)? */
export async function isPurchaseOrderPushEnabled() {
  return (await resolvePoConfig()).enabled;
}
...
function parsePoResponse(responseText) {
  const status = getLeafNodeText(responseText, 'Status') || getNodeText(responseText, 'Status'); /* Header/Status: 'completed' */
  return {
...
export async function createPurchaseOrderInSrs(order, { srsSupplierId } = {}) {
  const poCfg = await resolvePoConfig();
  if (!poCfg.enabled) {
    const e = new Error(PO_DISABLED_MSG);
  ...
  const orderType = poCfg.orderType;
  const inner = `    <tran:Create>
      ${buildHeaderXml(id, password)}
      <tran:Body>
        <tran:ConfigurationId>${xmlEscape(poCfg.configId)}</tran:ConfigurationId>
...
export async function cancelPurchaseOrderInSrs(srsOrderNr) {
  if (!(await resolvePoConfig()).enabled) {
    const e = new Error('Annuleren in SRS kan nog niet: ConfigurationId ontbreekt. Stel die in via Instellingen → SRS-koppeling.');
    e.code = 'PO_PUSH_DISABLED';
    throw e;
  }
...
export async function receivePurchaseOrderInSrs(srsOrderNr, items) {
  if (!(await resolvePoConfig()).enabled) {
    const e = new Error('Binnenmelden in SRS kan nog niet: ConfigurationId ontbreekt. Stel die in via Instellingen → SRS-koppeling.');
    e.code = 'PO_PUSH_DISABLED';
    throw e;
  }
  const nr = String(srsOrderNr || '').trim();
...
export async function receiveCorrectionInSrs(srsOrderNr, items) {
  if (!(await resolvePoConfig()).enabled) {
    const e = new Error('Correctie binnenmelding kan nog niet: ConfigurationId ontbreekt. Stel die in via Instellingen → SRS-koppeling.');
    e.code = 'PO_PUSH_DISABLED';
    throw e;
  }
  const nr = String(srsOrderNr || '').trim();
