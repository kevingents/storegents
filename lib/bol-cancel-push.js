...
import { bolPost, bolGet, bolOrdersVersion, bolOrdersWriteVersion } from './bol-client.js';
...
export async function cancelBolOrderViaApi(bolOrderId, { reasonCode, orderItemIds } = {}) {
  ...
  const resp = await bolPost(`/orders/${encodeURIComponent(id)}/cancellation`, body, { method: 'PUT', version: bolOrdersWriteVersion() });
  return {
    bolOrderId: id,
