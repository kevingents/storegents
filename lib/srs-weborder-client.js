...
export function buildReserveringWeborderXml(input) {
  ...
  const extendedAttributes = [
    ...
    extendedAttributeXml(sourceAttr, input.sellingBranchId),
    extendedAttributeXml('verzend_filiaal', input.fulfilmentBranchId),
    extendedAttributeXml('weborder_type', 'reservering'),
    extendedAttributeXml('reservering_id', input.reserveringId || ''),
    extendedAttributeXml('opmerking', input.note || 'Reservering via GENTS portaal')
  ].join('');
