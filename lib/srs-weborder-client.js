...
export async function srsApiLoginAs(userId) {
  ...
  /* De Bol-user kan een EIGEN wachtwoord hebben. Staat SRS_BOL_API_PASSWORD in
     de Vercel-env (secret), dan gebruiken we dat; anders hetzelfde wachtwoord als
     de hoofd-login. Wachtwoord komt nooit uit de request. */
  const pw = String(process.env.SRS_BOL_API_PASSWORD || password);
  return srsLoginWith(u, pw);
}
