...
/* ─────────────────────── Correcte bonus-antwoorden (admin) ─────────────────────── */

const CORRECT_BONUS_PATH = 'wk-poule/correct-bonus.json';

/**
 * Admin vult de juiste bonus-antwoorden in. Wordt door de scoring engine
 * gebruikt om bonus-punten toe te kennen.
 *
 * Structuur: { answers: { 'bq-champion': 'Brazilië', ... }, updatedAt, updatedBy }
 */
export async function getCorrectBonusAnswers() {
  const data = await readJsonBlob(CORRECT_BONUS_PATH, null);
  return {
    answers: (data && typeof data.answers === 'object') ? data.answers : {},
    updatedAt: data?.updatedAt || null,
    updatedBy: data?.updatedBy || null
  };
}

export async function setCorrectBonusAnswers(answers, updatedBy = 'admin') {
  const payload = {
    answers: answers && typeof answers === 'object' ? answers : {},
    updatedAt: new Date().toISOString(),
    updatedBy: String(updatedBy)
  };
  await writeJsonBlob(CORRECT_BONUS_PATH, payload);
  return payload;
}
