  ...
  birthday: {
    ...
    email: (c, store, _m, _ctx, cfg, theme) => ({
      ...
      html: emailShell({
        ...
        bodyHtml: `<p style="margin:0 0 14px">Van het hele team van GENTS${store ? ' ' + esc(store) : ''}: een hele fijne verjaardag! Bedankt dat je klant bij ons bent.</p>${voucherBox(cfg.voucherText, theme)}<p style="margin:0 0 14px">Kom je verjaardag bij ons vieren met iets moois?</p>${ctaButton('Shop je cadeau', '', theme)}`
      })
    })
  },
