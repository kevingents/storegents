...
export const AUTOMATIONS = {
  ...
  birthday: {
    ...
    email: (c, store, _m, _ctx, cfg, theme) => ({
      ...
      html: emailShell({
        store, firstName: clean(c.firstName), theme,
        bodyHtml: `<p style="margin:0 0 14px">Van het hele team van GENTS${store ? ' ' + esc(store) : ''}: een hele fijne verjaardag! Bedankt dat je klant bij ons bent.</p>${voucherBox(cfg.voucherText)}<p style="margin:0 0 14px">Kom je verjaardag bij ons vieren met iets moois?</p>${ctaButton('Shop je cadeau', '', theme)}`
      })
    })
  },
  ...
  winback: {
    ...
    email: (c, store, m, _ctx, cfg, theme) => ({
      ...
      html: emailShell({
        store, firstName: clean(c.firstName), theme,
        bodyHtml: `<p style="margin:0 0 14px">Het is alweer even geleden${m.favHg ? ` sinds je je laatste ${esc(m.favHg.toLowerCase())} bij ons koos` : ''}. We zouden je graag weer in stijl helpen.</p>${voucherBox(cfg.voucherText)}<p style="margin:0 0 14px">Er is genoeg nieuws — kom langs of kijk online.</p>${ctaButton('Bekijk wat nieuw is', '', theme)}`
      })
    })
  },
  ...
  replenishment: {
    ...
    email: (c, store, m, _ctx, cfg, theme) => {
      ...
      return {
        ...
        html: emailShell({
          store, firstName: clean(c.firstName), theme,
          bodyHtml: `<p style="margin:0 0 14px">Op is op — en jouw ${esc(list.toLowerCase())} zijn misschien wel toe aan vers. We hebben volop voorraad.</p>${ctaButton('Vul je voorraad aan', '', theme)}`
        })
      };
    }
  }
