...
import { esc, lc, clean, emailShell, ctaButton, voucherBox, buildPurchaseProfile, makeLookup, parseDate } from './automations-core.js';

const daysAgo = (ts) => (ts ? Math.round((Date.now() - ts) / 86400000) : null);

/* Bewerkbare mailtekst: als cfg.bodyText is ingevuld gebruiken we díe (met
   placeholders zoals {voornaam}/{winkel}), anders de vaste fallback-copy.
   De groet + afsluiting komen sowieso uit emailShell/het thema. */
function renderBody(cfg, theme, fallbackBodyHtml, ctaLabel, vars = {}) {
  const txt = clean(cfg.bodyText);
  if (!txt) return fallbackBodyHtml;
  let t = txt;
  for (const [k, v] of Object.entries(vars)) t = t.split('{' + k + '}').join(v == null ? '' : String(v));
  const paras = t.split(/\n{2,}/).map((p) => `<p style="margin:0 0 14px">${esc(p).replace(/\n/g, '<br>')}</p>`).join('');
  return `${paras}${voucherBox(cfg.voucherText, theme)}${ctaButton(ctaLabel, '', theme)}`;
}
const BODY_FIELD = { key: 'bodyText', label: 'Mailtekst (leeg = standaard). Variabelen: {voornaam} {winkel}', type: 'textarea' };
...
export const AUTOMATIONS = {
  ...
  birthday: {
    ...
    defaults: { subject: 'Een cadeautje voor jou — fijne verjaardag!', leadDays: 0, voucherText: '', bodyText: '', maxPerRun: 300 },
    context: async () => ({}),
    eligible: (c, _ctx, cfg) => {
    ...
    match: () => ({ kind: 'birthday' }),
    email: (c, store, _m, _ctx, cfg, theme) => {
      const fallback = `<p style="margin:0 0 14px">Van het hele team van GENTS${store ? ' ' + esc(store) : ''}: een hele fijne verjaardag! Bedankt dat je klant bij ons bent.</p>${voucherBox(cfg.voucherText, theme)}<p style="margin:0 0 14px">Kom je verjaardag bij ons vieren met iets moois?</p>${ctaButton('Shop je cadeau', '', theme)}`;
      return {
        subject: cfg.subject || 'Fijne verjaardag!',
        html: emailShell({ store, firstName: clean(c.firstName), theme, bodyHtml: renderBody(cfg, theme, fallback, 'Shop je cadeau', { voornaam: clean(c.firstName), winkel: store }) })
      };
    }
  },
  ...
  winback: {
    ...
    defaults: { subject: 'We missen je bij GENTS', minDays: 180, maxDays: 730, voucherText: '', bodyText: '', maxPerRun: 80 },
    ...
    email: (c, store, m, _ctx, cfg, theme) => {
      const fallback = `<p style="margin:0 0 14px">Het is alweer even geleden${m.favHg ? ` sinds je je laatste ${esc(m.favHg.toLowerCase())} bij ons koos` : ''}. We zouden je graag weer in stijl helpen.</p>${voucherBox(cfg.voucherText, theme)}<p style="margin:0 0 14px">Er is genoeg nieuws — kom langs of kijk online.</p>${ctaButton('Bekijk wat nieuw is', '', theme)}`;
      return {
        subject: cfg.subject || 'We missen je bij GENTS',
        html: emailShell({ store, firstName: clean(c.firstName), theme, bodyHtml: renderBody(cfg, theme, fallback, 'Bekijk wat nieuw is', { voornaam: clean(c.firstName), winkel: store, hoofdgroep: m.favHg || '', dagen: m.daysSince == null ? '' : m.daysSince }) })
      };
    }
  },
  ...
  replenishment: {
    ...
    defaults: { subject: 'Tijd om aan te vullen?', repeatGroups: 'Sokken, Ondergoed, Overhemden', cycleDays: 120, windowDays: 45, voucherText: '', bodyText: '', maxPerRun: 80 },
    context: async (cfg) => ({
    ...
    email: (c, store, m, _ctx, cfg, theme) => {
      ...
      const fallback = `<p style="margin:0 0 14px">Op is op — en jouw ${esc(list.toLowerCase())} zijn misschien wel toe aan vers. We hebben volop voorraad.</p>${voucherBox(cfg.voucherText, theme)}${ctaButton('Vul je voorraad aan', '', theme)}`;
      return {
        subject: cfg.subject || 'Tijd om aan te vullen?',
        html: emailShell({ store, firstName: clean(c.firstName), theme, bodyHtml: renderBody(cfg, theme, fallback, 'Vul je voorraad aan', { voornaam: clean(c.firstName), winkel: store, groepen: list }) })
      };
    }
  }
};
