...
export async function setReportBinding(reportKey, kpiKeys, actor = 'admin') {
  ...
  await mutateJsonBlob(CONFIG_PATH, (data0) => {
    ...
    return { ...data, reportBindings: bindings, updatedAt: new Date().toISOString(), updatedBy: actor };
...
export async function deleteReportBinding(reportKey, actor = 'admin') {
  ...
  await mutateJsonBlob(CONFIG_PATH, (data0) => {
    const data = (data0 && typeof data0 === 'object') ? data0 : {};
    const bindings = data.reportBindings || {};
    if (!(cleanKey in bindings)) { removed = false; return data; }
...
export async function updateKpiOverride(key, patch = {}, actor = 'admin') {
  ...
  await mutateJsonBlob(CONFIG_PATH, (data0) => {
    ...
    return { ...data, overrides, updatedAt: new Date().toISOString(), updatedBy: actor };
  }, { fallback: { overrides: {}, targets: {}, reportBindings: {} } });
  invalidateCache();
...
export async function resetKpiOverride(key, actor = 'admin') {
  ...
  await mutateJsonBlob(CONFIG_PATH, (data0) => {
    const data = (data0 && typeof data0 === 'object') ? data0 : {};
    const overrides = data.overrides || {};
    if (!(cleanKey in overrides)) { removed = false; return data; }
