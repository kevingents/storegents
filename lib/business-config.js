...
export const BUSINESS_CONFIG = Object.freeze({
   ...
   *   kind: 'retail' | 'warehouse' | 'showroom' | 'webshop' | 'admin'
   *   NB: alleen 'retail' telt als fysieke winkel (KPI per-store eval, targets).
   *       'webshop' / warehouse / showroom / admin = internal (geen winkel-target).
   */
  branches: Object.freeze({
    /* ── BRON-WAARHEID ──────────────────────────────────────────────
     * Deze branchId's zijn afgestemd op de SRS-filiaalnummers zoals ze in
     * de praktijk gebruikt worden (geverifieerd: 50=Antwerpen, 12=Hilversum,
     * 22=Zwolle, 23=Den Bosch). lib/branch-metrics.js gebruikt dezelfde
     * nummers voor het labelen van voorraad-data — beide MOETEN gelijk
     * blijven. Wijzig je hier een nummer, pas dan ook branch-metrics aan. */
    list: Object.freeze([
      /* ── Fysieke winkels (retail) — tellen mee voor winkel-targets ── */
      { store: 'GENTS Almere',       branchId: '1',   kind: 'retail'    },
      { store: 'GENTS Amersfoort',   branchId: '2',   kind: 'retail'    },
      { store: 'GENTS Amsterdam',    branchId: '15',  kind: 'retail'    },
      { store: 'GENTS Antwerpen',    branchId: '50',  kind: 'retail'    },
      { store: 'GENTS Arnhem',       branchId: '3',   kind: 'retail'    },
      { store: 'GENTS Breda',        branchId: '4',   kind: 'retail'    },
      { store: 'GENTS Delft',        branchId: '5',   kind: 'retail'    },
      { store: 'GENTS Den Bosch',    branchId: '23',  kind: 'retail'    },
      { store: 'GENTS Enschede',     branchId: '8',   kind: 'retail'    },
      { store: 'GENTS Groningen',    branchId: '10',  kind: 'retail'    },
      { store: 'GENTS Hilversum',    branchId: '12',  kind: 'retail'    },
      { store: 'GENTS Leiden',       branchId: '13',  kind: 'retail'    },
      { store: 'GENTS Maastricht',   branchId: '14',  kind: 'retail'    },
      { store: 'GENTS Nijmegen',     branchId: '16',  kind: 'retail'    },
      { store: 'GENTS Rotterdam',    branchId: '20',  kind: 'retail'    },
      ...
      { store: 'GENTS Zwolle',       branchId: '22',  kind: 'retail'    },

      /* ── Distributiemagazijnen (warehouse) ─────────────────────────
       * Alléén de centrale GENTS-DC's tellen mee als "magazijn" in de
       * uitlever-analyse (magazijn-vs-winkel). Overige magazijn-achtige
       * locaties (returns/extern/Suitconcern) staan als 'admin' hieronder. */
      { store: 'GENTS Magazijn',     branchId: '99',  kind: 'warehouse' },
      { store: 'Uitlevertafel',      branchId: '97',  kind: 'warehouse' },

      /* ── Webshop ───────────────────────────────────────────────── */
      { store: 'GENTS Webshop',      branchId: '90',  kind: 'webshop'   },

      /* ── Showroom ──────────────────────────────────────────────── */
      { store: 'GENTS Showroom',          branchId: '700', kind: 'showroom' },
      { store: 'Reserveringen Showroom',  branchId: '777', kind: 'showroom' },

      /* ── Interne / logistieke filialen (geen winkel, geen DC) ──────
       * kind: 'admin' = internal: telt niet als winkel-target en niet als
       * distributiemagazijn. Alleen voor naamgeving in voorraad-overzichten. */
      { store: 'GENTS Brandstores',           branchId: '900', kind: 'admin' },
      { store: 'Transfiliaal',                branchId: '100', kind: 'admin' },
      { store: 'Fotoshoot / Uitleen',         branchId: '701', kind: 'admin' },
      { store: 'Suitconcern',                 branchId: '702', kind: 'admin' },
      { store: 'B2B / Studenten',             branchId: '703', kind: 'admin' },
      { store: 'Suitconcern magazijn',        branchId: '704', kind: 'admin' },
      { store: 'Extern magazijn',             branchId: '705', kind: 'admin' },
      { store: 'Lost & Found',                branchId: '706', kind: 'admin' },
      { store: 'Klachten / Schade / Herstel', branchId: '707', kind: 'admin' },
      { store: 'Afkeur / Derving',            branchId: '708', kind: 'admin' },
      { store: 'Sample / Sale',               branchId: '709', kind: 'admin' },
      { store: 'Webretouren magazijn',        branchId: '98',  kind: 'admin' },
      { store: 'Tijdelijke retouren',         branchId: '502', kind: 'admin' }
    ])
    /* Wil je een nieuwe winkel toevoegen?
     *   1. Voeg toe aan de list hierboven met juiste branchId + kind
     *   2. Voeg hetzelfde id→naam paar toe aan lib/branch-metrics.js
     *   3. Deploy storegents — frontend leest direct via /api/branches
     *   4. Check dat snapshot-cron de nieuwe branchId picked up
     */
  }),
   ...
   * VOORRAAD-GEZONDHEID
   * ===========================================================
   * Welke filialen NIET getoond worden in het voorraad-gezondheid
   * dashboard + locatie-dekking. Niet-relevant voor winkel/DC-gezondheid:
   *   90  = GENTS Webshop (geen fysieke voorraadlocatie)
   *   704 = Suitconcern magazijn (apart merk/kanaal)
   * Voeg een branchId toe om 'm uit de gezondheids-views te verbergen
   * (de ruwe rijen blijven wél beschikbaar in de rapport-bouwer).
   */
  voorraad: Object.freeze({
    healthExcludeBranchIds: Object.freeze(['90', '704'])
  }),
...
/** Mag dit filiaal in voorraad-gezondheid / locatie-dekking getoond worden? */
export function isExcludedFromVoorraadHealth(branchId) {
  return BUSINESS_CONFIG.voorraad.healthExcludeBranchIds.includes(String(branchId || '').trim());
}
