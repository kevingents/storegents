...
export async function sendMail({ to, cc, bcc, subject, html, text, from, replyTo, headers, tags }) {
  ...
  /* Resend-tags voor engagement-tracking (open/click events worden in de
     webhook teruggekoppeld met deze tags). Format: [{name, value}]. */
  if (Array.isArray(tags) && tags.length) {
    payload.tags = tags
      .filter((t) => t && t.name && (t.value != null))
      .map((t) => ({ name: String(t.name).slice(0, 256), value: String(t.value).slice(0, 256) }));
  }
