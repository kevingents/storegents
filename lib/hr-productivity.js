...
/* Extra kantoor-/uitgesloten-afdelingen uit de tool-config (Instellingen-menu);
   per call herladen zodat aanpassen in het portaal direct effect heeft (geen env). */
let __EXTRA_OFFICE = new Set();
let __EXTRA_EXCLUDE = new Set();
async function ensureHrDeptConfig() {
  try {
    const cfg = await readPortalConfig();
    __EXTRA_OFFICE = new Set((cfg.hr?.officeDepts || []).map(low).filter(Boolean));
    __EXTRA_EXCLUDE = new Set((cfg.hr?.excludeDepts || []).map(low).filter(Boolean));
  } catch (_) { /* val terug op defaults/env */ }
}

function isOfficeDept(depName) {
  const n = low(depName);
  return OFFICE_DEPT_NAMES.has(n) || __EXTRA_OFFICE.has(n);
}

/* Afdelingen die volledig BUITEN de winkel-productiviteit vallen (niet als
   winkel, niet als kantoor). Suitconcern is een apart concept, geen retail-
   filiaal — die uren tellen dus niet mee. Uit te breiden via env
   WERKTIJDEN_EXCLUDE_DEPTS. */
const EXCLUDE_DEPT_NAMES = new Set([
  'suitconcern', 'suit concern', 'suitconcer', 'suit concer'
]);
for (const extra of String(process.env.WERKTIJDEN_EXCLUDE_DEPTS || '').split(',')) {
  const v = low(extra); if (v) EXCLUDE_DEPT_NAMES.add(v);
}
function isExcludedDept(depName) {
  const n = low(depName);
  if (EXCLUDE_DEPT_NAMES.has(n) || __EXTRA_EXCLUDE.has(n)) return true;
  return /suit\s*conce?rn?/.test(n); /* vangt 'Suitconcern', 'Suit Concern', 'Suitconcer' */
}
...
export async function computeHrProductivity({ from, to }) {
  await ensureHrDeptConfig(); /* office/exclude-afdelingen uit de tool-config laden */
  ...
  for (const [depId, hrs] of hoursByDep) {
    ...
    if (isExcludedDept(depName)) { excludedHours += hrs; continue; } /* bv. Suitconcern — telt niet mee */
  ...
  return {
    ...
    totals: {
      ...
      uitgeslotenUren: round(excludedHours, 1)
    },
