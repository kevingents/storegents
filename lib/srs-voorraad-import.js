...
export async function importVoorraad({ remotePath = '/' } = {}) {
  const { file, content } = await withSftp(async (sftp) => {
    const entries = await sftp.list(remotePath === '/' ? '/' : remotePath);
    const f = pickLatestFile(entries, 'voorraad');
    if (!f) return { file: null, content: null };
    const fullPath = (remotePath === '/' ? '' : remotePath) + '/' + f.name;
    const buf = await sftp.get(fullPath);
...
export async function importLocaties({ remotePath = '/' } = {}) {
  const { file, content } = await withSftp(async (sftp) => {
    const entries = await sftp.list(remotePath === '/' ? '/' : remotePath);
    const f = pickLatestFile(entries, 'locaties');
    if (!f) return { file: null, content: null };
    const fullPath = (remotePath === '/' ? '' : remotePath) + '/' + f.name;
    const buf = await sftp.get(fullPath);
    return { file: f, content: buf };
