...
import { writeVoorraadSnapshot, writeLocatiesSnapshot, writeSrsVoorraadRows } from './srs-voorraad-store.js';
import { listReserveringBranches } from './reserveringen-branch-mapping.js';
import { updateResAging, resAgingKey } from './reservering-aging-store.js';
...
async function findLatestRecursive(sftp, basePath, kind, maxDepth = 3) {
  ...
  async function walk(dir, depth, isRoot) {
    ...
    for (const e of entries) {
      ...
      if (e.type === 'd') {
        if (depth < maxDepth) await walk(full, depth + 1, false);
...
export async function importVoorraad({ remotePath = '/Dataexport' } = {}) {
  ...
  const meta = { sourceFile: file.name, fileTo: fileToDate(file.name), bron: 'srs' };
  const result = await writeVoorraadSnapshot(rows, meta);
  /* Bewaar de RAUWE SRS-voorraad (alle filialen, incl. interne branches zoals
     701 fotoshoot) in een apart blob dat de Shopify-snapshot niet overschrijft.
     Gebruikt door tools voor niet-Shopify-filialen (bv. "Bij fotograaf"). */
  try { await writeSrsVoorraadRows(rows, meta); } catch (e) { console.error('[srs-voorraad-import] srs-rows kopie faalde:', e.message); }

  /* Reservering-aging watermark: eerste-keer-gezien per (RES-branch, sku) zodat
     we straks "X dagen in reservering" kunnen tonen. Additief — fout mag de
     voorraad-import niet breken. Doet niets zolang de RES-filialen nog niet in
     de export zitten (lege key-set). */
  try {
    const resBranchIds = new Set(listReserveringBranches().map((b) => String(b.branchId)));
    const keys = [];
    for (const r of rows) {
      if (!resBranchIds.has(String(r.filiaalNummer))) continue;
      if (Number(r.voorraad || 0) <= 0) continue;
      keys.push(resAgingKey(r.filiaalNummer, r.sku));
    }
    await updateResAging(keys);
  } catch (e) {
    console.error('[srs-voorraad-import] reservering-aging faalde:', e.message);
  }
