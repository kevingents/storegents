 ...
 */
/* ─── Uitwisselingen-rapport fetcher ───────────────────────────────
   Aggregeert per winkel: ontvangen / afgerond / open >1 week binnen periode.
   Bron: blob srs-exchanges/open-exchange-state.json (gebouwd door SRS-cron). */
...
async function readExchangeStateBlob() {
  try {
    ...
    console.error('[fetchExchangesReport] readStateBlob:', err);
    return { exchanges: {} };
  }
}

async function fetchExchangesReport(params = {}) {
  const { fromStr, toStr } = dateRangeFromParams(params);
  const storeFilter = String(params.store || '').trim();

  const state = await readExchangeStateBlob();
  ...
  const init = (store) => {
    if (!byStore.has(store)) {
      byStore.set(store, { store, received: 0, completed: 0, openOverOneWeek: 0 });
    }
  ...
  for (const id of Object.keys(exchanges)) {
    ...
    const store = String(ex.storeName || getStoreNameByBranchId(ex.branchId) || 'Onbekend').trim();
    if (storeFilter && store !== storeFilter) continue;
    const row = init(store);

    /* Received in periode (createdAt / firstDetectedAt) */
    const createdIso = String(ex.firstDetectedAt || ex.createdAt || '').slice(0, 10);
    if (createdIso && (!fromStr || createdIso >= fromStr) && (!toStr || createdIso <= toStr)) {
      row.received += 1;
    }

    /* Completed in periode (closedAt) */
    const closedIso = String(ex.closedAt || '').slice(0, 10);
    if (closedIso && (!fromStr || closedIso >= fromStr) && (!toStr || closedIso <= toStr)) {
      row.completed += 1;
    }

    /* Currently open >1 week (live, niet binnen periode) */
    if (!ex.closedAt) {
      const firstMs = new Date(ex.firstDetectedAt || ex.createdAt || 0).getTime();
      if (firstMs && (todayMs - firstMs) >= 7 * 86400_000) row.openOverOneWeek += 1;
    }
  }

  const rows = Array.from(byStore.values()).sort((a, b) => a.store.localeCompare(b.store, 'nl'));
  ...
  return {
    title: 'Uitwisselingen rapport',
    subtitle: `${rows.length} winkels${storeFilter ? ` · ${storeFilter}` : ''}${fromStr || toStr ? ` · ${fromStr || '—'} t/m ${toStr || '—'}` : ''}`,
    columns: [
      { key: 'store', label: 'Winkel' },
      { key: 'received', label: 'Ontvangen' },
      { key: 'completed', label: 'Afgerond' },
      { key: 'openOverOneWeek', label: 'Open > 1 week' }
    ],
    rows,
    totals: {
      'Totaal winkels': rows.length,
      'Totaal ontvangen': totals.received,
      'Totaal afgerond': totals.completed,
      'Open > 1 week': totals.openOverOneWeek
    },
    filters: { from: fromStr, to: toStr, store: storeFilter || null }
  };
}
...
export const REPORT_FETCHERS = {
  ...
  'uitwisselingen-report': fetchExchangesReport
};
