...
import { bolPost, isBolConfigured, bolOffersVersion } from './bol-client.js';
...
export async function runBolPriceSync({ dryRun = true, onlyChanged = true, force = false } = {}) {
  ...
  for (const it of plan.items) {
    ...
    try {
      await bolPost(`/offers/${it.offerId}/price`, { pricing: { bundlePrices: [{ quantity: 1, unitPrice: it.pariteitPrijs }] } }, { method: 'PUT', version: bolOffersVersion() });
      gepusht += 1;
