...
import { parseDhlInvoiceRoutes } from './dhl-invoice-detail-parser.js';
...
/** Lees PDF-tekst uit bytes met unpdf (serverless-vriendelijk). Per-pagina
 *  samengevoegd: de samenvatting-parser normaliseert toch alle witruimte, en de
 *  route-parser heeft de per-pagina-volgorde nodig voor de zending-regels. */
 ...
 */
/**
 * Parse + verrijk een DHL-factuur-PDF ZONDER op te slaan. Retourneert het
 * verrijkte factuur-object (incl. routeBreakdown). Gebruikt door bulk-inlezen
 * (parse los, daarna alles in één keer opslaan via saveInvoices).
 */
export async function parseDhlInvoice(bytes, meta = {}) {
  let text = '';
  try {
  ...
  /* Route-verdeling op basis van de échte van-/naar-postcodes per zending
     (magazijn→winkel, winkel→winkel, naar consument). Best-effort. */
  let routeBreakdown = null;
  try {
    routeBreakdown = parseDhlInvoiceRoutes(text, parsed.services, parsed.totalShipments);
  } catch (e) {
    console.warn('[dhl-invoice-ingest] route-parse mislukt:', e.message);
  }

  const enriched = enrich({ ...parsed, source: meta.source || 'upload', addedBy: meta.addedBy || 'portaal' });
  if (routeBreakdown) enriched.routeBreakdown = routeBreakdown;
  return enriched;
}

/** Parse + bewaar één DHL-factuur (email-inbound + losse upload). */
export async function ingestDhlInvoice(bytes, meta = {}) {
  return saveInvoice(await parseDhlInvoice(bytes, meta));
}
