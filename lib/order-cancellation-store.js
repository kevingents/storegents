...
function normalizeCancellation(input = {}) {
  ...
  const orderNr = clean(input.orderNr || input.orderName).replace(/^#/, '');
  ...
  return {
    ...
    orderNr,
    shopifyOrderNr: clean(input.shopifyOrderNr),
    weborderNr: clean(input.weborderNr),
    srsOrderNr: clean(input.srsOrderNr),
    customerOrderNr: clean(input.customerOrderNr),
    type,
    ...
    items: items.map((item) => ({
      ...
      articleId: clean(item.articleId || item.artikelId),
      sku: clean(item.sku || item.barcode),
      barcode: clean(item.barcode || item.sku),
      title: clean(item.title || item.productName || item.sku || item.barcode),
      ...
      srsStatus: clean(item.srsStatus || item.status),
      branchId: clean(item.branchId || item.fulfilmentBranchId || item.fulfillmentBranchId),
      currentBranch: clean(item.currentBranch || item.huidigFiliaal),
      originBranch: clean(item.originBranch || item.herkomstFiliaal),
      lastResponsibleStore: clean(item.lastResponsibleStore || input.store),
      srsUnavailableStore: clean(item.srsUnavailableStore),
      shopifyFulfillmentLocation: clean(item.shopifyFulfillmentLocation),
      shopifyLineItemId: clean(item.shopifyLineItemId)
    })),
    status: input.status || 'requested',
    srsStatus: input.srsStatus || 'pending',
    srsCancelStatus: input.srsCancelStatus || 'pending',
    refundStatus: input.refundStatus || 'pending',
    mailStatus: input.mailStatus || 'pending',
    stockReturnStatus: input.stockReturnStatus || input.srsResult?.stockReturnStatus || 'skipped_no_stock_return',
    error: input.error || '',
    source: input.source || '',
    srsSourceStatus: clean(input.srsSourceStatus || input.srsStatus),
    stockSnapshot: input.stockSnapshot || input.srsResult?.stockSnapshot || null,
    lostFoundCheck: input.lostFoundCheck || null,
    shopifyContext: input.shopifyContext || null,
    srsResult: input.srsResult || null,
    srsCancelResult: input.srsCancelResult || null,
...
export function buildIdempotencyKey({ store, orderNr, type, items = [] }) {
  const itemKey = (items || [])
    .map((item) => [item.fulfillmentId, item.orderLineNr, item.sku || item.barcode, item.srsStatus || item.status].filter(Boolean).join(':'))
    .filter(Boolean)
    .sort()
    .join('|');
  return [clean(store).toLowerCase(), clean(orderNr).replace(/^#/, ''), type || 'partial', itemKey || 'all'].join('::');
}

async function readBlobJson({ strict = false } = {}) {
  /* strict:true betekent: bij lees-/parse-fout NIET silent een lege state
     teruggeven (zoals dat was) maar throwen. Verplicht voor write-paden —
     anders wipet één transient blob-fout de complete annuleringen-historie
     wanneer caller daarna saveOrderCancellations([new]) doet. */
  try {
    const result = await list({ prefix: STORE_KEY, limit: 1 });
    const blob = (result.blobs || []).find((item) => item.pathname === STORE_KEY) || result.blobs?.[0];
    if (!blob?.url) return { cancellations: [] }; /* legitiem leeg = eerste run */

    const response = await fetch(blob.url, { cache: 'no-store' });
    if (!response.ok) {
      if (strict) throw new Error(`order-cancellations blob-read mislukt: HTTP ${response.status}`);
      return { cancellations: [] };
    }
    const text = await response.text();
    const parsed = safeJson(text, null);
    if (parsed == null) {
      if (strict) throw new Error('order-cancellations blob JSON-parse mislukt');
      return { cancellations: [] };
    }
    return parsed;
  } catch (error) {
    if (strict) throw error;
    console.error('[order-cancellation-store] read fout (fail-soft):', error.message);
    return { cancellations: [] };
  }
}
...
async function getOrderCancellationsStrict() {
  const data = await readBlobJson({ strict: true });
  return Array.isArray(data.cancellations) ? data.cancellations.map(normalizeCancellation) : [];
}

export async function addOrderCancellation(input) {
  const current = await getOrderCancellationsStrict(); /* throw i.p.v. wipe bij read-fout */
...
export async function updateOrderCancellation(id, patch = {}) {
  const current = await getOrderCancellationsStrict();
...
export function monthKeyFromCancellation(item = {}) {
  ...
  return clean(item.month).slice(0, 7);
}

export function filterCancellationsByMonth(cancellations, month) {
  const key = month || monthKeyFromDate();
  return (cancellations || []).filter((item) => monthKeyFromCancellation(item) === key);
}
...
export function cancellationLineRows(cancellations) {
  ...
  for (const item of cancellations || []) {
    ...
    items.forEach((line, index) => {
      ...
      const lineStatus = clean(line.srsStatus || line.status || item.srsSourceStatus || item.srsStatus);
      ...
      rows.push({
        ...
        title: clean(line.title || line.productName),
        ...
        originBranch: clean(line.originBranch || line.herkomstFiliaal || item.store),
        lastResponsibleStore: clean(line.lastResponsibleStore || item.store || 'Onbekend'),
        srsUnavailableStore: clean(line.srsUnavailableStore),
        branchId: clean(line.branchId),
        quantity,
        amount,
        ...
        srsSourceStatus: item.srsSourceStatus || '',
        srsCancelStatus: item.srsCancelStatus || 'pending',
        refundStatus: item.refundStatus || 'pending',
        mailStatus: item.mailStatus || 'pending',
        stockReturnStatus: item.stockReturnStatus || 'skipped_no_stock_return',
        error: item.error || '',
        source: item.source || '',
        stockSnapshot: item.stockSnapshot || null,
        lostFoundCheck: item.lostFoundCheck || null,
        shopifyContext: item.shopifyContext || null,
