 ...
 *
 * @returns {Promise<Array<{ store, branchId, totalWins, hoofdprijs, klantKoning, loyaltyKampioen, crossChannelHeld, dataMeester, months }>>}
 */
export async function readAllTimeLeaderboard() {
  ...
  for (const month of index.months || []) {
    ...
    bump(winner.subWinners?.loyalty?.store, 'loyaltyKampioen', month, winner.subWinners?.loyalty?.branchId);
  ...
  return Array.from(stats.values()).sort((a, b) => {
    ...
    const subWinsA = a.klantKoning + a.loyaltyKampioen + a.crossChannelHeld + a.dataMeester;
    const subWinsB = b.klantKoning + b.loyaltyKampioen + b.crossChannelHeld + b.dataMeester;
