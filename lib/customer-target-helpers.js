...
/* De KPI-store levert per winkel de samengevoegde targets (KPI-waarde wint,
   oude customer-targets als fallback), zodat targets die via het KPI-beheer
   OF via de oude invoer gezet zijn allebei in het rapport verschijnen.
   Mapping: customers_new→inschrijvingen, customers_with_bon→metBon,
   customers_with_email→metEmail. */
import { getTargetsForStores } from './kpi-targets-store.js';
...
export async function getTargetsForPeriod(stores, dateFrom, dateTo) {
  ...
  /* Per maand de samengevoegde KPI-targets ophalen (KPI-store mergt zelf de
     nieuwe kpi-config.json met de oude customer-targets als fallback). */
  const byMonth = {};
  for (const { year, month } of months) {
    try { byMonth[monthKey(year, month)] = await getTargetsForStores(year, month, stores); }
    catch (_) { byMonth[monthKey(year, month)] = {}; }
  }
  ...
  for (const store of stores) {
    ...
    for (const { year, month, daysOverlap, daysInMonth: dim } of months) {
      if (daysOverlap <= 0 || dim <= 0) continue;
      const row = byMonth[monthKey(year, month)]?.[store] || {};
      const ratio = daysOverlap / dim;
      result.inschrijvingen += (Number(row.customers_new) || 0) * ratio;
      result.metBon += (Number(row.customers_with_bon) || 0) * ratio;
      result.metEmail += (Number(row.customers_with_email) || 0) * ratio;
    }
    result.inschrijvingen = Math.round(result.inschrijvingen);
    result.metBon = Math.round(result.metBon);
    result.metEmail = Math.round(result.metEmail);
