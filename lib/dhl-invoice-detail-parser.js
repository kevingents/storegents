...
/* Magazijn-postcode(s) — bepaalt 'magazijn→winkel' vs 'winkel→winkel'. Standaard
   1101AJ (Lemelerbergweg 15, Amsterdam); overschrijfbaar via env als het magazijn
   verhuist of erbij komt (komma-gescheiden), zodat dit niet hardcoded vastzit. */
const MAGAZIJN_POSTCODES = new Set(
  String(process.env.DHL_MAGAZIJN_POSTCODES || '1101AJ')
    .split(',')
    .map((s) => s.trim().toUpperCase().replace(/\s+/g, ''))
    .filter(Boolean)
);

/* Vaste lijst GENTS-postcodes (magazijn + alle winkels/HQ), uit de SendCloud-
   afzenderadressen. Nodig zodat een winkel die in een factuur alléén ONTVANGT
   (en dus niet in de afzender-kolom staat) tóch als GENTS-locatie herkend wordt
   → GENTS→GENTS telt altijd als intern, nooit als consument. We unionen dit met
   de uit-de-factuur-afgeleide afzender-postcodes (vangt postcode-varianten).
   Overschrijfbaar via env DHL_GENTS_POSTCODES (komma-gescheiden). */
const DEFAULT_GENTS_POSTCODES =
  '1101AJ,8011LS,1315HC,3811AC,1073LN,6811EH,2611GK,7511HC,2312DN,6511HG,2711HP,3511JM,5038AX,6211GT,1214AW,9712CM,4811WT,3012JG,5211ET';
const GENTS_POSTCODES = new Set(
  String(process.env.DHL_GENTS_POSTCODES || DEFAULT_GENTS_POSTCODES)
    .split(',')
    .map((s) => s.trim().toUpperCase().replace(/\s+/g, ''))
    .filter(Boolean)
);
...
function parseShipments(text) {
  ...
  /* Blok-start: <vrachtbrief/tracking 8-12 cijfers> <klantnr 4-9 cijfers> <dd/mm/jjjj>.
     8 cijfers toegestaan want Europlus-vrachtbrieven zijn 8-cijferig. */
  const parts = full.split(/(?=\b\d{8,12} \d{4,9} \d{2}\/\d{2}\/\d{4})/);
  const ships = [];
  for (const blk of parts) {
    if (!/^\d{8,12} \d{4,9} \d{2}\/\d{2}\/\d{4}/.test(blk)) continue;
...
export function parseDhlInvoiceRoutes(detailText, services = [], totalShipments = 0) {
  ...
  /* GENTS-locaties = vaste lijst (magazijn + winkels) ∪ alle afzender-postcodes
     uit dit detail (vangt postcode-varianten + nieuwe locaties). Zo telt een
     zending NAAR een GENTS-winkel altijd als intern, ook als die winkel in deze
     factuur alleen ontvangt. */
  const gentsPostcodes = new Set(ships.map((s) => s.from).filter(Boolean));
  GENTS_POSTCODES.forEach((p) => gentsPostcodes.add(p));
