...
function sameSrsLine(srsRow = {}, row = {}) {
  const fulfillmentId = clean(row.fulfillmentId);
  const orderLineNr = clean(row.orderLineNr);
  const sku = clean(row.sku || row.barcode);
  const srsFulfillmentId = clean(srsRow.fulfillmentId || srsRow.id);
  const srsOrderLineNr = clean(srsRow.orderLineNr);
  const srsSku = clean(srsRow.sku || srsRow.barcode);

  if (fulfillmentId && srsFulfillmentId && fulfillmentId === srsFulfillmentId) return true;
  if (orderLineNr && srsOrderLineNr && orderLineNr === srsOrderLineNr && (!sku || !srsSku || sku === srsSku)) return true;
  if (sku && srsSku && sku === srsSku && (!orderLineNr || !srsOrderLineNr || orderLineNr === srsOrderLineNr)) return true;
  return false;
}

async function getLiveSrsLine(row = {}) {
  try {
    const result = await getFulfillments({ orderNr: row.srsOrderNr || row.orderNr });
    const fulfillments = result.fulfillments || [];
    const matching = fulfillments.filter((item) => sameSrsLine(item, row));
    return {
      success: true,
      matching,
      statuses: matching.map((item) => item.status).filter(Boolean),
      cancelled: matching.some((item) => isSrsCancelledStatus(item.status)),
      unavailable: matching.some((item) => isSrsUnavailableStatus(item.status)),
      found: matching.length > 0
    };
  } catch (error) {
    return { success: false, error: error.message || 'SRS live controle mislukt.', matching: [], statuses: [], cancelled: false, unavailable: false, found: false };
  }
}
...
async function cancel(row) {
  ...
  const liveBefore = await getLiveSrsLine(row);
  if (liveBefore.success && liveBefore.found && liveBefore.cancelled && !liveBefore.unavailable) {
    return { success: true, skipped: true, status: 'already_cancelled_live_srs', message: 'SRS regel was al geannuleerd.', liveBefore };
  }
  if (liveBefore.success && liveBefore.found && !liveBefore.unavailable) {
    return { success: true, skipped: true, status: 'already_resolved_live_srs', message: 'SRS regel stond niet meer op niet-leverbaar.', liveBefore };
  }
  ...
  if (isSrsCancelDoneResult(result)) return { ...result, success: true, status: 'already_cancelled_or_verified' };
  return result;
}
