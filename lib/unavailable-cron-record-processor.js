import { addOrderCancellation, updateOrderCancellation } from './order-cancellation-store.js';
...
export async function processSyncedUnavailableRecord(record, { employeeName = 'Automatische niet-leverbaar cron' } = {}) {
  ...
  try {
    ...
    await log(row, { type: patch.refundStatus === 'already_refunded' ? 'shopify_already_refunded' : 'shopify_refund_created', success: true, processedBy: employeeName, refundStatus: patch.refundStatus, amount: patch.amount, message: patch.refundStatus === 'already_refunded' ? 'Shopify was al terugbetaald.' : 'Shopify terugbetaling verwerkt.', result: results.refund });
    ...
    try {
      ...
      if (!results.srsCancel?.success) patch.error = results.srsCancel?.messages?.join(' | ') || results.srsCancel?.message || 'SRS cancel is niet bevestigd.';
    } catch (error) {
    ...
    await log(row, { type: patch.srsCancelStatus === 'cancelled_in_srs' ? 'srs_cancel_success' : 'srs_cancel_failed', success: patch.srsCancelStatus === 'cancelled_in_srs', processedBy: employeeName, refundStatus: patch.refundStatus, srsCancelStatus: patch.srsCancelStatus, message: patch.error || 'SRS orderregel geannuleerd of stond al geannuleerd.', result: results.srsCancel });
    ...
    const updated = await updateOrderCancellation(stored.id, patch);
    return { success: done, partial: !done, id: row.id, cancellation: updated, results, message: done ? 'Cronregel volledig verwerkt.' : 'Cronregel gedeeltelijk verwerkt.' };
  } catch (error) {
    patch.status = 'open';
    patch.error = error.message || 'Verwerking mislukt.';
    const updated = await updateOrderCancellation(stored.id, patch);
    await log(row, { type: 'process_failed', success: false, processedBy: employeeName, refundStatus: patch.refundStatus || row.refundStatus, srsCancelStatus: patch.srsCancelStatus || row.srsCancelStatus, message: patch.error, result: { error: patch.error } });
    return { success: false, partial: false, id: row.id, cancellation: updated, results, error: patch.error, message: patch.error };
  }
}
