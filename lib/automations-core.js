...
export const EMAIL_THEME_DEFAULTS = {
  ...
  buttonRadius: 8,         // hoekafronding knop (px)
...
export function headFont(t) { const th = theme(t); return th.headingFont === 'sans' ? FONT : HEAD_FONT; }

export function ctaButton(label, url, t) {
  const th = theme(t);
  const rad = Number.isFinite(Number(th.buttonRadius)) ? Number(th.buttonRadius) : 8;
  const outline = th.buttonStyle === 'outline';
  const bg = outline ? '#ffffff' : th.buttonBg;
  const color = outline ? th.buttonBg : '#ffffff';
  const border = outline ? `2px solid ${esc(th.buttonBg)}` : 'none';
  /* Gecentreerde, bullet-proof knop. */
  return `<table role="presentation" cellpadding="0" cellspacing="0" style="margin:6px 0"><tr><td align="center" bgcolor="${esc(bg)}" style="border-radius:${rad}px">
    <a href="${esc(url || th.shopUrl)}" style="display:inline-block;background:${esc(bg)};color:${esc(color)};border:${border};text-decoration:none;font-family:${FONT};font-size:14px;font-weight:600;letter-spacing:.3px;padding:13px 26px;border-radius:${rad}px">${esc(label || th.buttonLabel)}</a>
  </td></tr></table>`;
}
...
export function emailShell({ store = '', firstName = '', bodyHtml = '', footer = '', preheader = '', greeting = true, theme: t } = {}) {
  const th = theme(t);
  const HEAD = th.headingFont === 'sans' ? FONT : HEAD_FONT;
  const align = th.contentAlign === 'center' ? 'center' : 'left';
  const hiLine = greeting
    ? `<p style="margin:0 0 16px;font-size:17px;font-weight:600">${firstName ? `${esc(th.greetingPrefix)} ${esc(firstName)},` : `${esc(th.greetingPrefix)},`}</p>`
    : '';
  const brand = th.logoUrl
    ? `<img src="${esc(th.logoUrl)}" alt="${esc(th.brandName)}" height="30" style="display:block;margin:0 auto;border:0">`
    : `<div style="font-family:${HEAD};font-size:24px;font-weight:700;letter-spacing:3px;text-transform:uppercase">${esc(th.brandName)}</div>`;
  ...
  <body style="margin:0;padding:0;background:${esc(th.pageBg)};-webkit-font-smoothing:antialiased">
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${esc(th.pageBg)}"><tr><td align="center" style="padding:28px 14px">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:600px;max-width:100%;background:#ffffff;border-radius:14px;overflow:hidden;box-shadow:0 1px 4px rgba(0,0,0,.06)">
        ...
        <tr><td style="padding:34px 36px 26px;font-family:${FONT};color:${esc(th.textColor)};font-size:15px;line-height:1.62;text-align:${align}">
          ${hiLine}
          ${bodyHtml}
          ${greeting ? `<p style="margin:22px 0 0;color:#777;font-size:14px">${esc(th.signoff)}${store ? ' in ' + esc(store) : ''},<br><strong style="color:${esc(th.textColor)}">${esc(th.brandName)}${store ? ' ' + esc(store) : ''}</strong></p>` : ''}
        </td></tr>
...
export function productCard(p, t) {
  ...
  return `
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 16px;border:1px solid #ececec;border-radius:10px;overflow:hidden">
      <tr>
        <td width="132" valign="top" style="background:#f6f7f9">${p.image ? `<img src="${esc(p.image)}" width="132" alt="" style="display:block;border:0;width:132px">` : '<div style="width:132px;height:132px;background:repeating-linear-gradient(135deg,#f1f2f4,#f1f2f4 10px,#ececef 10px,#ececef 20px)"></div>'}</td>
        <td valign="top" style="padding:14px 16px;font-family:${FONT};color:${esc(th.textColor)}">
          <div style="font-family:${HEAD};font-size:17px;font-weight:700;margin-bottom:4px">${esc(p.title || 'Item')}</div>
