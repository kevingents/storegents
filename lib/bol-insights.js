...
const VERZEND_SURCHARGE = Number(process.env.BOL_SHIPPING_SURCHARGE || 0); /* verzendkosten bovenop webshop-prijs voor pariteit */
const PARITEIT_TOL = Number(process.env.BOL_PARITY_TOLERANCE || 0.02); /* € speling voor "gelijk" */
...
function pariteit(webshopPrijs) { return webshopPrijs != null ? r2(webshopPrijs + VERZEND_SURCHARGE) : null; }
...
export async function runBolInsights({ topN = 60 } = {}) {
  ...
  for (const [ean, o] of Object.entries(offerMap)) {
    ...
    const par = pariteit(web);
    if (bolPrijs == null || par == null) continue;
    const verschil = r2(bolPrijs - par);
    if (Math.abs(verschil) <= PARITEIT_TOL) continue;
  ...
  const result = {
    ...
    pariteit: {
      verzendkosten: VERZEND_SURCHARGE,
...
export async function getRepriceAdvice(ean) {
  ...
  const pariteitPrijs = pariteit(webshopPrijs);
  ...
  else if (bolPrijs == null) advies = `Zet de bol-prijs op €${pariteitPrijs.toFixed(2)} (= webshop €${(webshopPrijs || 0).toFixed(2)}${VERZEND_SURCHARGE ? ` + €${VERZEND_SURCHARGE.toFixed(2)} verzending` : ''}).`;
  else if (Math.abs(bolPrijs - pariteitPrijs) <= PARITEIT_TOL) advies = `Pariteit klopt — bol €${bolPrijs.toFixed(2)} ≈ webshop €${pariteitPrijs.toFixed(2)}.`;
  ...
  return { ean: e, webshopPrijs, verzendkosten: VERZEND_SURCHARGE, pariteitPrijs, onzePrijs: bolPrijs, buyBoxPrijs, adviesPrijs: pariteitPrijs, advies, aantalOffers };
}
