...
export async function runBolInsights({ topN = 60 } = {}) {
  ...
  const buyboxResults = await mapPool(buyboxKandidaten, 6, async (it) => {
    ...
    return { ean: it.ean, titel: it.titel, offerId: it.offerId, buyboxPct: bb == null ? null : Math.round(bb), bezoeken: visits == null ? null : Math.round(visits), conversiePct: conversie == null ? null : Math.round(conversie * 10) / 10 };
  });
  ...
  const bronnen = {
    omzet: {
      ...
      note: sales.error ? `Orders-API gaf een fout: ${sales.error}` : (sales.orders ? null : 'Geen orders ontvangen — kan kloppen (geen verkoop) of de Orders-API leverde niets.')
    },
    buyBox: {
