...
import { bolGet, isBolConfigured, bolOrdersVersion } from './bol-client.js';
...
/* Bounded-concurrency map: voert fn parallel uit met max `limit` tegelijk. Nodig
   omdat de Insights-API per offer apart bevraagd wordt — sequentieel zijn dat
   100+ calls (→ timeout). bol heeft rate-limits, dus niet álles tegelijk. */
...
async function fetchSales({ maxPages = 25 } = {}) {
  const byEan = new Map();
  const byDay = new Map(); /* 'YYYY-MM-DD' → omzet */
  let stuks = 0, omzet = 0, orders = 0, error = null;
  for (let page = 1; page <= maxPages; page++) {
    let resp;
    try { resp = await bolGet('/orders', { query: { status: 'ALL' }, page, version: bolOrdersVersion() }); }
    catch (e) { if (page === 1) error = e.message || 'Orders-API faalde'; break; }
    const list = resp?.orders || [];
    if (!list.length) break;
    orders += list.length;
    for (const o of list) {
      const dag = clean(o.orderPlacedDateTime || o.dateTimeOrderPlaced || o.orderPlacedDate).slice(0, 10);
      for (const it of (o.orderItems || o.items || [])) {
        ...
        byEan.set(ean, cur);
        stuks += q; omzet += q * p;
        if (dag) byDay.set(dag, (byDay.get(dag) || 0) + q * p);
      }
    }
    if (list.length < 50) break;
  }
  return { byEan, byDay, stuks, omzet, orders };
}
...
export async function runBolInsights({ topN = 60 } = {}) {
  ...
  const buyboxKandidaten = buyboxBron.slice(0, topN).filter((it) => it.offerId);
  /* Parallel met max 6 tegelijk i.p.v. ~120 calls sequentieel (timeout-fix). */
  const buyboxResults = await mapPool(buyboxKandidaten, 6, async (it) => {
    const [bb, visits] = await Promise.all([
      offerMetric(it.offerId, 'BUY_BOX_PERCENTAGE'),
      offerMetric(it.offerId, 'PRODUCT_VISITS')
    ]);
    if (bb == null && visits == null) return null;
    /* Conversie NIET berekenen uit (totale verkoop / 7-daagse bezoeken): teller en
       noemer hebben verschillende perioden → misleidend (kan >100%). Bewust
       weggelaten tot er een verkoop-metric over dezelfde window beschikbaar is. */
    return { ean: it.ean, titel: it.titel, offerId: it.offerId, buyboxPct: bb == null ? null : Math.round(bb), bezoeken: visits == null ? null : Math.round(visits), conversiePct: null };
  });
  const buybox = buyboxResults.filter(Boolean);
  const buyboxVerliezers = buybox.filter((b) => b.buyboxPct != null && b.buyboxPct < BUYBOX_DREMPEL).sort((a, b) => (a.buyboxPct || 0) - (b.buyboxPct || 0));
  const buyboxMet = buybox.filter((b) => b.buyboxPct != null);
  const buyBoxRate = buyboxMet.length ? Math.round(buyboxMet.reduce((n, b) => n + b.buyboxPct, 0) / buyboxMet.length) : null;

  /* Omzet-trend (laatste 14 dagen) + delta laatste 7 vs daarvoor. */
  const dagen = [];
  const vandaag = new Date();
  for (let i = 13; i >= 0; i--) {
    const dt = new Date(vandaag.getTime() - i * 86400000);
    const key = dt.toISOString().slice(0, 10);
    dagen.push({ datum: key, omzet: r2(sales.byDay.get(key) || 0) });
  }
  const laatste7 = dagen.slice(7).reduce((n, d) => n + d.omzet, 0);
  const vorige7 = dagen.slice(0, 7).reduce((n, d) => n + d.omzet, 0);
  const omzetDeltaPct = vorige7 > 0 ? r2(((laatste7 - vorige7) / vorige7) * 100) : null;

  /* Potentiële omzet-impact: prijs-gat van de te-goedkope offers (onder pariteit). */
  const potentieleImpact = r2(pariteitAfwijkingen.filter((a) => a.verschil < 0).reduce((n, a) => n + Math.abs(a.verschil), 0));

  /* Databron-diagnose: laat per bron zien of er écht data uit bol kwam, zodat
     omzet/buy-box-cijfers niet onterecht als betrouwbaar worden gelezen. */
  const offerMapCount = Object.keys(offerMap).length;
  const bronnen = {
    omzet: {
      ok: !sales.error && sales.orders > 0,
      orders: sales.orders,
      error: sales.error || null,
      note: sales.error ? `Orders-API gaf een fout: ${sales.error}` : (sales.orders ? 'Bruto besteld (status=ALL, incl. eventuele annuleringen/retouren) — niet de netto-omzet.' : 'Geen orders ontvangen — kan kloppen (geen verkoop) of de Orders-API leverde niets.')
    },
    buyBox: {
      ok: buyboxMet.length > 0,
      gemeten: buyboxMet.length,
      gevraagd: Math.min(topN, buyboxBron.length),
      note: buyboxMet.length ? null : 'Geen buy-box %/bezoeken ontvangen — Insights-API per offer leverde niets of er zijn geen offers (ververs de offer-map).'
    },
    performance: {
      ok: performance.length > 0,
      aantal: performance.length,
      note: performance.length ? null : 'Geen performance-indicatoren ontvangen van bol.'
    },
    offerMap: {
      ok: offerMapCount > 0,
      count: offerMapCount,
      note: offerMapCount ? null : 'Geen offer-map — ververs die in Voorraad-sync (nodig voor buy-box + prijspariteit).'
    }
  };
  ...
  const result = {
    ...
    stuks: sales.stuks,
    orders: sales.orders,
    omzetLaatste7: r2(laatste7),
    omzetVorige7: r2(vorige7),
    omzetDeltaPct,
    trend: dagen,
    buyBoxRate,
    potentieleImpact,
