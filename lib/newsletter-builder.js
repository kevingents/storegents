...
import { createAbTest, getAbTest, patchAbTest, variantFor } from './ab-test-store.js';
...
/* UTM-tag toevoegen aan een link zodat Shopify-orders aan deze nieuwsbrief
   toegeschreven kunnen worden (omzet-attributie). */
function withUtm(url, campaign) {
  const u = clean(url);
  if (!u || !/^https?:\/\//i.test(u) || !campaign) return u;
  const sep = u.includes('?') ? '&' : '?';
  return `${u}${sep}utm_source=nieuwsbrief&utm_medium=email&utm_campaign=${encodeURIComponent(campaign)}`;
}

function renderBlock(b, theme, campaign) {
  const t = { buttonBg: '#071B3A', textColor: '#111111', ...(theme || {}) };
  const U = (u) => withUtm(u, campaign);
  switch (b && b.type) {
    case 'hero':
      return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 18px"><tr><td>
        ...
        ${b.buttonLabel ? ctaButton(b.buttonLabel, U(b.buttonUrl), theme) : ''}
      </td></tr></table>`;
    case 'text':
      ...
      const cell = (p) => `<td width="50%" valign="top" style="padding:0 6px 12px">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #ececec;border-radius:10px;overflow:hidden"><tr><td>
          ...
          <div style="padding:12px 14px;font-family:${FONT}">
            ...
            ${p.url ? `<a href="${esc(U(p.url))}" style="color:${esc(t.buttonBg)};text-decoration:none;font-size:13px;font-weight:600;border-bottom:2px solid ${esc(t.buttonBg)}">Bekijk →</a>` : ''}
          </div>
        </td></tr></table>
      </td>`;
      const rowsHtml = [];
    ...
    case 'banner':
      return b.image
        ? `<div style="margin:0 0 18px">${b.url ? `<a href="${esc(U(b.url))}">` : ''}<img src="${esc(b.image)}" width="528" alt="" style="display:block;width:100%;border-radius:10px;border:0">${b.url ? '</a>' : ''}</div>`
        : '';
    case 'button':
      return `<div style="text-align:center;margin:6px 0 18px">${ctaButton(b.label, U(b.url), theme)}</div>`;
    case 'divider':
...
export function renderNewsletterHtml(nl, theme) {
  const campaign = nl.id || '';
  const body = (nl.blocks || []).map((b) => renderBlock(b, theme, campaign)).join('');
...
/* ── A/B-test op het onderwerp ── */
const validEmail = (e) => /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(clean(e).toLowerCase());
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

/* Start: stuur onderwerp A naar steekproef A en B naar steekproef B (op hash). */
export async function startNewsletterAbTest(newsletterId, { subjectA, subjectB, samplePct }) {
  const nl = await getNewsletter(newsletterId);
  if (!nl) throw new Error('Nieuwsbrief niet gevonden.');
  if (!clean(subjectA) || !clean(subjectB)) throw new Error('Vul beide onderwerpen in.');
  const test = await createAbTest({ newsletterId, subjectA, subjectB, samplePct });
  const [theme, senderCfg] = await Promise.all([getEmailTheme(), getStoreSenderConfig()]);
  const html = renderNewsletterHtml(nl, theme);
  const from = storeFromAddress('', senderCfg);
  const counts = { A: 0, B: 0 };
  const seen = new Set();
  const cap = 1500; /* steekproef-plafond per run */
  outer:
  for (let page = 1; page <= 60; page++) {
    let batch; try { batch = await getCustomers({ page, pageSize: 500 }); } catch (_) { break; }
    ...
    for (const c of rows) {
      ...
      if (!allow || !validEmail(email) || seen.has(email)) continue;
      seen.add(email);
      const v = variantFor(email, test.samplePct);
      if (v === 'remainder') continue;
      try {
        await sendGentsMail({
          to: email, subject: v === 'A' ? test.subjectA : test.subjectB, html, from,
          headers: { 'List-Unsubscribe': `<mailto:afmelden@${senderCfg.domain}>` },
          tags: [{ name: 'abtest', value: test.id }, { name: 'abvariant', value: v }],
          type: 'nieuwsbrief-abtest', meta: { newsletter: newsletterId, abtest: test.id, variant: v }
        });
        counts[v]++;
        if ((counts.A + counts.B) >= cap) break outer;
        await sleep(160);
      } catch (_) { /* skip */ }
    }
    if (rows.length < 500) break;
  }
  return patchAbTest(test.id, { sentCounts: { A: counts.A, B: counts.B, remainder: 0 } });
}

/* Stuur het winnende onderwerp naar de rest (in batches; cursor onthouden). */
export async function sendNewsletterAbWinner(testId, { variant, maxPerRun = 400 } = {}) {
  const test = await getAbTest(testId);
  if (!test) throw new Error('A/B-test niet gevonden.');
  const nl = await getNewsletter(test.newsletterId);
  if (!nl) throw new Error('Nieuwsbrief niet gevonden.');
  const winner = variant === 'A' || variant === 'B' ? variant : ((test.opens?.B || 0) > (test.opens?.A || 0) ? 'B' : 'A');
  const subject = winner === 'A' ? test.subjectA : test.subjectB;
  const [theme, senderCfg] = await Promise.all([getEmailTheme(), getStoreSenderConfig()]);
  const html = renderNewsletterHtml(nl, theme);
  const from = storeFromAddress('', senderCfg);
  let startPage = Math.max(1, Number(test.remainderCursorPage) || 1);
  let sent = 0, page = startPage, done = false;
  for (; page <= startPage + 120; page++) {
    let batch; try { batch = await getCustomers({ page, pageSize: 500 }); } catch (_) { break; }
    const rows = Array.isArray(batch) ? batch : (batch?.customers || batch?.rows || []);
    ...
    for (const c of rows) {
      const allow = c.allowMailings === true || String(c.allowMailings).toLowerCase() === 'true';
      const email = clean(c.email).toLowerCase();
      if (!allow || !validEmail(email)) continue;
      if (variantFor(email, test.samplePct) !== 'remainder') continue;
      try {
        await sendGentsMail({
          to: email, subject, html, from,
          headers: { 'List-Unsubscribe': `<mailto:afmelden@${senderCfg.domain}>` },
          tags: [{ name: 'abtest', value: test.id }, { name: 'abvariant', value: 'winner' }],
          type: 'nieuwsbrief-abwinner', meta: { newsletter: test.newsletterId, abtest: test.id }
        });
        sent++;
        await sleep(160);
      } catch (_) { /* skip */ }
    ...
    if (sent >= maxPerRun) { page++; break; }
  }
  const patch = {
    winner, decidedAt: new Date().toISOString(),
    remainderCursorPage: done ? page : page,
    sentCounts: { ...(test.sentCounts || {}), remainder: (test.sentCounts?.remainder || 0) + sent },
    status: done ? 'done' : 'sending-remainder'
  };
  await patchAbTest(testId, patch);
  return { ok: true, winner, sent, done };
}
