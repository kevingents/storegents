 ...
 */
/* Sla evidence op via disputeEvidenceUpdate.
 * Shopify's mutation heeft GEEN submit-argument — evidence wordt opgeslagen
 * als concept. De admin klikt daarna in Shopify Admin op "Submit evidence"
 * om het definitief in te dienen bij de bank. */
export async function submitDisputeEvidence(disputeId, input) {
  ...
  const data = await shopifyGql(mutation, { id, input });
...
export async function handleDispute(dispute, allReturnRequests, opts = {}) {
  ...
  try {
    const updated = await submitDisputeEvidence(evidenceId, evidenceInput);
    result.updatedEvidence = updated;
    result.message = `Evidence opgeslagen voor dispute ${dispute.id}. Ga naar Shopify Admin → Finances → Disputes om definitief in te dienen.`;
  } catch (e) {
    result.ok = false;
