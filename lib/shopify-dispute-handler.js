  ...
  const query = `#graphql
    query OpenDisputes($first: Int!) {
      shopifyPaymentsAccount {
        disputes(first: $first, query: "status:needs_response OR status:under_review") {
          edges {
            node {
              id
              type
 ...
 *
 * @param {string} evidenceId  De ID van het disputeEvidence-object (gid://shopify/...)
 ...
 */
export async function submitDisputeEvidence(evidenceId, input, { submit = false } = {}) {
  const id = clean(evidenceId);
  if (!id) throw new Error('evidenceId verplicht.');
...
export async function handleDispute(dispute, allReturnRequests, opts = {}) {
  ...
  if (!evidenceId) {
    result.ok = false;
    result.message = `Geen disputeEvidence.id gevonden — dispute ${dispute.id} heeft mogelijk al een andere status.`;
    return result;
  }
