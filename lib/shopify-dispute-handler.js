...
export function getShopifyCfg() {
  const domain = clean(
    ...
    process.env.SHOPIFY_STORE_URL ||      /* gents-production.myshopify.com */
    process.env.SHOPIFY_DOMAIN ||
    ''
  ).replace(/^https?:\/\//, '').replace(/\/$/, '');
  const token = clean(process.env.SHOPIFY_ADMIN_ACCESS_TOKEN || process.env.SHOPIFY_ADMIN_API_TOKEN || '');
  if (!domain || !token) throw new Error('Shopify-credentials ontbreken (SHOPIFY_STORE_URL / SHOPIFY_ADMIN_ACCESS_TOKEN).');
...
export async function submitDisputeEvidence(disputeId, input, { submit = false } = {}) {
  const numId = String(disputeId).replace('gid://shopify/ShopifyPaymentsDispute/', '');
  if (!numId) throw new Error('disputeId verplicht.');

  /* Pogingsvolgorde:
     1. GraphQL disputeEvidenceUpdate — gebruikt write_shopify_payments_disputes
        scope die beschikbaar is in custom apps.
     2. REST PUT — vereist shopify_payments_dispute_evidences scope die NIET
        beschikbaar is in Shopify Admin custom app builder → fallback met
        nuttige foutmelding. */

  const gid = `gid://shopify/ShopifyPaymentsDispute/${numId}`;
  const gqlInput = {};
  if (input.uncategorized_text) gqlInput.uncategorizedText = input.uncategorized_text;
  if (input.customer_email_address) gqlInput.customerEmailAddress = input.customer_email_address;
  if (input.refund_refusal_explanation) gqlInput.refundRefusalExplanation = input.refund_refusal_explanation;
  if (input.customer_first_name) gqlInput.customerFirstName = input.customer_first_name;
  if (input.customer_last_name) gqlInput.customerLastName = input.customer_last_name;

  /* Stap 1: haal het ShopifyPaymentsDisputeEvidence-ID op via node-query.
     De disputeEvidenceUpdate mutatie verwacht het EVIDENCE-ID, niet het dispute-ID. */
  let evidenceGid = null;
  let nodeQueryError = null;
  try {
    const nodeQuery = `#graphql
      query GetDisputeEvidence($id: ID!) {
        node(id: $id) {
    ...
    const nodeData = await shopifyGql(nodeQuery, { id: gid });
    evidenceGid = nodeData?.node?.disputeEvidence?.id || null;
  } catch (e) {
    nodeQueryError = e.message;
  }

  /* Stap 2: GQL mutation met evidenceGid (of dispute GID als fallback). */
  const mutationId = evidenceGid || gid;
  let gqlError = null;
  try {
    ...
    const data = await shopifyGql(mutation, { id: mutationId, input: gqlInput });
    const errs = data?.disputeEvidenceUpdate?.userErrors || [];
    if (errs.length) throw new Error(`GQL userErrors: ${errs.map((e) => e.message).join('; ')}`);
    const evidence = data?.disputeEvidenceUpdate?.disputeEvidence;
    if (!evidence) throw new Error('GQL: geen disputeEvidence in response');

    /* Submit via REST als scope beschikbaar (optioneel). */
    if (submit) {
      try {
        await shopifyRest(`/shopify_payments/disputes/${numId}/dispute_evidences.json`,
          { method: 'PUT', body: { dispute_evidence: { submit_evidence: true } }, disputeVersion: true });
      } catch {}
    }
    return { ...evidence, _via: 'graphql', nodeQueryError };
  } catch (e) {
    gqlError = e.message;
  }

  /* Stap 3: REST fallback (vereist shopify_payments_dispute_evidences scope). */
  try {
    const body = { dispute_evidence: { ...input, submit_evidence: submit } };
    const data = await shopifyRest(`/shopify_payments/disputes/${encodeURIComponent(numId)}/dispute_evidences.json`,
      { method: 'PUT', body, disputeVersion: true });
    return { ...(data?.dispute_evidence || {}), _via: 'rest' };
  } catch (restErr) {
    /* Gooi gecombineerde fout zodat de caller alle context heeft. */
    throw new Error(
      `Alle pogingen gefaald:\n` +
      `  node-query: ${nodeQueryError || 'ok (evidenceGid=${evidenceGid})'}\n` +
      `  GQL mutation (id=${mutationId}): ${gqlError}\n` +
