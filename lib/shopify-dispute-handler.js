...
async function shopifyGql(query, variables = {}) {
  ...
  if (!resp.ok) throw new Error(`Shopify ${resp.status}: ${JSON.stringify(data).slice(0, 300)}`);
...
export async function fetchOpenDisputes({ limit = 50 } = {}) {
  /* disputeEvidence bestaat niet als subveld in de Shopify GraphQL API —
     evidence wordt via de dispute-ID apart geladen bij submitDisputeEvidence.
     Filter op needs_response + under_review via aparte queries want OR-syntax
     wordt niet ondersteund in disputes query filter. */
  const query = `#graphql
    query OpenDisputes($first: Int!) {
  ...
  const data = await shopifyGql(query, { first: limit });
  const edges = data?.shopifyPaymentsAccount?.disputes?.edges || [];
  /* Filter client-side op actionable statussen. */
  return edges
    .map((e) => e.node)
    .filter((d) => ['NEEDS_RESPONSE', 'UNDER_REVIEW'].includes((d.status || '').toUpperCase()));
}

/* ─── Returnista-bewijs opbouwen ────────────────────────────────────────── */

/**
 * Vind de Returnista return-request(s) die horen bij een Shopify-ordernummer.
 * Geeft alle items terug (1 order kan meerdere retouren hebben).
 */
export async function findReturnistaRetourForOrder(orderName, allReturnRequests) {
  const nr = clean(orderName).replace(/^#/, '').toLowerCase();
  return (allReturnRequests || []).filter((r) => {
    const rNr = clean(r.purchaseOrderNumber || r.orderNr || '').replace(/^#/, '').toLowerCase();
    return rNr === nr;
  });
}
...
export async function handleDispute(dispute, allReturnRequests, opts = {}) {
  ...
  /* Stap 1: zoek retour in Returnista. */
  const returnItems = await findReturnistaRetourForOrder(orderName, allReturnRequests);
  ...
  /* Gebruik het dispute-ID direct voor de evidence-mutatie. */
  const evidenceId = dispute.id;
