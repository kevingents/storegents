...
export async function submitDisputeEvidence(disputeId, input, { submit = false } = {}) {
  ...
  try {
    /* De GraphQL input gebruikt camelCase-veldnamen. */
    const gqlInput = {};
    ...
    const data = await shopifyGql(mutation, { id: gid, input: gqlInput });
    ...
    /* Als submit=true: nog een GQL-call of REST-call om te submitten. */
    if (submit) {
      /* Probeer REST submit_evidence=true — werkt alleen als scope aanwezig is. */
      try {
        await shopifyRest(
          `/shopify_payments/disputes/${numId}/dispute_evidences.json`,
          { method: 'PUT', body: { dispute_evidence: { submit_evidence: true } }, disputeVersion: true }
        );
      } catch (submitErr) {
        console.warn(`[dispute] evidence opgeslagen maar submit mislukt: ${submitErr.message}`);
        /* Ga verder — evidence is tenminste opgeslagen. */
      }
    }
    return evidence;
  } catch (gqlErr) {
    console.warn(`[dispute] GQL poging mislukt: ${gqlErr.message} — probeer REST`);

    /* Poging 2: REST (vereist shopify_payments_dispute_evidences scope) */
    const body = { dispute_evidence: { ...input, submit_evidence: submit } };
    const data = await shopifyRest(
      `/shopify_payments/disputes/${encodeURIComponent(numId)}/dispute_evidences.json`,
