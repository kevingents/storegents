...
export async function debugGetEvidence(disputeId) {
  ...
  const paths = [
    ...
    `/shopify_payments/disputes/${numId}/dispute_evidence.json`
  ];
  const results = {};
  for (const path of paths) {
    for (const ver of ['2026-01', '2025-01', '2024-10']) {
      const key = `${ver}:${path.split('/').pop()}`;
      try {
        const cfg = getShopifyCfg();
        const url = `https://${cfg.domain}/admin/api/${ver}${path}`;
        const resp = await fetch(url, { headers: { 'X-Shopify-Access-Token': cfg.token, Accept: 'application/json' } });
        const data = await resp.json().catch(() => ({}));
        results[key] = { status: resp.status, ok: resp.ok, data };
        if (resp.ok) return { ok: true, path, version: ver, data };
      } catch (e) {
        results[key] = { error: e.message };
      }
    }
  }
  return { ok: false, tried: results, numId };
}
