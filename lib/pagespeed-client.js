...
function targetUrls() {
  ...
  return [
    { url: 'https://www.gents.nl/', label: 'Homepage' },
    { url: 'https://www.gents.nl/collections/all', label: 'Alle producten' }
  ];
}
...
async function runOne(target, key) {
  ...
  try {
    const resp = await fetch(`${PSI_URL}?${params.toString()}`, { signal: ctrl.signal });
