...
export function getBolConfig() {
  ...
  /* Default v10 — vrijwel de hele Retailer-API (orders/returns/shipments/content/
     insights/invoices) draait op v10. ALLEEN de offer-api (/offers, /offers/export)
     + /shared/process-status zitten op v11 (zie bolOffersVersion). v11 op een
     v10-endpoint geeft 400 "media type not supported"; v10 op een offer-endpoint
     geeft 403 "Unauthorized request". Override: BOL_API_VERSION. */
...
/* Bol versioneert PER functioneel gebied. De ORDER-API draait op v10 — v11 geeft
   daar HTTP 400 "The supplied accept header media type is not supported". De
   offer-api/process-status draaien juist op v11 (v10 → 403). De globale default
   (v11) blijft dus voor offers; order-calls geven expliciet v10 mee via deze
   helper. Override mogelijk met env BOL_ORDERS_API_VERSION. */
export function bolOrdersVersion() {
  return clean(process.env.BOL_ORDERS_API_VERSION) || 'v10';
}

/* De order-WRITES (POST /shipments, PUT cancellation) zitten net als de reads op
   v10. Override mogelijk via BOL_ORDERS_WRITE_API_VERSION. */
export function bolOrdersWriteVersion() {
  return clean(process.env.BOL_ORDERS_WRITE_API_VERSION) || 'v10';
}

/* De offer-api (/offers, /offers/{id}/price, /offers/{id}/stock, /offers/export)
   én /shared/process-status zitten op v11 — op v10 geven die 403. Override:
   BOL_OFFERS_API_VERSION. (Alle andere endpoints draaien op de globale v10.) */
export function bolOffersVersion() {
  return clean(process.env.BOL_OFFERS_API_VERSION) || 'v11';
}
...
/* Public: forceer een nieuwe token-fetch bij next call (voor stale-recovery
   in offer-export e.d., zonder withAuth's interne 401/403-retry te wachten). */
export function invalidateBolToken() {
  __tok = { value: '', exp: 0 };
  __tokPromise = null;
}
...
export async function bolGetCsv(path, { version } = {}) {
  const cfg = getBolConfig();
  if (!cfg.configured) throw new Error(`bol niet gekoppeld — ontbrekend: ${cfg.missing.join(', ')}`);
  const ver = clean(version) || cfg.version;
  const url = `${cfg.base}${cfg.prefix}${path}`;
  const resp = await withAuth(cfg, (token) => [url, {
    headers: { Authorization: `Bearer ${token}`, Accept: `application/vnd.retailer.${ver}+csv`, 'Accept-Language': ACCEPT_LANGUAGE }
  }]);
  const text = await resp.text();
...
async function safeProcessStatus(id) {
  try {
    return await bolGet(`/shared/process-status/${id}`, { version: bolOffersVersion() });
  } catch (e) {
 ...
 */
export async function bolPost(path, body, { method = 'POST', version } = {}) {
  const cfg = getBolConfig();
  if (!cfg.configured) throw new Error(`bol niet gekoppeld — ontbrekend: ${cfg.missing.join(', ')}`);
  const ver = clean(version) || cfg.version;
  const url = `${cfg.base}${cfg.prefix}${path}`;
  ...
  const resp = await withAuth(cfg, (token) => [url, {
    ...
    headers: {
      ...
      Accept: `application/vnd.retailer.${ver}+json`,
      'Content-Type': `application/vnd.retailer.${ver}+json`,
