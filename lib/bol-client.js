...
export function getBolConfig() {
  ...
  /* Default v11 — Bol heeft v10 deprecated voor offers/export en
     process-status. v10 calls geven daar 403 "Unauthorized request" terug.
     Override via env BOL_API_VERSION=v10 als je tijdelijk terug moet. */
  const version = clean(process.env.BOL_API_VERSION) || 'v11';
...
/* De WRITE-endpoints van de order-API (PUT shipment/cancellation) zitten op v11:
   op v10 geven die 403 "Unauthorized request" (zelfde signaal als de offer-api).
   De GET-reads (bolOrdersVersion) blijven v10. Override: BOL_ORDERS_WRITE_API_VERSION. */
export function bolOrdersWriteVersion() {
  return clean(process.env.BOL_ORDERS_WRITE_API_VERSION) || 'v11';
}
