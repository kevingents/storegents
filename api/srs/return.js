...
function buildSrsItems({ selectedItems, refundLineItems, orderLineItems }) {
  return refundLineItems.map((refundLineItem) => {
    ...
    return {
      ...
      sku: selected?.sku || lineItem?.sku || '',
      barcode: selected?.sku || lineItem?.sku || '',
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const refundLineItems = selectedItems
      .map((selectedItem) => {
        ...
        return {
          ...
          // Belangrijk:
          // SRS is leidend voor voorraad. Shopify mag hier NIET ook restocken,
          // anders krijg je dubbele voorraad. SRS Return boekt voorraad op srsBranchId.
    ...
    const missingSkuItems = srsItems.filter((item) => !item.sku && !item.barcode && !item.fulfillmentId && !item.orderLineNr);
