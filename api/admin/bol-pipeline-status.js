...
export default async function handler(req, res) {
  ...
  try {
    const [bolData, pushedState, shippedState, cancelState, labels] = await Promise.all([
    ...
    const rows = [];
    const counts = { opgehaald: 0, naarSrs: 0, label: 0, verzonden: 0, geannuleerd: 0, wachtOpLabel: 0, wachtOpSrs: 0, wachtOpShip: 0 };
    ...
    for (const o of orders) {
      ...
      if (isCancelled) {
        ...
        fase = 'wacht-op-srs';
        nextAction = 'bol-srs-sync moet deze nog naar SRS pushen (draait :20). Check failures.';
        counts.wachtOpSrs += 1;
      } else if (!hasLabel) {
        fase = 'wacht-op-label';
    ...
    const faseOrder = { 'wacht-op-srs': 0, 'wacht-op-label': 1, 'wacht-op-verzending': 2, 'verzonden': 3, 'geannuleerd': 4 };
    ...
    const filterMap = { 'needs-label': 'wacht-op-label', 'needs-srs': 'wacht-op-srs', 'needs-ship': 'wacht-op-verzending' };
