...
function key(value) {
  return clean(value).toLowerCase().replace(/^\d+\s*-\s*/i, '').replace(/\s+/g, ' ');
}

function typeForLocation(store) {
  const value = key(store);
...
function dateValue(row = {}) {
  return row.createdAt || row.updatedAt || row.date || row.cancelledAt || row.orderDate || row.created || '';
}
...
function addMetric(target, store, metric, amount = 1) {
  const item = target.get(key(store));
  if (!item) return;
...
function storeFromCancellation(row = {}) {
  ...
  return clean(item.lastResponsibleStore || row.lastResponsibleStore || row.store || item.store || item.currentBranch || item.originBranch || 'Onbekend');
}
...
function flattenCancellations(records = []) {
  return records.flatMap((record) => {
    ...
    return items.map((item, index) => ({
      ...record,
      lineIndex: index,
      store: clean(item.lastResponsibleStore || record.store || item.currentBranch || item.originBranch || 'Onbekend'),
...
async function getWeborderRows() {
  try {
    ...
    return (result.items || [])
      ...
      .filter((row) => isOpenWeborderStatus(row.status || row.srsStatus || row.fulfillmentStatus || 'open'));
  } catch (error) {
...
function storeFromWeborder(row = {}) {
  return clean(row.currentStore || row.fulfilmentStore || row.fulfillmentStore || row.huidigFiliaalNaam || row.store || row.branchName || 'Onbekend');
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [weborders, dragerRows, cancellations, unavailable] = await Promise.all([
      getWeborderRows(),
      getDragerCache(),
      getOrderCancellations().catch((error) => {
        console.error('[admin/dashboard/location-overview] cancellations failed:', error);
        return [];
      }),
      listUnavailableOrderLines({ status: 'open' }).catch((error) => {
        console.error('[admin/dashboard/location-overview] unavailable failed:', error);
        return { rows: [] };
      })
    ]);
    ...
    flattenCancellations(cancellations).forEach((row) => {
      ...
      const store = storeFromCancellation(row);
      addMetric(locations, store, 'openExchanges', 1);
      if (isLate(row, 48)) addMetric(locations, store, 'lateExchanges', 1);
    });

    (unavailable.rows || []).forEach((row) => {
      const store = clean(row.store || row.lastResponsibleStore || row.currentBranch || 'Onbekend');
    ...
    const rows = Array.from(locations.values())
      .map((row) => ({
        ...row,
