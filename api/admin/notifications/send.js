...
export default async function handler(req, res) {
  ...
  try {
    ...
    let pushResults = { attempted: 0, sent: 0, removed: 0 };
    try {
      const subs = await getSubscriptionsForStores(stores);
      pushResults.attempted = subs.length;
      ...
      pushResults.sent = results.filter((r) => r.sent).length;
      pushResults.removed = results.filter((r) => r.removed).length;
    } catch (e) {
      console.error('[notifications/send] push fail:', e);
    }

    return res.status(200).json({ success: true, notification, push: pushResults });
  } catch (error) {
