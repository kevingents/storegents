...
export default async function handler(req, res) {
  ...
  try {
    ...
    /* Magazijnen = filialen die in de locaties-data voorkomen (daar worden
       bin-locaties getrackt). */
    const magazijnFilialen = new Set(locRows.map((r) => r.filiaalNummer));
    ...
    if (store && type) {
      ...
      return res.status(400).json({ success: false, message: 'type moet zonder-locatie of spook zijn.' });
    }
    ...
    magazijnen.sort((a, b) => b.zonderLocatie - a.zonderLocatie);
